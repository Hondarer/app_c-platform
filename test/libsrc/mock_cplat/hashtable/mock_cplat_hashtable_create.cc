               ...
               size_t buf_mgmt_size, void *buf_data, size_t buf_data_size, cplat_hashtable **ht_out)
{
    ...
    {
        mock_ret =
            _mock_cplat->cplat_hashtable_create(config, buf_mgmt, buf_mgmt_size, buf_data, buf_data_size, ht_out);
    }
    else
    {
        mock_ret =
            delegate_real_cplat_hashtable_create(config, buf_mgmt, buf_mgmt_size, buf_data, buf_data_size, ht_out);
    }
