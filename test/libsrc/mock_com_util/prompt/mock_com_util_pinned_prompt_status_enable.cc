...
int delegate_real_com_util_pinned_prompt_status_enable(com_util_pinned_prompt_t *screen,
                                                       com_util_pinned_prompt_status_position_t position,
                                                       int enable)
{
    static auto real_fn =
        reinterpret_cast<decltype(&com_util_pinned_prompt_status_enable)>(
            resolveSharedSymbolOrExit(kLibComUtilName, "com_util_pinned_prompt_status_enable"));

    return real_fn(screen, position, enable);
}

MOCK_WEAK_IMPL(int, com_util_pinned_prompt_status_enable, com_util_pinned_prompt_t *screen,
