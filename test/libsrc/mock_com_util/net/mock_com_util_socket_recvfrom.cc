...
MOCK_WEAK_IMPL(int, com_util_socket_recvfrom, com_util_socket sock, void *buf, size_t len, com_util_ipv4_endpoint *peer_out, size_t *received_out, com_util_error *detail_out)
{
    ...
    {
        rtc = _mock_com_util->com_util_socket_recvfrom(sock, buf, len, peer_out, received_out, detail_out);
    }
    else
    {
        rtc = delegate_real_com_util_socket_recvfrom(sock, buf, len, peer_out, received_out, detail_out);
    }
