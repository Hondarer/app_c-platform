...
MOCK_WEAK_IMPL(int, cplat_socket_get_pending_error, cplat_socket sock, cplat_error *detail_out)
{
    int mock_ret = CPLAT_ERR_UNKNOWN;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_socket_get_pending_error(sock, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_socket_get_pending_error(sock, detail_out);
    }
