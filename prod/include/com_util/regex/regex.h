...
#ifndef COM_UTIL_REGEX_REGEX_H
#define COM_UTIL_REGEX_REGEX_H
...
/**
 *  @ingroup        COM_UTIL_REGEX
 *  @{
 */

/** @brief 不参加の捕捉グループを表すオフセット値。 */
#define COM_UTIL_REGEX_NPOS ((size_t)-1)

/** @brief 入力またはパターンとして受け付ける最大バイト数。 */
#define COM_UTIL_REGEX_MAX_LENGTH (1024U * 1024U)

/**
 *  @name           コンパイル フラグ
 *  @brief          @ref com_util_regex_create() の `flags` に指定するビット値です。
 *  @{
 */
#define COM_UTIL_REGEX_DEFAULT  0x0000U /**< ECMAScript 方言、大小の区別あり。 */
#define COM_UTIL_REGEX_EXTENDED 0x0001U /**< POSIX 拡張正規表現 (ERE) として解釈する。 */
#define COM_UTIL_REGEX_BASIC    0x0002U /**< POSIX 基本正規表現 (BRE) として解釈する。 */
#define COM_UTIL_REGEX_ICASE    0x0004U /**< 大小を区別しない (ASCII 範囲のみ)。 */
#define COM_UTIL_REGEX_NOSUB    0x0008U /**< 捕捉グループを記録しない。 */
#define COM_UTIL_REGEX_OPTIMIZE 0x0010U /**< 構築に時間をかけて照合を高速化する。 */
/** @} */
 ...
 *  @{
 */
#define COM_UTIL_REGEX_MATCH_DEFAULT  0x0000U /**< 既定の照合を行う。 */
#define COM_UTIL_REGEX_MATCH_NOTBOL   0x0001U /**< 照合開始位置を行頭とみなさない。 */
#define COM_UTIL_REGEX_MATCH_NOTEOL   0x0002U /**< 入力末尾を行末とみなさない。 */
#define COM_UTIL_REGEX_MATCH_NOTEMPTY 0x0004U /**< 空文字列への一致を認めない。 */
#define COM_UTIL_REGEX_MATCH_ANCHORED 0x0008U /**< 照合開始位置に固定して照合する。 */
/** @} */

/**
 *  @name           置換フラグ
 *  @brief          @ref com_util_regex_replace() の `flags` に、照合フラグと合わせて
 *                  指定するビット値です。
 *  @{
 */
#define COM_UTIL_REGEX_REPLACE_FIRST_ONLY 0x0100U /**< 最初に一致した箇所のみを置換する。 */
#define COM_UTIL_REGEX_REPLACE_NO_COPY    0x0200U /**< 一致しなかった部分を出力しない。 */
#define COM_UTIL_REGEX_REPLACE_SED        0x0400U /**< 置換文字列を sed の書式で解釈する。 */
/** @} */
...
extern "C"
{
    ...
    typedef struct com_util_regex com_util_regex;
     ...
     *                  @ref COM_UTIL_ERR_INVALID_ENCODING 、@ref COM_UTIL_ERR_INVALID_PATTERN 、
     *                  @ref COM_UTIL_ERR_UNSUPPORTED 、@ref COM_UTIL_ERR_LIMIT_EXCEEDED 、
     *                  @ref COM_UTIL_ERR_OUT_OF_MEMORY 、@ref COM_UTIL_ERR_UNKNOWN のいずれかを返します。
     *
     *  @ref COM_UTIL_REGEX_EXTENDED と @ref COM_UTIL_REGEX_BASIC の同時指定、および
     *  未定義のビットの指定は @ref COM_UTIL_ERR_INVALID_ARGUMENT を返します。\n
     *  `pattern` が @ref COM_UTIL_REGEX_MAX_LENGTH バイトを超える場合は
     *  @ref COM_UTIL_ERR_LIMIT_EXCEEDED を返します。\n
     *  生成したハンドルは @ref com_util_regex_dispose() で破棄してください。
     *
     *  @note           @ref COM_UTIL_REGEX_ICASE の大小の畳み込みは ASCII 範囲に限ります。
     *                  `Ä` と `ä` は一致しません。\n
     ...
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_create(const char *pattern, unsigned int flags,
     ...
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_regex_dispose(com_util_regex *regex);
     ...
     *  @ref COM_UTIL_REGEX_NOSUB を指定してコンパイルした場合は 1 を返します。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  ハンドルを変更しません。
     */
    COM_UTIL_EXPORT size_t COM_UTIL_API com_util_regex_get_group_count(const com_util_regex *regex);
     ...
     *  @param[in]      match_flags       照合フラグ (@ref COM_UTIL_REGEX_MATCH_DEFAULT など) のビット和。
     ...
     *                  NULL 以外を指定した場合、成功時は空の値を格納します。
     *  @return         @ref COM_UTIL_OK 、@ref COM_UTIL_ERR_INVALID_ARGUMENT 、
     *                  @ref COM_UTIL_ERR_INVALID_ENCODING 、@ref COM_UTIL_ERR_LIMIT_EXCEEDED 、
     *                  @ref COM_UTIL_ERR_OUT_OF_MEMORY 、@ref COM_UTIL_ERR_UNKNOWN のいずれかを返します。
     *
     *  一致しなかったことはエラーではありません。戻り値は @ref COM_UTIL_OK となり、
     *  `matched_out` に 0 を格納します。\n
     *  `matches_out` の要素数が @ref com_util_regex_get_group_count() の戻り値より少ない場合、
     ...
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_search(const com_util_regex *regex, const char *text,
     ...
     *                  @ref COM_UTIL_ERR_OUT_OF_MEMORY 、@ref COM_UTIL_ERR_UNKNOWN のいずれかを返します。
     *
     *  @ref com_util_regex_search() と異なり、入力の先頭から末尾までがパターンに
     ...
     *  ハンドルを変更せず、内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_matches(const com_util_regex *regex, const char *text,
                                                            size_t text_len, unsigned int match_flags,
                                                            com_util_regex_match *matches_out, size_t matches_capacity,
     ...
     *                  NULL 以外を指定した場合、成功時は空の値を格納します。
     *  @return         @ref COM_UTIL_OK 、@ref COM_UTIL_ERR_INVALID_ARGUMENT 、
     *                  @ref COM_UTIL_ERR_INVALID_ENCODING 、@ref COM_UTIL_ERR_BUFFER_TOO_SMALL 、
     *                  @ref COM_UTIL_ERR_LIMIT_EXCEEDED 、@ref COM_UTIL_ERR_OUT_OF_MEMORY 、
     ...
     *  `result_size` が不足する場合は @ref COM_UTIL_ERR_BUFFER_TOO_SMALL を返し、
     *  `required_size_out` に必要なバイト数を格納します。
     *
     *  @note           置換文字列の書式は既定で ECMAScript の書式 (`$1` `$&` `` $` `` `$'` `$$`) です。\n
     *                  @ref COM_UTIL_REGEX_EXTENDED や @ref COM_UTIL_REGEX_BASIC を指定して
     *                  コンパイルした場合も、置換文字列の書式は変わりません。\n
     *                  sed の書式を使用する場合は @ref COM_UTIL_REGEX_REPLACE_SED を指定してください。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  ハンドルを変更せず、内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_replace(const com_util_regex *regex, const char *text,
                                                            size_t text_len, const char *replacement,
                                                            unsigned int flags, char *result_out, size_t result_size,
                                                            size_t *required_size_out, com_util_error *detail_out);

    /** @brief 一致箇所を順に列挙するイテレーターのハンドル (不透明構造体)。 */
    typedef struct com_util_regex_iter com_util_regex_iter;
     ...
     *  生成したハンドルは @ref com_util_regex_iter_dispose() で破棄してください。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  呼び出しごとに独立したハンドルを生成します。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_iter_create(const com_util_regex *regex, const char *text,
                                                                size_t text_len, unsigned int match_flags,
                                                                com_util_regex_iter **iter_out,
                                                                com_util_error *detail_out);
     ...
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_iter_next(com_util_regex_iter *iter,
                                                              com_util_regex_match *matches_out,
     ...
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_regex_iter_dispose(com_util_regex_iter *iter);
     ...
     *  `parts_capacity` が不足する場合は @ref COM_UTIL_ERR_BUFFER_TOO_SMALL を返し、
     *  `part_count_out` に必要な件数を格納します。格納できる範囲までは `parts_out` へ書き込みます。\n
     *  `parts_out` に NULL、`parts_capacity` に 0 を指定した場合は、件数の問い合わせとして
     *  @ref COM_UTIL_OK を返し、`part_count_out` にのみ件数を格納します。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  ハンドルを変更せず、内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_regex_split(const com_util_regex *regex, const char *text,
                                                          size_t text_len, size_t max_parts, unsigned int match_flags,
                                                          com_util_regex_match *parts_out, size_t parts_capacity,
