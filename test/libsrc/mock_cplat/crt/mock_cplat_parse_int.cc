...
MOCK_WEAK_IMPL(int, cplat_parse_int, int *value_out, const char *text, int base)
{
    int mock_ret = -1;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_parse_int(value_out, text, base);
    }
    else
    {
        mock_ret = delegate_real_cplat_parse_int(value_out, text, base);
    }
