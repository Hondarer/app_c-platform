...
MOCK_WEAK_IMPL(int, cplat_hashtable_find_timestamp_ref, const cplat_hashtable *ht, const void *key,
               const cplat_timespec **timestamp_out)
{
    ...
    {
        mock_ret = _mock_cplat->cplat_hashtable_find_timestamp_ref(ht, key, timestamp_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_hashtable_find_timestamp_ref(ht, key, timestamp_out);
    }
