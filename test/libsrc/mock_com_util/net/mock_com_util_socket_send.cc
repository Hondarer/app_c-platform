...
MOCK_WEAK_IMPL(int, com_util_socket_send, com_util_socket sock, const void *buf, size_t len, size_t *sent_out, com_util_error *detail_out)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_socket_send(sock, buf, len, sent_out, detail_out);
    }
    else
    {
        mock_ret = delegate_real_com_util_socket_send(sock, buf, len, sent_out, detail_out);
    }
