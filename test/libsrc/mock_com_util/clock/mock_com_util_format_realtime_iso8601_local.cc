               ...
               const com_util_timespec *timestamp)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_format_realtime_iso8601_local(buf, buf_size, timestamp);
    }
    else
    {
        mock_ret = delegate_real_com_util_format_realtime_iso8601_local(buf, buf_size, timestamp);
    }
