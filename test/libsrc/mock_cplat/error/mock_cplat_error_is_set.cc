...
MOCK_WEAK_IMPL(int, cplat_error_is_set, const cplat_error *error)
{
    int mock_ret;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_error_is_set(error);
    }
    else
    {
        mock_ret = delegate_real_cplat_error_is_set(error);
    }
