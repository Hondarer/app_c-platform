...
size_t delegate_real_com_util_pinned_prompt_write(com_util_pinned_prompt_t *screen,
                                                  com_util_pinned_prompt_channel_t channel,
                                                  const void *data,
                                                  size_t size)
{
    static auto real_fn =
        reinterpret_cast<decltype(&com_util_pinned_prompt_write)>(
            resolveSharedSymbolOrExit(kLibComUtilName, "com_util_pinned_prompt_write"));

    return real_fn(screen, channel, data, size);
}

MOCK_WEAK_IMPL(size_t, com_util_pinned_prompt_write, com_util_pinned_prompt_t *screen,
