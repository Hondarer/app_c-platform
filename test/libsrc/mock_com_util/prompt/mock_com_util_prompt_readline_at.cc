               ...
               const char *file, int line)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_prompt_readline_at(p, buf, buf_size, prompt_str, file, line);
    }
    else
    {
        mock_ret = delegate_real_com_util_prompt_readline_at(p, buf, buf_size, prompt_str, file, line);
    }
