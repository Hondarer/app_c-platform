...
MOCK_WEAK_IMPL(int, com_util_open_fmt, int flags, int mode, com_util_error *detail_out, const char *format, ...)
{
    int mock_ret = -1;
    ...
    {
        mock_ret = _mock_com_util->com_util_open_fmt(flags, mode, detail_out, buf.data());
    }
    else
    {
        mock_ret = delegate_real_com_util_open_fmt(flags, mode, detail_out, buf.data());
    }
