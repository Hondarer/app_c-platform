...
TEST_F(timeTest, localtime_null_time)
{
    ...
    // Act
    // Assert
    EXPECT_EQ(-1, com_util_localtime(&local_tm, NULL)); // [手順] - 時刻に NULL を渡して com_util_localtime を呼び出す。
                                                        // [確認_異常系] - 戻り値が -1 であること。
}
...
TEST_F(timeTest, ctime_matches_platform_result)
{
    ...
    // Assert
    EXPECT_EQ(
        0, com_util_ctime(actual, sizeof(actual), &epoch)); // [手順] - com_util_ctime(actual, 26, &epoch) を呼び出す。
                                                            // [確認_正常系] - 戻り値が 0 であること。
...
TEST_F(timeTest, ctime_null_time_zeroes_buf)
{
    ...
    // Act
    // Assert
    EXPECT_EQ(-1, com_util_ctime(buf, sizeof(buf), NULL)); // [手順] - 時刻に NULL を渡して com_util_ctime を呼び出す。
                                                           // [確認_異常系] - 戻り値が -1 であること。
...
TEST_F(timeTest, ctime_small_buf_zeroes_buf)
{
    ...
    // Act
    // Assert
    EXPECT_EQ(-1, com_util_ctime(buf, sizeof(buf),
                                 &epoch)); // [手順] - サイズ 25 のバッファーを渡して com_util_ctime を呼び出す。
                                           // [確認_異常系] - 戻り値が -1 であること。
