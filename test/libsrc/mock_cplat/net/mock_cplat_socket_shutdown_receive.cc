...
MOCK_WEAK_IMPL(int, cplat_socket_shutdown_receive, cplat_socket *sock_inout, cplat_error *detail_out)
{
    int mock_ret = CPLAT_ERR_UNKNOWN;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_socket_shutdown_receive(sock_inout, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_socket_shutdown_receive(sock_inout, detail_out);
    }
