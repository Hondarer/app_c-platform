...
{
    static auto real_fn = reinterpret_cast<decltype(&com_util_vfopen_fmt)>(
               ...
               va_list args)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_vfopen_fmt(modes, detail_out, buf.data());
    }
    else
    {
        mock_ret = delegate_real_com_util_vfopen_fmt(modes, detail_out, format, args);
    }
