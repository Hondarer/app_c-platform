               ...
               const uint8_t *key, const uint8_t *nonce, const uint8_t *aad, size_t aad_len)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_decrypt(dst, dst_len, src, src_len, key, nonce, aad, aad_len);
    }
    else
    {
        mock_ret = delegate_real_com_util_decrypt(dst, dst_len, src, src_len, key, nonce, aad, aad_len);
    }
