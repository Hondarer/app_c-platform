...
TEST_F(hashtableConfigTest, buffer_size_rejects_null_arguments)
{
    ...
    int actual_ret_null_ht =
        cplat_hashtable_buffer_size(NULL, &mgmt_size, &data_size);          // [手順] - ht に NULL を渡す。
    int actual_ret_null_both = cplat_hashtable_buffer_size(ht, NULL, NULL); // [手順] - 両方の出力先に NULL を渡す。
    int actual_ret_only_mgmt =
        cplat_hashtable_buffer_size(ht, &mgmt_size, NULL); // [手順] - データ側だけ NULL を渡す。
    int actual_ret_only_data =
        cplat_hashtable_buffer_size(ht, NULL, &data_size);                       // [手順] - 管理側だけ NULL を渡す。
    int actual_ret_ok = cplat_hashtable_buffer_size(ht, &mgmt_size, &data_size); // [手順] - 妥当な引数で呼び出す。
    cplat_hashtable_dispose(ht);
    ...
    EXPECT_EQ(CPLAT_OK, actual_ret_ok);        // [確認_正常系] - 妥当な引数で取得できること。
