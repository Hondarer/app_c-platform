...
TEST_F(fgetsTest, returns_eof_at_end_of_stream)
{
    ...
    // Act
    int first_ret = com_util_fgets(buf, sizeof(buf), kStream, NULL); // [手順] - 1 回目の呼び出しで唯一の行を読み取る。
    int second_ret =
        com_util_fgets(buf, sizeof(buf), kStream, NULL); // [手順] - 2 回目の呼び出しを行末尾の状態で実行する。
...
TEST_F(fgetsTest, continues_reading_remainder_after_buffer_too_small)
{
    ...
    // Act
    int first_ret =
        com_util_fgets(buf, sizeof(buf), kStream, NULL); // [手順] - 1 回目の呼び出しで先頭 4 文字分を読み取らせる。
    int second_ret = com_util_fgets(buf, sizeof(buf), kStream, NULL); // [手順] - 2 回目の呼び出しで行の残りを読み取る。
