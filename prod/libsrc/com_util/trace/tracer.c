...
#if defined(PLATFORM_WINDOWS)
    com_util_trace_level etw_level;
...
static int registry_expand_locked(void)
{
    ...
    new_items = (com_util_tracer **)com_util_realloc(s_trace_registry.items, new_capacity, sizeof(com_util_tracer *));
    if (new_items == NULL)
    {
        return -1;
    }
 ...
 */
static int to_syslog_level(const com_util_trace_level lv)
{
    switch (lv)
    {
 ...
 */
static int to_etw_level(const com_util_trace_level lv)
{
    switch (lv)
    {
    case COM_UTIL_TRACE_LEVEL_CRITICAL:
...
static char *build_effective_name(const char *name, const int64_t identifier)
{
...

    {
        ...
        result = (char *)com_util_malloc(total);
        if (result == NULL)
        {
            return NULL;
        }
        if (com_util_snprintf(result, total, "%s_%" PRId64, base, identifier) != COM_UTIL_OK)
        {
            com_util_free(result);
            return NULL;
        }
        return result;
    }
...
static int resolve_file_name(const com_util_tracer *handle, char *file_name_out, const size_t file_name_size)
{
    ...
    {
        ret = com_util_snprintf(name_buf, sizeof(name_buf), "%s", handle->file_name);
    }
    else
    {
        char path_buf[256];

        ret = com_util_snprintf(name_buf, sizeof(name_buf), "%s", get_process_basename(path_buf, sizeof(path_buf)));
#if defined(PLATFORM_WINDOWS)
        if (ret == COM_UTIL_OK)
        {
...
static int build_default_file_path(const com_util_tracer *handle, char *path_out, const size_t path_size)
{
    ...
    if (com_util_snprintf(log_file_name, sizeof(log_file_name), "%s.log", name_buf) != COM_UTIL_OK)
    {
        return -1;
    }
...
static void trace_handle_release_normal(com_util_tracer *handle)
{
    ...
    com_util_free(handle->file_path);
    handle->file_path = NULL;
    com_util_free(handle->file_name);
...
com_util_tracer *com_util_tracer_create(const com_util_tracer_concurrency_mode concurrency_mode)
{
...
#if defined(PLATFORM_LINUX)
    {
        ...
        handle = (com_util_tracer *)com_util_malloc(sizeof(com_util_tracer));
        if (handle == NULL)
        {
        ...
        handle->effective_name = com_util_strdup(effective_name);
...
#elif defined(PLATFORM_WINDOWS)
    {
        ...
        svc = com_util_strdup(effective_name);
        if (svc == NULL)
        {
            return NULL;
        }
        eventlog_name = com_util_strdup(effective_name);
...
com_util_tracer_state com_util_tracer_get_state(com_util_tracer *handle)
{
    com_util_tracer_state state;
...
static int should_output(const com_util_trace_level msg_level, const com_util_trace_level threshold);
 ...
 */
static int write_os_backends(com_util_tracer *handle, const com_util_trace_level level,
 ...
 *  @param[in]      msg    書き込むメッセージ文字列。
 */
static void write_stderr_entry(const com_util_trace_level level, const char *timestamp_text, const char *msg)
{
 ...
 */
static int write_dual(com_util_tracer *handle, const com_util_trace_level level, const com_util_timespec *timestamp,
                      const char *msg)
{
    int os_result = 0;
    int file_result = 0;
...
int com_util_tracer_write_at(com_util_tracer *handle, const com_util_trace_level level,
...
int com_util_tracer_vwritef_at(com_util_tracer *handle, const com_util_trace_level level,
...
int com_util_tracer_writef_at(com_util_tracer *handle, const com_util_trace_level level,
                            const com_util_timespec *timestamp, const char *format, ...)
{
    ...
    ret = com_util_tracer_vwritef_at(handle, level, timestamp, format, args);
    va_end(args);
...
const char *com_util_tracer_hex_sep(const char *message)
{
...
const char *com_util_tracer_hex_msg(const char *message)
{
    if (message != NULL)
    {
        return message;
...
int com_util_tracer_write_with_source(com_util_tracer *handle, const com_util_trace_level level,
                                      const com_util_timespec *timestamp, const char *file, const int line,
                                      const char *message)
{
    if (message != NULL)
    {
        return com_util_tracer_writef_at(handle, level, timestamp, "[%s:%d] %s", file, line, message);
    }

    return com_util_tracer_writef_at(handle, level, timestamp, "[%s:%d]", file, line);
}
...
int com_util_tracer_write_hex_at(com_util_tracer *handle, const com_util_trace_level level,
...
int com_util_tracer_vwrite_hexf_at(com_util_tracer *handle, const com_util_trace_level level,
                                 const com_util_timespec *timestamp, const void *data, const size_t size,
...
int com_util_tracer_write_hexf_at(com_util_tracer *handle, const com_util_trace_level level,
                                const com_util_timespec *timestamp, const void *data, const size_t size,
                                const char *format, ...)
{
    va_list args;
    ...
    va_start(args, format);
    ret = com_util_tracer_vwrite_hexf_at(handle, level, timestamp, data, size, format, args);
    va_end(args);
...
int com_util_tracer_set_name(com_util_tracer *handle, const char *name, const int64_t identifier)
{
...
#if defined(PLATFORM_LINUX)
    {
        ...
        ret = com_util_syslog_sink_rename(handle->syslog_handle, effective);
        if (ret != 0)
        {
            com_util_free(effective);
            config_unlock_exclusive(handle);
            return COM_UTIL_ERR_UNKNOWN;
        }
        com_util_free(handle->effective_name);
...
int com_util_tracer_set_file_name(com_util_tracer *handle, const char *name, const int64_t identifier)
{
    ...
    if (name != NULL)
    {
        name_copy = com_util_strdup(name);
        if (name_copy == NULL)
        {
            return COM_UTIL_ERR_OUT_OF_MEMORY;
        }
...
com_util_trace_level com_util_tracer_get_os_level(com_util_tracer *handle)
{
    com_util_trace_level lv;
...
int com_util_tracer_set_os_level(com_util_tracer *handle, const com_util_trace_level level)
{
    if (tracer_enter_exclusive(handle) != 0)
    {
        return COM_UTIL_ERR_UNKNOWN;
...
com_util_trace_level com_util_tracer_get_etw_level(com_util_tracer *handle)
{
#if defined(PLATFORM_WINDOWS)
    com_util_trace_level lv;
...
int com_util_tracer_set_etw_level(com_util_tracer *handle, const com_util_trace_level level)
{
#if defined(PLATFORM_WINDOWS)
    if (tracer_enter_exclusive(handle) != 0)
    {
...
com_util_trace_level com_util_tracer_get_file_level(com_util_tracer *handle)
{
    com_util_trace_level lv;
...
int com_util_tracer_set_file_level(com_util_tracer *handle, const char *path, const com_util_trace_level level,
                                   const size_t max_bytes, const int generations, const int flags)
{
    char *path_copy = NULL;
    ...
    if (path != NULL)
    {
        path_copy = com_util_strdup(path);
        if (path_copy == NULL)
        {
            return COM_UTIL_ERR_OUT_OF_MEMORY;
        }
...
com_util_trace_level com_util_tracer_get_stderr_level(com_util_tracer *handle)
{
    com_util_trace_level lv;
...
int com_util_tracer_set_stderr_level(com_util_tracer *handle, const com_util_trace_level level)
{
    if (tracer_enter_exclusive(handle) != 0)
    {
        return COM_UTIL_ERR_UNKNOWN;
...
com_util_tracer_hook_entry *com_util_tracer_set_hook(com_util_tracer *handle, com_util_tracer_hook_fn fn, void *context)
{
    ...
    entry = (com_util_tracer_hook_entry *)com_util_malloc(sizeof(com_util_tracer_hook_entry));
    if (entry == NULL)
    {
        return NULL;
    }
...
void com_util_tracer_call_next_hook(com_util_tracer_hook_entry *prev, com_util_tracer *handle,
                                    const com_util_trace_level level, const com_util_timespec *timestamp,
                                    const char *message)
{
