...
TEST_F(argparserTest, create_and_dispose)
{
    ...
    com_util_argparser *parser =
        com_util_argparser_create(NULL); // [手順] - オプション NULL で com_util_argparser_create を呼び出す。
    bool handle_created = parser != NULL;
    com_util_argparser_dispose(parser); // [手順] - 生成したハンドルを dispose する。
    com_util_argparser_dispose(NULL);   // [手順] - NULL ハンドルで dispose を呼び出す。

    // Assert
    EXPECT_TRUE(handle_created); // [確認_正常系] - com_util_argparser_create の戻り値が NULL でないこと。
...
TEST_F(argparserTest, create_returns_null_on_alloc_failure)
{
    ...
    // Act
    com_util_argparser *parser = com_util_argparser_create(NULL); // [手順] - com_util_argparser_create を呼び出す。

    // Assert
    EXPECT_EQ(nullptr, parser); // [確認_異常系] - com_util_argparser_create の戻り値が NULL であること。
}
...
TEST_F(argparserTest, default_shutdown_callback_keeps_process_lifetime_lock)
{
    ...
    com_util_argparser *parser =
        com_util_argparser_default(NULL); // [状態] - 終了コールバックを登録した default ハンドルを用意する。
...
TEST_F(argparserTest, default_returns_same_handle_across_calls)
{
    ...
    com_util_argparser *first =
        com_util_argparser_default(NULL); // [手順] - 1 回目の com_util_argparser_default を呼び出す。
    com_util_argparser *second =
        com_util_argparser_default(NULL); // [手順] - 続けて 2 回目の com_util_argparser_default を呼び出す。

    // Assert
    ASSERT_NE(nullptr, first); // [確認_正常系] - ハンドルが NULL でないこと。
    EXPECT_EQ(
        first,
        second); // [確認_正常系] - com_util_argparser_default の戻り値として、2 回の呼び出しで同一ハンドルが返ること。
}
...
TEST_F(argparserTest, default_options_applied_only_on_first_call)
{
    // Arrange
    com_util_argparser *first =
        com_util_argparser_default(NULL); // [状態] - 初回呼び出し済みの default ハンドルを用意する。
    ASSERT_NE(nullptr, first); // [状態確認] - ハンドルが非 NULL であること。

    char before[256];
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_get_usage(first, before, sizeof(before),
                                                         NULL)); // [状態] - 初回時点の usage を記録する。
                                                                 // [状態確認] - com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
    ...
    // Act
    com_util_argparser *second =
        com_util_argparser_default(&options); // [手順] - 生成オプション付きで 2 回目の default を呼び出す。

    char after[256];
    int actual_ret_argparser_get_usage =
        com_util_argparser_get_usage(second, after, sizeof(after), NULL); // [手順] - 2 回目時点の usage を取得する。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - 2 回目時点の usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。

    // Assert
    EXPECT_EQ(first, second);    // [確認_正常系] - com_util_argparser_default の戻り値として、同一ハンドルが返ること。
...
{
    // Arrange
    com_util_argparser *first = com_util_argparser_default(NULL); // [状態] - default ハンドルを取得しておく。
    ASSERT_NE(nullptr, first); // [状態確認] - ハンドルが非 NULL であること。

    // Pre-Assert

    // Act
    com_util_argparser_dispose(first);                             // [手順] - default ハンドルを dispose に渡す。
    com_util_argparser *second = com_util_argparser_default(NULL); // [手順] - 再度 default を呼び出す。
...
TEST_F(argparserTest, default_returns_same_handle_to_concurrent_callers)
{
    ...
    // Act
    // [手順] - 16 スレッドから並行に com_util_argparser_default を呼び出す。
    for (size_t i = 0; i < kThreadCount; i++)
    {
...
{
    // Arrange
    com_util_argparser *parser = com_util_argparser_create(NULL); // [状態] - 生成済みの parser を用意する。
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int storage = 0;
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_register_flag(parser, "-v", "--verbose", NULL,
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_register_flag(parser, "v", NULL, NULL,
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_register_flag(parser, NULL, "--a=b", NULL,
                                          &storage)); // [確認_異常系] - 長い名前 "--a=b" の形式不正が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_register_option_int(
                  parser, "-c", NULL, NULL, NULL, 0x100u,
                  &storage)); // [確認_異常系] - 未定義の登録フラグ 0x100 が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_register_option_string_array(
                  parser, "-i", NULL, NULL, NULL, 0, &string_storage, 0,
                  NULL)); // [確認_異常系] - 配列オプションの capacity 0 / count NULL が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, com_util_argparser_register_positional_int(
                                                 parser, NULL, NULL, 0,
                                                 &storage)); // [確認_異常系] - 位置引数の名前 NULL が検出されること。
    size_t positional_count = 0;
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_register_positional_int_array(
...
TEST_F(argparserTest, register_rejects_duplicate_definition)
{
    ...
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_flag(parser, "-v", "--verbose", NULL,
                                                &storage)); // [状態] - "-v" / "--verbose" を登録済みとする。
                                                            // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_register_option_int(parser, "-v", NULL, NULL, NULL, 0,
                                                &storage)); // [確認_異常系] - 短い名前 "-v" の重複が検出されること。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        com_util_argparser_register_flag(parser, NULL, "--verbose", NULL,
...
TEST_F(argparserTest, register_rejects_required_positional_after_optional)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(
                               parser, "first", NULL, 0,
                               &first)); // [状態] - 任意の位置引数 "first" を登録済みとする。
                                         // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, register_accepts_required_positionals_in_sequence)
{
    ...
    // Act
    int first_result =
        com_util_argparser_register_positional_string(parser, "first", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                       &first); // [手順] - 必須位置引数 "first" を登録する。
    int second_result =
        com_util_argparser_register_positional_string(parser, "second", NULL, COM_UTIL_ARGPARSER_REQUIRED,
...
TEST_F(argparserTest, register_requires_variadic_positional_to_be_last)
{
    ...
    int verbose = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string_array(
                               parser, "values", NULL, 0, values, 2,
                               &value_count)); // [状態] - 可変長位置引数 "values" を登録済みとする。
                                               // [状態確認] - com_util_argparser_register_positional_string_array の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_register_positional_string_array(
                  parser, "more", NULL, 0, values, 2,
                  &value_count)); // [確認_異常系] - 可変長位置引数を複数登録できないこと。
    EXPECT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(
...
TEST_F(argparserTest, register_error_collection_accumulates_all_failures)
{
    ...
    // Act
    int actual_ret_argparser_register_flag = com_util_argparser_register_flag(
        parser, "-a", "--aa", NULL, &a); // [手順] - 成功する登録を行う (コレクションに積まれない)。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_register_flag); // [確認_正常系] - 成功する登録に対する com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    int actual_ret_argparser_register_flag_2 = com_util_argparser_register_flag(
        parser, "-a", "--bb", NULL, &b); // [手順] - 短い名前の重複 (1 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        actual_ret_argparser_register_flag_2); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、短い名前の重複 (1 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    int actual_ret_argparser_register_flag_3 = com_util_argparser_register_flag(
        parser, "-c", "--cc", NULL, NULL); // [手順] - 格納先 NULL (2 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_argparser_register_flag_3); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、格納先 NULL (2 件目のエラー) を発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    int actual_ret_argparser_register_flag_4 = com_util_argparser_register_flag(
        parser, "-d", "--aa", NULL, &b); // [手順] - 別の重複 (3 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        actual_ret_argparser_register_flag_4); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、別の重複 (3 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。

    // Assert
    ASSERT_EQ((size_t)3,
              com_util_argparser_get_register_error_count(parser)); // [確認_正常系] - エラー件数が 3 であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_get_register_error(parser,
                                                     0)); // [確認_正常系] - 1 件目が DUPLICATE_DEFINITION であること。
    EXPECT_STREQ("--bb", com_util_argparser_get_register_error_target(
                             parser, 0)); // [確認_正常系] - 1 件目の対象が "--bb" であること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, com_util_argparser_get_register_error(
                                                 parser, 1)); // [確認_正常系] - 2 件目が INVALID_ARGUMENT であること。
    EXPECT_STREQ("--cc", com_util_argparser_get_register_error_target(
                             parser, 1)); // [確認_正常系] - 2 件目の対象が "--cc" であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_get_register_error(parser,
                                                     2)); // [確認_正常系] - 3 件目が DUPLICATE_DEFINITION であること。
    EXPECT_STREQ("--aa", com_util_argparser_get_register_error_target(
...
TEST_F(argparserTest, register_error_getters_default_when_absent_or_out_of_range)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int a = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", "--aa", NULL,
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_get_register_error(
            parser,
            0)); // [確認_正常系] - com_util_argparser_get_register_error の戻り値として、エラーなしのとき OK が返ること。
    EXPECT_EQ(nullptr, com_util_argparser_get_register_error_target(
                           parser, 0)); // [確認_正常系] - エラーなしのとき対象が NULL であること。

    EXPECT_EQ(
        (size_t)0,
        com_util_argparser_get_register_error_count(
            NULL)); // [確認_異常系] - com_util_argparser_get_register_error_count の戻り値として、NULL ハンドルで件数 0 が返ること。
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_get_register_error(
            NULL,
            0)); // [確認_異常系] - com_util_argparser_get_register_error の戻り値として、NULL ハンドルで OK が返ること。
    EXPECT_EQ(
        nullptr,
        com_util_argparser_get_register_error_target(
            NULL,
            0)); // [確認_異常系] - com_util_argparser_get_register_error_target の戻り値として、NULL ハンドルで NULL が返ること。

    int actual_ret_argparser_register_flag =
        com_util_argparser_register_flag(parser, "-a", NULL, NULL, NULL); // [手順] - エラーを 1 件発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_argparser_register_flag); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、エラーを 1 件発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    ASSERT_EQ((size_t)1, com_util_argparser_get_register_error_count(parser));
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_get_register_error(
            parser,
            1)); // [確認_異常系] - com_util_argparser_get_register_error の戻り値として、範囲外インデックスで OK が返ること。
    EXPECT_EQ(
        nullptr,
        com_util_argparser_get_register_error_target(
            parser,
            1)); // [確認_異常系] - com_util_argparser_get_register_error_target の戻り値として、範囲外インデックスで NULL が返ること。
...
TEST_F(argparserTest, register_error_message_formatting)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int a = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", "--aa", NULL, &a)); // [状態] - フラグ "-a" / "--aa" を登録する。
                                                                                               // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_register_flag(parser, "-a", "--bb", NULL,
                                                &a)); // [状態] - 重複登録エラーを 1 件積んだ parser とする。
                                                      // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    ASSERT_EQ((size_t)1, com_util_argparser_get_register_error_count(parser)); // [状態確認] - com_util_argparser_get_register_error_count の戻り値が 1 であること。
    char message[128];

    // Pre-Assert

    // Act
    int actual_ret_argparser_get_register_error_message = com_util_argparser_get_register_error_message(
        parser, 0, message, sizeof(message)); // [手順] - 1 件目のエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_register_error_message); // [確認_正常系] - 1 件目のエラー メッセージを取得した com_util_argparser_get_register_error_message の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_get_register_error_message(
            ...
            sizeof(
                small_buffer))); // [確認_異常系] - com_util_argparser_get_register_error_message の戻り値として、バッファー不足時は切り詰めて BUFFER_TOO_SMALL が返ること。
...
TEST_F(argparserTest, print_register_error_messages_rejects_invalid_arguments)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_print_register_error_messages(
...
TEST_F(argparserTest, print_register_error_messages_is_noop_without_error)
{
    ...
    com_util_argparser *parser = com_util_argparser_create(NULL); // [状態] - register エラーのない parser を用意する。
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    ...
    // Act
    int result = com_util_argparser_print_register_error_messages(
        parser, stderr); // [手順] - print_register_error_messages を呼び出す。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        result); // [確認_正常系] - com_util_argparser_print_register_error_messages の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, print_register_error_messages_writes_all_to_stream)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        com_util_argparser_register_flag(parser, "-a", "--bb", NULL, &a)); // [状態] - 重複登録エラーを積んでおく。
                                                                            // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_register_flag(parser, "-c", "--cc", NULL, NULL)); // [状態] - 不正引数エラーを積んでおく。
                                                                              // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
...
TEST_F(argparserTest, register_grows_beyond_initial_capacity)
{
    ...
    for (int i = 0; i < spec_count; i++)
    {
        ...
        ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, NULL, long_name, NULL, &storages[i]));
    }
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_parse(
            parser, argc,
            argv)); // [確認_正常系] - com_util_argparser_parse の戻り値から、拡張後も解析が成功したと判断できること。
...
TEST_F(argparserTest, flag_counts_occurrences_and_resets_on_reparse)
{
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - フラグが 3 回出現する引数を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - フラグが 3 回出現する引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(3, verbose); // [確認_正常系] - 出現回数 3 が格納されること。
    }
    {
        ARGV(cstr("prog"));
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - フラグなしの引数で再解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - フラグなしの引数で再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, flag_with_value_is_unexpected_value)
{
    ...
    int result = com_util_argparser_parse(parser, argc, argv); // [手順] - "--verbose=1" を解析する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_UNEXPECTED_VALUE,
        result); // [確認_異常系] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNEXPECTED_VALUE であること。
    EXPECT_EQ(COM_UTIL_ERR_UNEXPECTED_VALUE,
              com_util_argparser_get_error(parser)); // [確認_異常系] - エラー種別が UNEXPECTED_VALUE であること。
    EXPECT_STREQ("--verbose",
                 com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "--verbose" であること。
    EXPECT_EQ(1, com_util_argparser_get_error_index(parser));  // [確認_異常系] - エラー位置が argv[1] であること。
...
TEST_F(argparserTest, flag_with_short_value_is_unexpected_value)
{
    ...
    int result = com_util_argparser_parse(parser, argc, argv); // [手順] - "-v=1" を解析する。
    ...
    EXPECT_EQ(COM_UTIL_ERR_UNEXPECTED_VALUE,
              com_util_argparser_get_error(parser)); // [確認_異常系] - エラー種別が UNEXPECTED_VALUE であること。
    EXPECT_STREQ("--verbose", com_util_argparser_get_error_target(
...
TEST_F(argparserTest, option_int_accepts_all_syntaxes)
{
    ...
    int count = -100;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_option_int(parser, "-c", "--count", "N", NULL, 0,
                                                      &count)); // [状態] - int オプション "-c" / "--count" を登録する。
                                                                // [状態確認] - com_util_argparser_register_option_int の戻り値が COM_UTIL_OK であること。
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c 5" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - "-c 5" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(5, count); // [確認_正常系] - "-c 5" で 5 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("--count"), cstr("6"));
        int actual_ret_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--count 6" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - "--count 6" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(6, count); // [確認_正常系] - "--count 6" で 6 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("--count=7"));
        int actual_ret_argparser_parse_3 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--count=7" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_3); // [確認_正常系] - "--count=7" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(7, count); // [確認_正常系] - "--count=7" で 7 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("-c=8"));
        int actual_ret_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c=8" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_4); // [確認_正常系] - "-c=8" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(8, count); // [確認_正常系] - "-c=8" で 8 が格納されること。
    }
    {
        /* 値位置のトークンは照合しないため負数を渡せる */
        ARGV(cstr("prog"), cstr("-c"), cstr("-5"));
        int actual_ret_argparser_parse_5 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c -5" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_5); // [確認_正常系] - "-c -5" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_6 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - オプション非出現の引数を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_6); // [確認_正常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_int_boundary_and_conversion_errors)
{
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MAX を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - INT_MAX を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MIN を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - INT_MIN を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(INT_MIN, count); // [確認_正常系] - INT_MIN が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("--count=2147483648"));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MAX + 1 の "2147483648" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_OUT_OF_RANGE,
            actual_ret_argparser_parse_3); // [確認_異常系] - INT_MAX + 1 の "2147483648" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 範囲外が OUT_OF_RANGE になること。
    }
    {
        ARGV(cstr("prog"), cstr("--count=12a"));
        int actual_ret_argparser_parse_4 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 数値でない "12a" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            actual_ret_argparser_parse_4); // [確認_異常系] - 数値でない "12a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("--count", com_util_argparser_get_error_target(
                                    parser)); // [確認_異常系] - エラー対象が "--count" であること。
    }
    {
        ARGV(cstr("prog"), cstr("--count="));
        int actual_ret_argparser_parse_5 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 空値の "--count=" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            actual_ret_argparser_parse_5); // [確認_異常系] - 空値の "--count=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - int の空値が INVALID_INTEGER になること。
    }
...
TEST_F(argparserTest, positional_int_accepts_negative_value_without_hiding_unknown_options)
{
    ...
    int value = 0;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_int(parser, "value", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                          &value)); // [状態] - 必須の int 位置引数 "value" を登録する。
                                                                    // [状態確認] - com_util_argparser_register_positional_int の戻り値が COM_UTIL_OK であること。
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - 負数 "-42" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - 負数 "-42" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(-42, value); // [確認_正常系] - 負数 -42 が位置引数として格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("-2147483649"));
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MIN - 1 の "-2147483649" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_OUT_OF_RANGE,
            actual_ret_argparser_parse_2); // [確認_異常系] - INT_MIN - 1 の "-2147483649" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 範囲外が OUT_OF_RANGE になること。
    }
    {
        ...
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 数値でない "-x" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse_3); // [確認_異常系] - 数値でない "-x" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            com_util_argparser_get_error(parser)); // [確認_異常系] - "-x" が UNKNOWN_OPTION として検出されること。
    }
...
TEST_F(argparserTest, option_string_points_into_argv)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_string(
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-n abc" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - "-n abc" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(argv[2], name);  // [確認_正常系] - 格納先が argv[2] そのものを指すこと。
        EXPECT_STREQ("abc", name); // [確認_正常系] - 値が "abc" であること。
    }
    {
        ARGV(cstr("prog"), cstr("--name=xyz"));
        int actual_ret_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--name=xyz" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - "--name=xyz" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_STREQ("xyz", name); // [確認_正常系] - argv トークン内の値部分 "xyz" を指すこと。
    }
    {
        ARGV(cstr("prog"), cstr("--name="));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 空値の "--name=" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_3); // [確認_正常系] - 空値の "--name=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_STREQ("", name); // [確認_正常系] - 文字列では空値 "" が受理されること。
    }
    {
        ARGV(cstr("prog"), cstr("-n=xyz"));
        int actual_ret_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-n=xyz" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_4); // [確認_正常系] - "-n=xyz" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_STREQ("xyz", name); // [確認_正常系] - 短いオプションの "=" 区切りが受理されること。
    }
    {
        ARGV(cstr("prog"), cstr("-n="));
        int actual_ret_argparser_parse_5 = com_util_argparser_parse(parser, argc, argv); // [手順] - 空値の "-n=" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_5); // [確認_正常系] - 空値の "-n=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_string_accepts_value_with_spaces)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_string(
                               ...
                               &param)); // [状態] - 文字列オプション "-p" / "--param" を登録する。
                                         // [状態確認] - com_util_argparser_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_string(parser, "input", NULL, 0,
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 空白を含む位置引数を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - 空白を含む位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - "--param=parameter string" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - "--param=parameter string" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - "-p=parameter string" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_3); // [確認_正常系] - "-p=parameter string" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_4 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - "--param" と後続トークンを解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_4); // [確認_正常系] - "--param" と後続トークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_5 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - "-p" と後続トークンを解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_5); // [確認_正常系] - "-p" と後続トークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_string_stores_argv_verbatim)
{
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 両端にクオートが残存した値を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - 両端にクオートが残存した値を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 値の途中にクオートを含むトークンを解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - 値の途中にクオートを含むトークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, positional_assignment_and_overflow)
{
    ...
    int level = -1;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_string(parser, "input", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                             &input)); // [状態] - 必須の位置引数 "input" を登録する。
                                                                       // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_int(parser, "level", NULL, 0,
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "in.txt 3" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - "in.txt 3" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - int 位置引数に "abc" を渡して解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            actual_ret_argparser_parse_2); // [確認_異常系] - int 位置引数に "abc" を渡して解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("level", com_util_argparser_get_error_target(
                                  parser)); // [確認_異常系] - エラー対象が位置引数名 "level" であること。
    }
    {
        ARGV(cstr("prog"), cstr("a"), cstr("1"), cstr("extra"));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 登録数を超える 3 つの位置引数を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
            actual_ret_argparser_parse_3); // [確認_異常系] - 登録数を超える 3 つの位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
        EXPECT_EQ(COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 超過が TOO_MANY_ARGUMENTS になること。
        EXPECT_STREQ("extra", com_util_argparser_get_error_target(
                                  parser)); // [確認_異常系] - エラー対象が超過トークン "extra" であること。
        EXPECT_EQ(3, com_util_argparser_get_error_index(parser)); // [確認_異常系] - エラー位置が argv[3] であること。
    }
    {
        ARGV(cstr("prog"));
        int actual_ret_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - 位置引数なしで解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            actual_ret_argparser_parse_4); // [確認_異常系] - 位置引数なしで解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 必須欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("input",
                     com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "input" であること。
        EXPECT_EQ(-1, com_util_argparser_get_error_index(
...
TEST_F(argparserTest, positional_string_array_assignment_and_reparse)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string_array(
                               ...
                               &file_count)); // [状態] - 任意の可変長位置引数 "files" を容量 2 で登録する。
                                              // [状態確認] - com_util_argparser_register_positional_string_array の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(
                               parser, "-v", "--verbose", NULL,
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(
            parser, argc, argv); // [手順] - 単数位置引数、可変長位置引数、フラグが混在する入力を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - 単数位置引数、可変長位置引数、フラグが混在する入力を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 容量 2 を超える 3 件の可変長位置引数を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
            actual_ret_argparser_parse_2); // [確認_異常系] - 容量 2 を超える 3 件の可変長位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
        EXPECT_EQ(COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 容量超過が TOO_MANY_ARGUMENTS になること。
        EXPECT_STREQ(
            "c.txt",
            com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が超過した "c.txt" であること。
        EXPECT_EQ(4,
                  com_util_argparser_get_error_index(
                      parser)); // [確認_異常系] - エラー位置が超過トークンの argv[4] であること。
    }
    {
        ARGV(cstr("prog"), cstr("in.txt"));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 可変長位置引数を省略して再解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_3); // [確認_正常系] - 可変長位置引数を省略して再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ((size_t)0,
...
TEST_F(argparserTest, positional_int_array_conversion_and_required)
{
    ...
    int values[3] = {};
    size_t value_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_int_array(
                               parser, "values", NULL, COM_UTIL_ARGPARSER_REQUIRED, values, 3,
                               &value_count)); // [状態] - 必須の可変長 int 位置引数 "values" を登録する。
                                               // [状態確認] - com_util_argparser_register_positional_int_array の戻り値が COM_UTIL_OK であること。
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 負数と正数を可変長 int 位置引数として解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - 負数と正数を可変長 int 位置引数として解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 整数へ変換できない "12a" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            actual_ret_argparser_parse_2); // [確認_異常系] - 整数へ変換できない "12a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("values",
    ...
    }
    {
        ARGV(cstr("prog"), cstr("-2147483649"));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - int の下限を下回る値を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_OUT_OF_RANGE,
            actual_ret_argparser_parse_3); // [確認_異常系] - int の下限を下回る値を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 範囲外エラーが OUT_OF_RANGE になること。
    }
    {
        ARGV(cstr("prog"));
        int actual_ret_argparser_parse_4 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 必須の可変長位置引数を省略して解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            actual_ret_argparser_parse_4); // [確認_異常系] - 必須の可変長位置引数を省略して解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 必須欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("values",
                     com_util_argparser_get_error_target(
    ...
    char usage[256];
    int actual_ret_argparser_get_usage = com_util_argparser_get_usage(
        parser, usage, sizeof(usage), NULL); // [手順] - 必須の可変長位置引数を含む usage を組み立てる。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、必須の可変長位置引数を含む usage を組み立てた結果が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
...
TEST_F(argparserTest, unknown_option_detection)
{
    ...
    {
        ARGV(cstr("prog"), cstr("-x"));
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - 未登録の "-x" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse); // [確認_異常系] - 未登録の "-x" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - UNKNOWN_OPTION になること。
        EXPECT_STREQ("-x",
                     com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "-x" であること。
    }
    {
        ARGV(cstr("prog"), cstr("-vv"));
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 短オプション連結の "-vv" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse_2); // [確認_異常系] - 短オプション連結の "-vv" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            com_util_argparser_get_error(parser)); // [確認_異常系] - 連結は未サポートで UNKNOWN_OPTION になること。
    }
    {
        ARGV(cstr("prog"), cstr("--bogus"));
        int actual_ret_argparser_parse_3 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 未登録の "--bogus" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse_3); // [確認_異常系] - 未登録の "--bogus" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_STREQ("--bogus", com_util_argparser_get_error_target(
...
TEST_F(argparserTest, missing_value_at_end)
{
    ...
    // Act
    ARGV(cstr("prog"), cstr("--count"));
    int result = com_util_argparser_parse(parser, argc, argv); // [手順] - 値なしの末尾 "--count" を解析する。

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_MISSING_VALUE,
              result); // [確認_異常系] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_VALUE であること。
    EXPECT_EQ(COM_UTIL_ERR_MISSING_VALUE,
              com_util_argparser_get_error(parser)); // [確認_異常系] - エラー種別が MISSING_VALUE であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, duplicate_option_occurrence)
{
    ...
    int result = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c 1" と "--count=2" を併記して解析する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_OPTION,
        result); // [確認_異常系] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_DUPLICATE_OPTION であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_OPTION,
              com_util_argparser_get_error(parser)); // [確認_異常系] - エラー種別が DUPLICATE_OPTION であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, array_option_multiple_occurrences)
{
    ...
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_option_string_array(
                  ...
                  &include_count)); // [状態] - 容量 2 の文字列配列オプション "-i" / "--include" を登録する。
                                    // [状態確認] - com_util_argparser_register_option_string_array の戻り値が COM_UTIL_OK であること。
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - "-i dir1 --include=dir2" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - "-i dir1 --include=dir2" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 容量 2 を超える 3 回の出現を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_OCCURRENCES,
            actual_ret_argparser_parse_2); // [確認_異常系] - 容量 2 を超える 3 回の出現を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_OCCURRENCES であること。
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_OCCURRENCES,
            com_util_argparser_get_error(parser)); // [確認_異常系] - 容量超過が TOO_MANY_OCCURRENCES になること。
        EXPECT_STREQ("--include", com_util_argparser_get_error_target(
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_3); // [確認_正常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, array_option_int_and_required)
{
    ...
    int ports[4] = {};
    size_t port_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int_array(
                               parser, "-p", "--port", "PORT", NULL, COM_UTIL_ARGPARSER_REQUIRED, ports, 4,
                               &port_count)); // [状態] - 必須の int 配列オプション "-p" / "--port" を登録する。
                                              // [状態確認] - com_util_argparser_register_option_int_array の戻り値が COM_UTIL_OK であること。
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-p 80 -p 443" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - "-p 80 -p 443" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            actual_ret_argparser_parse_2); // [確認_異常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_get_error(
                      parser)); // [確認_異常系] - REQUIRED の配列オプション欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("--port", com_util_argparser_get_error_target(
...
TEST_F(argparserTest, missing_required_option)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int(
    ...
    // Act
    ARGV(cstr("prog"));
    int result = com_util_argparser_parse(parser, argc, argv); // [手順] - オプション非出現の引数を解析する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_MISSING_REQUIRED,
        result); // [確認_異常系] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
    EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
              com_util_argparser_get_error(parser)); // [確認_異常系] - エラー種別が MISSING_REQUIRED であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, reparse_clears_error_state)
{
    ...
    // Assert
    {
        ARGV(cstr("prog"), cstr("-x"));
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 未登録の "-x" で解析エラーを発生させる。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse); // [確認_異常系] - com_util_argparser_parse の戻り値として、未登録の "-x" で解析エラーを発生させた結果が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - UNKNOWN_OPTION が記録されること。
    }
    {
        ARGV(cstr("prog"), cstr("-v"));
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 正しい引数 "-v" で再解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse_2); // [確認_正常系] - 正しい引数 "-v" で再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(COM_UTIL_OK,
                  com_util_argparser_get_error(parser)); // [確認_正常系] - エラー種別が NONE に戻ること。
        EXPECT_EQ(nullptr,
                  com_util_argparser_get_error_target(parser));    // [確認_正常系] - エラー対象が NULL に戻ること。
        EXPECT_EQ(-1, com_util_argparser_get_error_index(parser)); // [確認_正常系] - エラー位置が -1 に戻ること。
    }
...
{
    // Arrange
    com_util_argparser *parser1 = com_util_argparser_create(NULL); // [状態] - 生成済みの parser1 を用意する。
    com_util_argparser *parser2 = com_util_argparser_create(NULL); // [状態] - 生成済みの parser2 を用意する。
    ASSERT_NE(nullptr, parser1); // [状態確認] - ハンドルが非 NULL であること。
    ASSERT_NE(nullptr, parser2); // [状態確認] - ハンドルが非 NULL であること。
    int flag1 = 0;
    int flag2 = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser1, "-a", NULL, NULL,
                                                             &flag1)); // [状態] - parser1 に "-a" を登録する。
                                                                       // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser2, "-b", NULL, NULL,
    ...
    // Assert
    {
        ...
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser1, argc, argv); // [手順] - parser1 で "-a" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - parser1 で "-a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        int actual_ret_argparser_parse_2 =
            com_util_argparser_parse(parser2, argc, argv); // [手順] - "-a" 未登録の parser2 でも同じ引数を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse_2); // [確認_異常系] - "-a" 未登録の parser2 でも同じ引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    }
    EXPECT_EQ(1, flag1); // [確認_正常系] - parser1 側のフラグだけが 1 になること。
    EXPECT_EQ(0, flag2); // [確認_正常系] - parser2 側のフラグが 0 のままであること。
    EXPECT_EQ(COM_UTIL_OK,
              com_util_argparser_get_error(parser1)); // [確認_正常系] - parser1 のエラー状態が NONE であること。
    EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              com_util_argparser_get_error(
...
TEST_F(argparserTest, error_message_formatting)
{
    ...
    // Act
    int actual_ret_argparser_get_error_message = com_util_argparser_get_error_message(
        parser, message, sizeof(message)); // [手順] - 未解析の状態でエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_error_message); // [確認_正常系] - 未解析の状態でエラー メッセージを取得した com_util_argparser_get_error_message の戻り値が COM_UTIL_OK であること。
    EXPECT_STREQ("no error", message);    // [確認_正常系] - 未解析時は "no error" が返ること。

    {
        ARGV(cstr("prog"), cstr("--bogus"));
        int actual_ret_argparser_parse =
            com_util_argparser_parse(parser, argc, argv); // [手順] - 未登録の "--bogus" で解析エラーを発生させる。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            actual_ret_argparser_parse); // [確認_異常系] - com_util_argparser_parse の戻り値として、未登録の "--bogus" で解析エラーを発生させた結果が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    }
    EXPECT_EQ(COM_UTIL_OK, com_util_argparser_get_error_message(parser, message, sizeof(message)));
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_BUFFER_TOO_SMALL,
        com_util_argparser_get_error_message(
            parser, small_buffer,
            sizeof(
                small_buffer))); // [確認_異常系] - com_util_argparser_get_error_message の戻り値として、バッファー不足時に BUFFER_TOO_SMALL が返ること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_get_error_message(parser, NULL,
                                                    1)); // [確認_異常系] - buffer NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_get_error_message(parser, message,
...
TEST_F(argparserTest, usage_formatting)
{
    ...
    const char *files[2] = {};
    size_t file_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-v", "--verbose", "verbose output", &verbose)); // [状態] - フラグ "-v" / "--verbose" を登録する。
                                                                                                                      // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int(parser, "-c", "--count", "N", "count value",
                                                                   COM_UTIL_ARGPARSER_REQUIRED, &count)); // [状態] - 必須の int オプション "-c" / "--count" を登録する。
                                                                                                          // [状態確認] - com_util_argparser_register_option_int の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_option_string(parser, NULL, "--name", "NAME", "display name", 0, &name)); // [状態] - 文字列オプション "--name" を登録する。
                                                                                                                     // [状態確認] - com_util_argparser_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(parser, "input", "input file",
                                                                          COM_UTIL_ARGPARSER_REQUIRED, &input)); // [状態] - 必須の位置引数 "input" を登録する。
                                                                                                                 // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(parser, "output", "output file", 0, &output)); // [状態] - 位置引数 "output" を登録する。
                                                                                                                         // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string_array(
    ...
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、get_usage で usage を組み立てた結果が COM_UTIL_OK であること。
    ...
    size_t query_size = 0;
    int actual_ret_argparser_get_usage_2 = com_util_argparser_get_usage(
        parser, NULL, 0, &query_size); // [手順] - buffer NULL でサイズ問い合わせのみ行う。
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage_2); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、buffer NULL でサイズ問い合わせのみ行った結果が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_BUFFER_TOO_SMALL,
        com_util_argparser_get_usage(
            parser, small_buffer, sizeof(small_buffer),
            &small_required)); // [確認_異常系] - com_util_argparser_get_usage の戻り値として、バッファー不足時に BUFFER_TOO_SMALL が返ること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_get_usage(NULL, usage, sizeof(usage),
...
TEST_F(argparserTest, print_usage_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_print_usage(NULL, stdout)); // [確認_異常系] - parser NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_print_usage(parser, NULL)); // [確認_異常系] - stream NULL が INVALID_ARGUMENT になること。
...
TEST_F(argparserTest, print_usage_writes_to_stream)
{
    ...
    // Act
    int result = com_util_argparser_print_usage(parser, stdout); // [手順] - print_usage を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK,
              result); // [確認_正常系] - com_util_argparser_print_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, print_error_messages_rejects_invalid_arguments)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_print_error_messages(
                  NULL, stderr)); // [確認_異常系] - parser NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_print_error_messages(
                  parser, NULL)); // [確認_異常系] - stream NULL が INVALID_ARGUMENT になること。
...
TEST_F(argparserTest, print_error_messages_is_noop_without_error)
{
    // Arrange
    NiceMock<Mock_stdio> mock_stdio;
    com_util_argparser *parser = com_util_argparser_create(NULL); // [状態] - 解析エラーのない parser を用意する。
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    ...
    // Act
    int result = com_util_argparser_print_error_messages(parser, stderr); // [手順] - print_error_messages を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK,
              result); // [確認_正常系] - com_util_argparser_print_error_messages の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, print_error_messages_writes_to_stream)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int verbose = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-v", "--verbose", NULL, &verbose)); // [状態] - フラグ "-v" / "--verbose" を登録する。
                                                                                                          // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。

    ARGV(cstr("prog"), cstr("--bogus"));
    ASSERT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              com_util_argparser_parse(parser, argc, argv)); // [状態] - "--bogus" の解析エラーを発生させた状態とする。
                                                              // [状態確認] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
{
    // Arrange
    com_util_argparser *parser = com_util_argparser_create(NULL); // [状態] - program_name 未指定で parser を生成する。
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int verbose = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-v", NULL, NULL, &verbose)); // [状態] - フラグ "-v" を登録する。
                                                                                                   // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    char usage[256];
    ...
    // Act
    int actual_ret_argparser_get_usage =
        com_util_argparser_get_usage(parser, usage, sizeof(usage), NULL); // [手順] - 解析前に usage を取得する。

    // Assert
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - 解析前に usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
...

    {
        ...
        ASSERT_EQ(
            COM_UTIL_OK,
            actual_ret_argparser_parse); // [確認_正常系] - argv[0] を "/usr/local/bin/mytool" として解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    }
    int actual_ret_argparser_get_usage_2 =
        com_util_argparser_get_usage(parser, usage, sizeof(usage), NULL); // [手順] - 解析後に usage を取得する。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage_2); // [確認_正常系] - 解析後に usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, parse_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_parse(NULL, argc, argv)); // [確認_異常系] - parser NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_parse(parser, 0, argv)); // [確認_異常系] - argc 0 が INVALID_ARGUMENT になること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_parse(parser, argc, NULL)); // [確認_異常系] - argv NULL が INVALID_ARGUMENT になること。

    EXPECT_EQ(COM_UTIL_OK,
              com_util_argparser_get_error(NULL)); // [確認_異常系] - get_error が NULL ハンドルで NONE を返すこと。
    EXPECT_EQ(nullptr, com_util_argparser_get_error_target(
...
TEST_F(argparserTest, positional_register_rejects_null_parser_and_invalid_flags)
{
    ...
    // Act
    int null_parser_result = com_util_argparser_register_positional_int(
        NULL, "value", NULL, 0u, &storage); // [手順] - parser NULL で位置引数を登録する。
    int invalid_flags_result = com_util_argparser_register_positional_int(
...
TEST_F(argparserTest, default_returns_null_when_lock_creation_fails)
{
    ...
    // Act
    com_util_argparser *parser =
        com_util_argparser_default(NULL);                  // [手順] - ロック生成失敗状態で default parser を取得する。
...
TEST_F(argparserTest, default_returns_null_when_parser_allocation_fails)
{
    ...
    // Act
    com_util_argparser *parser =
        com_util_argparser_default(NULL); // [手順] - parser 本体の確保が失敗する状態で default parser を取得する。

    // Assert
    EXPECT_EQ(
        nullptr,
        parser); // [確認_異常系] - parser 本体の確保失敗時に com_util_argparser_default の戻り値が NULL であること。
}
...
TEST_F(argparserTest, default_returns_null_when_shutdown_registration_fails)
{
    ...
    // Act
    com_util_argparser *parser =
        com_util_argparser_default(NULL); // [手順] - shutdown 登録失敗状態で default parser を取得する。
...
{
    // Arrange
    com_util_argparser *parser = com_util_argparser_default(NULL); // [状態] - default ハンドルを用意する。
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    com_util_shutdown_event event = {};
    event.reason = COM_UTIL_SHUTDOWN_REASON_NORMAL_EXIT;
...
TEST_F(argparserTest, default_returns_null_when_locking_fails)
{
    ...
    // Act
    com_util_argparser *parser =
        com_util_argparser_default(NULL); // [手順] - ロック取得失敗状態で default parser を取得する。
...
TEST_F(argparserTest, register_wrappers_reject_invalid_storage_and_registration)
{
    ...
    // Act
    int option_int_null = com_util_argparser_register_option_int(
        parser, "-i", NULL, NULL, NULL, 0u, NULL); // [手順] - int オプションの storage NULL を検証する。
    int option_string_null = com_util_argparser_register_option_string(
        parser, "-s", NULL, NULL, NULL, 0u, NULL); // [手順] - string オプションの storage NULL を検証する。
    int option_int_array_storage_null = com_util_argparser_register_option_int_array(
        parser, "-a", NULL, NULL, NULL, 0u, NULL, 1u, &int_count); // [手順] - int 配列の storage NULL を検証する。
    int option_int_array_capacity_zero = com_util_argparser_register_option_int_array(
        parser, "-b", NULL, NULL, NULL, 0u, int_array, 0u, &int_count); // [手順] - int 配列の capacity 0 を検証する。
    int option_int_array_count_null = com_util_argparser_register_option_int_array(
        parser, "-c", NULL, NULL, NULL, 0u, int_array, 1u, NULL); // [手順] - int 配列の count NULL を検証する。
    int option_string_array_storage_null = com_util_argparser_register_option_string_array(
        parser, "-d", NULL, NULL, NULL, 0u, NULL, 1u,
        &string_count); // [手順] - string 配列の storage NULL を検証する。
    int option_string_array_capacity_zero = com_util_argparser_register_option_string_array(
        parser, "-e", NULL, NULL, NULL, 0u, string_array, 0u,
        &string_count); // [手順] - string 配列の capacity 0 を検証する。
    int option_string_array_count_null = com_util_argparser_register_option_string_array(
        parser, "-f", NULL, NULL, NULL, 0u, string_array, 1u, NULL); // [手順] - string 配列の count NULL を検証する。
    int positional_int_null = com_util_argparser_register_positional_int(
        parser, "value", NULL, 0u, NULL); // [手順] - int 位置引数の storage NULL を検証する。
    int positional_string_null = com_util_argparser_register_positional_string(
        parser, "text", NULL, 0u, NULL); // [手順] - string 位置引数の storage NULL を検証する。
    int positional_int_array_storage_null = com_util_argparser_register_positional_int_array(
        parser, "values", NULL, 0u, NULL, 1u, &int_count); // [手順] - int 配列位置引数の storage NULL を検証する。
    int positional_int_array_capacity_zero = com_util_argparser_register_positional_int_array(
        parser, "values2", NULL, 0u, int_array, 0u, &int_count); // [手順] - int 配列位置引数の capacity 0 を検証する。
    int positional_int_array_count_null = com_util_argparser_register_positional_int_array(
        parser, "values3", NULL, 0u, int_array, 1u, NULL); // [手順] - int 配列位置引数の count NULL を検証する。
    int positional_string_array_storage_null = com_util_argparser_register_positional_string_array(
        parser, "texts", NULL, 0u, NULL, 1u, &string_count); // [手順] - string 配列位置引数の storage NULL を検証する。
    int positional_string_array_capacity_zero = com_util_argparser_register_positional_string_array(
        parser, "texts2", NULL, 0u, string_array, 0u,
        &string_count); // [手順] - string 配列位置引数の capacity 0 を検証する。
    int positional_string_array_count_null = com_util_argparser_register_positional_string_array(
        parser, "texts3", NULL, 0u, string_array, 1u, NULL); // [手順] - string 配列位置引数の count NULL を検証する。

    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int_array(parser, "-x", "--x", NULL, NULL, 0u, int_array,
                                                                         1u, &int_count));
    int option_int_array_duplicate = com_util_argparser_register_option_int_array(
        parser, "-x", "--x2", NULL, NULL, 0u, int_array, 1u, &int_count); // [手順] - int 配列の重複登録を検証する。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_string_array(parser, "-y", "--y", NULL, NULL, 0u,
                                                                            string_array, 1u, &string_count));
    int option_string_array_duplicate =
        com_util_argparser_register_option_string_array(parser, "-y", "--y2", NULL, NULL, 0u, string_array, 1u,
                                                         &string_count); // [手順] - string 配列の重複登録を検証する。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string_array(parser, "tail", NULL, 0u, string_array,
                                                                                1u, &string_count));
    int positional_int_array_duplicate = com_util_argparser_register_positional_int_array(
        parser, "tail2", NULL, 0u, int_array, 1u, &int_count); // [手順] - 可変長位置引数後の int 配列登録を検証する。
    int positional_string_array_duplicate = com_util_argparser_register_positional_string_array(
...
TEST_F(argparserTest, parse_rejects_null_tokens_and_handles_negative_positionals)
{
    ...
    int values[2] = {};
    size_t value_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int(parser, "-c", "--count", NULL, NULL, 0u, &count)); // [状態] - int オプション "-c" / "--count" を登録する。
                                                                                                                      // [状態確認] - com_util_argparser_register_option_int の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_int_array(parser, "values", NULL, 0u, values, 2u, &value_count)); // [状態] - 可変長 int 位置引数 "values" を登録する。
                                                                                                                        // [状態確認] - com_util_argparser_register_positional_int_array の戻り値が COM_UTIL_OK であること。
    ...
    int null_token_result =
        com_util_argparser_parse(parser, 2, null_token_argv); // [手順] - argv 中の NULL token を解析する。
    char *long_null_value_argv[] = {cstr("prog"), cstr("--count"), NULL};
    int long_null_value_result =
        com_util_argparser_parse(parser, 3, long_null_value_argv); // [手順] - 長形式オプションの NULL 値を解析する。
    char *short_null_value_argv[] = {cstr("prog"), cstr("-c"), NULL};
    int short_null_value_result =
        com_util_argparser_parse(parser, 3, short_null_value_argv); // [手順] - 短形式オプションの NULL 値を解析する。
    char *short_missing_value_argv[] = {cstr("prog"), cstr("-c")};
    int short_missing_value_result =
        com_util_argparser_parse(parser, 2, short_missing_value_argv); // [手順] - 末尾の短形式オプションを解析する。
    char *negative_array_argv[] = {cstr("prog"), cstr("-1")};
    int negative_array_result =
        com_util_argparser_parse(parser, 2, negative_array_argv); // [手順] - 負数を可変長 int 位置引数として解析する。
...
TEST_F(argparserTest, parse_classifies_bare_prefixes_and_unregistered_negative_value)
{
    ...
    int double_dash_result = com_util_argparser_parse(
        parser, 2, double_dash_argv); // [手順] - 長形式オプションの接頭辞だけである "--" を解析する。
    char *single_dash_argv[] = {cstr("prog"), cstr("-")};
    int single_dash_result = com_util_argparser_parse(
        parser, 2, single_dash_argv); // [手順] - 短形式オプションの接頭辞だけである "-" を解析する。
    char *negative_argv[] = {cstr("prog"), cstr("-1")};
    int negative_result = com_util_argparser_parse(
        parser, 2, negative_argv); // [手順] - 位置引数登録のない parser で負数形式の "-1" を解析する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        double_dash_result); // [確認_異常系] - "--" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
        single_dash_result); // [確認_異常系] - "-" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        negative_result); // [確認_異常系] - 登録のない負数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, parse_searches_options_with_only_one_name)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", NULL, NULL, &short_flag)); // [状態] - フラグ "-a" を登録する。
                                                                                                      // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(parser, "input", NULL, 0u, &positional)); // [状態] - 位置引数 "input" を登録する。
                                                                                                                    // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, NULL, "--beta", NULL, &long_flag)); // [状態] - フラグ "--beta" を登録する。
                                                                                                         // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ...
    int unknown_long_result = com_util_argparser_parse(
        parser, 2, unknown_long_argv); // [手順] - 同じ長さの未登録長形式オプション --zeta を解析する。
    char *unknown_short_argv[] = {cstr("prog"), cstr("-b")};
    int unknown_short_result = com_util_argparser_parse(
        parser, 2, unknown_short_argv); // [手順] - 同じ長さの未登録短形式オプション -b を解析する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        unknown_long_result); // [確認_異常系] - --zeta を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        unknown_short_result); // [確認_異常系] - -b を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, parse_handles_positional_search_and_array_conversion_failure)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(parser, "text", NULL, 0u, &text)); // [状態] - 位置引数 "text" を登録する。
                                                                                                             // [状態確認] - com_util_argparser_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_int_array(parser, "-n", "--number", NULL, NULL, 0u,
                                                                         numbers, 1u, &number_count)); // [状態] - int 配列オプション "-n" / "--number" を登録する。
                                                                                                       // [状態確認] - com_util_argparser_register_option_int_array の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_register_positional_int_array(parser, "values", NULL, 0u, values, 1u, &value_count)); // [状態] - 可変長 int 位置引数 "values" を登録する。
                                                                                                                        // [状態確認] - com_util_argparser_register_positional_int_array の戻り値が COM_UTIL_OK であること。
    ...
    int negative_string_result = com_util_argparser_parse(
    ...
    int invalid_array_result =
        com_util_argparser_parse(parser, 2, invalid_array_argv); // [手順] - int 配列へ変換できない値を解析する。
...
TEST_F(argparserTest, parse_accepts_null_program_name)
{
    ...
    // Act
    int result = com_util_argparser_parse(parser, 1, argv); // [手順] - argv[0] が NULL の argv を解析する。
...
TEST_F(argparserTest, usage_handles_short_name_default_value_and_long_label)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-v", NULL, NULL, &flag)); // [状態] - フラグ "-v" を登録する。
                                                                                                // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_option_string(parser, "-n", NULL, NULL, NULL, 0u, &value)); // [状態] - 文字列オプション "-n" を登録する。
                                                                                                                    // [状態確認] - com_util_argparser_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_positional_string(parser, "123456789012345678901234567890",
    ...
    int unexpected_result =
        com_util_argparser_parse(parser, 2, unexpected_argv); // [手順] - 短い名前のみのフラグへ値を指定する。
    int usage_result = com_util_argparser_get_usage(parser, usage, sizeof(usage),
...
TEST_F(argparserTest, error_message_formats_all_error_results)
{
    ...
    for (int result : results)
    {
        ...
        return_codes.push_back(com_util_argparser_get_error_message(
...
TEST_F(argparserTest, error_message_returns_invalid_argument_when_snprintf_fails)
{
    ...
    // Act
    int result = com_util_argparser_get_error_message(
...
TEST_F(argparserTest, register_error_message_formats_all_results)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int storage = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", "--alpha", NULL, &storage)); // [状態] - フラグ "-a" / "--alpha" を登録する。
                                                                                                        // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_register_flag(parser, "-a", "--beta", NULL, &storage)); // [状態] - 重複登録エラーを積んだ状態とする。
                                                                                          // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    ASSERT_EQ((size_t)1, com_util_argparser_get_register_error_count(parser)); // [状態確認] - com_util_argparser_get_register_error_count の戻り値が 1 であること。
    char message[128] = {};
    const int results[] = {
    ...
    for (int result : results)
    {
        ...
        return_codes.push_back(com_util_argparser_get_register_error_message(
            parser, 0u, message, sizeof(message))); // [手順] - 各登録結果のエラーメッセージを取得する。
    }
    test_argparser_clear_register_error_target(parser, 0u);
    int null_target_result = com_util_argparser_get_register_error_message(
...
TEST_F(argparserTest, register_error_message_returns_invalid_argument_when_snprintf_fails)
{
    ...
    // Act
    int result = com_util_argparser_get_register_error_message(
...
TEST_F(argparserTest, public_api_wrappers_are_callable)
{
    ...
    // Act
    com_util_argparser_default_init("public API"); // [手順] - 公開 default parser を初期化する。
    int flag_result =
        com_util_argparser_default_register_flag("-f", "--flag", NULL, &flag); // [手順] - 公開 flag 登録 API を呼び出す。
    int int_result = com_util_argparser_default_register_option_int(
        "-i", "--integer", NULL, NULL, 0u, &int_value); // [手順] - 公開 int オプション登録 API を呼び出す。
    int string_result = com_util_argparser_default_register_option_string(
        "-s", "--string", NULL, NULL, 0u, &string_value); // [手順] - 公開 string オプション登録 API を呼び出す。
    int int_array_result =
        com_util_argparser_default_register_option_int_array("-a", "--array-int", NULL, NULL, 0u, int_values, 2u,
                                                     &int_value_count); // [手順] - 公開 int 配列登録 API を呼び出す。
    int string_array_result = com_util_argparser_default_register_option_string_array(
        "-b", "--array-string", NULL, NULL, 0u, string_values, 2u,
        &string_value_count); // [手順] - 公開 string 配列登録 API を呼び出す。
    int positional_int_result = com_util_argparser_default_register_positional_int(
        "posint", NULL, 0u, &positional_int); // [手順] - 公開 int 位置引数登録 API を呼び出す。
    int positional_string_result = com_util_argparser_default_register_positional_string(
        "posstr", NULL, 0u, &positional_string); // [手順] - 公開 string 位置引数登録 API を呼び出す。
    int positional_string_array_result = com_util_argparser_default_register_positional_string_array(
        "posstrs", NULL, 0u, positional_string_values, 2u,
        &positional_string_value_count); // [手順] - 公開 string 配列位置引数登録 API を呼び出す。
    int invalid_register_result = com_util_argparser_default_register_flag(
        NULL, NULL, NULL, &flag); // [手順] - 公開登録 API で登録エラーを 1 件発生させる。
    int positional_int_array_result = com_util_argparser_default_register_positional_int_array(
        "posints", NULL, 0u, positional_int_values, 2u,
        &positional_int_value_count); // [手順] - 公開 int 配列位置引数登録 API を呼び出す。
    char *parse_argv[] = {cstr("prog")};
    int parse_result = com_util_argparser_default_parse(1, parse_argv); // [手順] - 公開 parse API を引数なしで呼び出す。
    char *error_argv[] = {cstr("prog"), cstr("--unknown")};
    int error_parse_result =
        com_util_argparser_default_parse(2, error_argv);       // [手順] - 公開 parse API で解析エラーを発生させる。
    int error_result = com_util_argparser_default_get_error(); // [手順] - 公開 get_error API を呼び出す。
    const char *error_target = com_util_argparser_default_get_error_target(); // [手順] - 公開 get_error_target API を呼び出す。
    int error_index = com_util_argparser_default_get_error_index();           // [手順] - 公開 get_error_index API を呼び出す。
    int error_message_result = com_util_argparser_default_get_error_message(
        error_message, sizeof(error_message)); // [手順] - 公開 get_error_message API を呼び出す。
    int usage_result =
        com_util_argparser_default_get_usage(usage, sizeof(usage), &required_size); // [手順] - 公開 get_usage API を呼び出す。
    int print_usage_result = com_util_argparser_default_print_usage(stdout); // [手順] - 公開 print_usage API を呼び出す。
    int print_error_result =
        com_util_argparser_default_print_error_messages(stderr); // [手順] - 公開 print_error_messages API を呼び出す。
    size_t register_error_count =
        com_util_argparser_default_get_register_error_count(); // [手順] - 公開登録エラー件数 API を呼び出す。
    int register_error_result =
        com_util_argparser_default_get_register_error(0u); // [手順] - 公開登録エラー取得 API を呼び出す。
    const char *register_error_target =
        com_util_argparser_default_get_register_error_target(0u); // [手順] - 公開登録エラー対象 API を呼び出す。
    int register_error_message_result = com_util_argparser_default_get_register_error_message(
        0u, register_error_message,
        sizeof(register_error_message)); // [手順] - 公開登録エラーメッセージ API を呼び出す。
    int print_register_error_result =
        com_util_argparser_default_print_register_error_messages(stderr); // [手順] - 公開登録エラー出力 API を呼び出す。
    ...
    EXPECT_CALL(mock_com_util_, com_util_malloc(_))
        .WillOnce(
            [&](size_t size)
            {
                ...
                return delegate_real_com_util_malloc(size);
            }); // [Pre-Assert確認_異常系] - usage バッファー確保前に説明文が変更されること。

    // Act
    int result = com_util_argparser_print_usage(
...
TEST_F(argparserTest, print_error_messages_writes_buffer_too_small_message)
{
    ...
    ASSERT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        com_util_argparser_parse(parser, 2, argv)); // [状態] - 600 文字の未知オプションで解析エラーを発生させる。
                                                     // [状態確認] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    ...
    // Act
    int result = com_util_argparser_print_error_messages(
...
TEST_F(argparserTest, print_error_messages_skips_message_when_snprintf_fails)
{
    ...
    ASSERT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              com_util_argparser_parse(parser, 2, argv)); // [状態] - 未知オプションで解析エラーを発生させる。
                                                           // [状態確認] - com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, print_register_error_messages_writes_buffer_too_small_message)
{
    ...
    int storage = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", long_name.c_str(), NULL, &storage)); // [状態] - 長い long_name のフラグ "-a" を登録する。
                                                                                                                // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_register_flag(parser, "-b", long_name.c_str(), NULL,
    ...
    // Act
    int result =
        com_util_argparser_print_register_error_messages(parser, stderr); // [手順] - 長い対象の登録エラーを出力する。
...
TEST_F(argparserTest, print_register_error_messages_skips_message_when_snprintf_fails)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", "--alpha", NULL, &storage)); // [状態] - フラグ "-a" / "--alpha" を登録する。
                                                                                                        // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_register_flag(parser, "-b", "--alpha", NULL,
...
TEST_F(argparserTest, parse_skips_short_name_when_length_differs)
{
    ...
    ASSERT_NE(nullptr, parser); // [状態確認] - ハンドルが非 NULL であること。
    int flag = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_register_flag(parser, "-a", NULL, NULL, &flag)); // [状態] - フラグ "-a" を登録する。
                                                                                                // [状態確認] - com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    ...
    int result = com_util_argparser_parse(parser, 2, argv); // [手順] - 長さ 2 の短いオプション -a を解析する。

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              result); // [確認_異常系] - 長さが一致しない短い名前を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
