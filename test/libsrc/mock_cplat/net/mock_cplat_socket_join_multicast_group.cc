...
MOCK_WEAK_IMPL(int, cplat_socket_join_multicast_group, cplat_socket sock, uint32_t group_address, uint32_t interface_address, cplat_error *detail_out)
{
    ...
    {
        mock_ret = _mock_cplat->cplat_socket_join_multicast_group(sock, group_address, interface_address, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_socket_join_multicast_group(sock, group_address, interface_address, detail_out);
    }
