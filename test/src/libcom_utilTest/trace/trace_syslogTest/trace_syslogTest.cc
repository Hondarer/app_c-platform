...
TEST_F(trace_syslogTest, test_write_all_levels)
{
    ...
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_CRIT, NULL, "critical"));   // [確認_正常系] - CRIT レベルで書き込めること。
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_ERR, NULL, "error"));       // [確認_正常系] - ERR レベルで書き込めること。
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_WARNING, NULL, "warning")); // [確認_正常系] - WARNING レベルで書き込めること。
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_INFO, NULL, "info"));       // [確認_正常系] - INFO レベルで書き込めること。
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_DEBUG, NULL, "debug"));     // [確認_正常系] - DEBUG レベルで書き込めること。
...
TEST_F(trace_syslogTest, test_write_to_test_fd_prefixes_timestamp)
{
    ...
    com_util_realtime_timestamp_t timestamp = {1412916640LL, 0, 0};
...
TEST_F(trace_syslogTest, test_null_arguments_are_safe)
{
    ...
    EXPECT_EQ(0, com_util_syslog_sink_write(NULL, LOG_INFO, NULL, "test message"));            // [確認_異常系] - NULL ハンドルが安全であること。
    EXPECT_EQ(0, com_util_syslog_sink_write(handle, LOG_INFO, NULL, NULL));                    // [確認_異常系] - NULL message が安全であること。
