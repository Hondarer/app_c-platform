...
MOCK_WEAK_IMPL(int, cplat_socket_set_broadcast, cplat_socket sock, int enable, cplat_error *detail_out)
{
    int mock_ret = CPLAT_ERR_UNKNOWN;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_socket_set_broadcast(sock, enable, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_socket_set_broadcast(sock, enable, detail_out);
    }
