...
TEST_F(hashtableGuardTest, add_rejects_null_arguments)
{
    ...
    int actual_ret_null_ht = cplat_hashtable_add(
        NULL, "a", value.data(), CPLAT_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - ht に NULL を渡す。
    int actual_ret_null_key = cplat_hashtable_add(
        ht, NULL, value.data(), CPLAT_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - key に NULL を渡す。
    int actual_ret_null_value = cplat_hashtable_add(
        ht, "a", NULL, CPLAT_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - value に NULL を渡す。
...
TEST_F(hashtableGuardTest, update_rejects_invalid_arguments_and_walks_chain)
{
    ...
    int actual_ret_too_long =
        cplat_hashtable_update(ht, too_long, value.data()); // [手順] - 長すぎるキーで更新する。
...
TEST_F(hashtableGuardTest, find_value_ref_rejects_invalid_arguments)
{
    ...
    int actual_ret_too_long =
        cplat_hashtable_find_value_ref(ht, too_long, &found); // [手順] - 長すぎるキーで検索する。
    cplat_hashtable_dispose(ht);
...
TEST_F(hashtableGuardTest, find_value_copy_rejects_invalid_arguments_and_succeeds)
{
    ...
    int actual_ret_not_found = cplat_hashtable_find_value_copy(
...
TEST_F(hashtableGuardTest, get_key_ref_rejects_invalid_arguments)
{
    ...
    int actual_ret_rec_hi =
        cplat_hashtable_get_key_ref(ht, 3, &key_out); // [手順] - capacity 超のレコード番号を渡す。
...
TEST_F(hashtableGuardTest, count_wrappers_reject_invalid_arguments)
{
    ...
    // Assert
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_count_null_ht);
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_count_null_out);
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_deleted_null_ht);
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_deleted_null_out);
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_empty_null_ht);
    EXPECT_EQ(CPLAT_ERR_INVALID_ARGUMENT, actual_ret_empty_null_out);
}
