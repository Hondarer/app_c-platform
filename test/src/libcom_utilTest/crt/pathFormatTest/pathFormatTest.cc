...
static int call_vformat_path(char *path, size_t path_size, int *error_out, const char *format, ...)
{
    ...
    int actual_ret = com_util_vformat_path(path, path_size, format, args, error_out);
    va_end(args);
...
TEST(pathFormatTest, rejects_null_path)
{
    ...
    // Assert
    EXPECT_EQ(-1, actual_ret);           // [確認_異常系] - com_util_vformat_path の戻り値が -1 であること。
...
TEST(pathFormatTest, formats_path_and_preserves_error_out)
{
    ...
    // Assert
    EXPECT_EQ(0, actual_ret);             // [確認_正常系] - com_util_vformat_path の戻り値が 0 であること。
