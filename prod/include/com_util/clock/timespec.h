 ...
 *******************************************************************************
 *  @file           timespec.h
 *  @brief          プラットフォーム互換の標準時刻型 com_util_timespec と時刻計算 API を提供します。
 *  @author         Tetsuo Honda
 *  @date           2026/07/10
 *  @version        1.0.0
 *
 *  Linux x86-64 の `struct timespec` とバイナリ レイアウト互換の時刻型 com_util_timespec と、\n
 *  正規化・加減算・比較・ミリ秒変換などの時刻計算 API を提供します。
 *
 *  @section        timespec_layout レイアウト互換の方針
 *
 *  com_util_timespec は、x86-64 上で Linux の `struct timespec`
 ...
 *  com_util_timespec_to_native() / com_util_timespec_from_native() で変換します。\n
 *  変換関数以外の箇所で `struct timespec` とのキャスト・混用を行ってはなりません。
 *
 *  @section        timespec_usage 使い分けの指針
 *
 *  - **時刻の保持・受け渡し** → com_util_timespec を使用する。\n
 *    絶対時刻 (com_util_get_realtime()) にも単調クロック値 (com_util_get_monotonic()) にも使用します。
 *  - **加減算・比較** → com_util_timespec_add() / com_util_timespec_sub() / com_util_timespec_cmp() を使用する。\n
 *    手書きの秒・ナノ秒演算は正規化漏れの温床となるため行いません。
 *  - **ミリ秒ベースの deadline / 経過時間** → com_util_timespec_add_ms() / com_util_timespec_diff_ms() を使用する。
 *
...
extern "C"
{
     ...
     *                  演算後の正規化には com_util_timespec_normalize() を使用してください。
     */
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_normalize(com_util_timespec *ts);
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_add(const com_util_timespec *a, const com_util_timespec *b,
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_sub(const com_util_timespec *a, const com_util_timespec *b,
                                                            com_util_timespec *result);
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_timespec_cmp(const com_util_timespec *a, const com_util_timespec *b);
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_add_ms(const com_util_timespec *ts, uint64_t timeout_ms,
                                                               com_util_timespec *result);
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int64_t COM_UTIL_API com_util_timespec_diff_ms(const com_util_timespec *end,
                                                                   const com_util_timespec *start);

    /**
     *  @brief          com_util_timespec をネイティブの `struct timespec` へ変換します。
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_to_native(const com_util_timespec *ts, struct timespec *native);

    /**
     *  @brief          ネイティブの `struct timespec` を com_util_timespec へ変換します。
     ...
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT void COM_UTIL_API com_util_timespec_from_native(const struct timespec *native,
