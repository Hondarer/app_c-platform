...
TEST_F(secureZeroTest, clears_whole_range)
{
    ...
    // Act
    com_util_secure_zero(buf, sizeof(buf)); // [手順] - バッファー全体を指定して com_util_secure_zero を呼び出す。
...
TEST_F(secureZeroTest, does_not_touch_outside_range)
{
    ...
    // Act
    com_util_secure_zero(buf, 8U); // [手順] - 先頭 8 byte だけを指定して com_util_secure_zero を呼び出す。
...
TEST_F(secureZeroTest, null_or_zero_size_is_noop)
{
    ...
    // Act
    com_util_secure_zero(NULL, 8U); // [手順] - バッファーに NULL を指定して呼び出す。
    com_util_secure_zero(buf, 0U);  // [手順] - サイズに 0 を指定して呼び出す。
