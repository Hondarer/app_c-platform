...
#ifndef COM_UTIL_NET_ENDPOINT_H
#define COM_UTIL_NET_ENDPOINT_H
...
#define COM_UTIL_IPV4_ADDR_STRLEN 16U

/** 任意のアドレスを表す IPv4 アドレス (0.0.0.0)。ネットワーク バイト オーダー。 */
#define COM_UTIL_IPV4_ADDR_ANY ((uint32_t)0U)

/** ループバック アドレス (127.0.0.1)。ネットワーク バイト オーダー。 */
#define COM_UTIL_IPV4_ADDR_LOOPBACK (com_util_hton32((uint32_t)0x7F000001U))
...
extern "C"
{
     ...
     *  ホスト バイト オーダーの値を設定する場合は、@ref com_util_hton32 と
     *  @ref com_util_hton16 を使用します。
     */
    typedef struct com_util_ipv4_endpoint
    {
        uint32_t address; /**< IPv4 アドレス (ネットワーク バイト オーダー)。 */
        uint16_t port;    /**< ポート番号 (ネットワーク バイト オーダー)。 */
        uint16_t pad;     /**< 明示的アラインメント */
    } com_util_ipv4_endpoint;
     ...
     *
     *  名前解決は行いません。ホスト名を解決する場合は @ref com_util_ipv4_resolve を使用します。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_ipv4_parse(const char *text, uint32_t *address_out);
     ...
     *  解決に失敗した場合、@p detail_out には @ref COM_UTIL_ERROR_DOMAIN_GAI ドメインの
     *  エラー コードを格納します。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_ipv4_resolve(const char *text, uint32_t *address_out,
     ...
     *  @param[in]      buffer_size @p buffer のバイト数。@ref COM_UTIL_IPV4_ADDR_STRLEN 以上を指定します。
     *  @param[out]     detail_out  エラー詳細の格納先。NULL 可。
     *  @return         @ref COM_UTIL_OK 、@ref COM_UTIL_ERR_INVALID_ARGUMENT 、
     *                  @ref COM_UTIL_ERR_BUFFER_TOO_SMALL 、@ref COM_UTIL_ERR_UNKNOWN のいずれかを返します。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     *  内部に共有状態を持ちません。
     */
    COM_UTIL_EXPORT int COM_UTIL_API com_util_ipv4_to_string(uint32_t address, char *buffer, size_t buffer_size,
                                                             com_util_error *detail_out);
