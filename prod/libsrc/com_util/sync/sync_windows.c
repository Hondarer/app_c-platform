...
#define APP_LOCK_RANGE_LOW 1U
#define APP_LOCK_RANGE_HIGH 0U
                                                     ...
                                                     com_util_app_lock_t **lock)
{
    ...
    handle = CreateFileA(identity, GENERIC_READ | GENERIC_WRITE,
...
static com_util_sync_result_t app_lock_take(com_util_app_lock_t *lock, DWORD flags,
                                            uint32_t timeout_ms)
{
    ULONGLONG deadline;
    OVERLAPPED ov;
    ...
    do
    {
        if (LockFileEx(lock->handle, flags | LOCKFILE_FAIL_IMMEDIATELY, 0,
                       APP_LOCK_RANGE_LOW, APP_LOCK_RANGE_HIGH, &ov))
        {
            lock->locked = 1;
            return COM_UTIL_SYNC_OK;
        }
...
com_util_sync_result_t com_util_mutex_create(com_util_mutex_t **mtx)
{
    ...
    if (!InitializeCriticalSectionAndSpinCount(&new_mtx->native, 1000))
    {
        free(new_mtx);
        return COM_UTIL_SYNC_SYSTEM_ERROR;
    }
...
com_util_sync_result_t com_util_mutex_lock(com_util_mutex_t *mtx, uint32_t timeout_ms)
{
    ...
    {
        EnterCriticalSection(&mtx->native);
        return COM_UTIL_SYNC_OK;
    }
    deadline = GetTickCount64() + (ULONGLONG)timeout_ms;
    do
    {
        if (TryEnterCriticalSection(&mtx->native))
        {
            return COM_UTIL_SYNC_OK;
        }
        if (timeout_ms == COM_UTIL_SYNC_NO_WAIT)
...
com_util_sync_result_t com_util_mutex_unlock(com_util_mutex_t *mtx)
{
    ...
    LeaveCriticalSection(&mtx->native);
    return COM_UTIL_SYNC_OK;
}
...
void com_util_mutex_destroy(com_util_mutex_t *mtx)
{
    if (mtx != NULL)
    {
        DeleteCriticalSection(&mtx->native);
        free(mtx);
    }
}
                                             ...
                                             uint32_t timeout_ms)
{
    ...
    if (SleepConditionVariableCS(&cv->native, &mtx->native, wait_ms))
    {
        return COM_UTIL_SYNC_OK;
    }
...
com_util_sync_result_t com_util_app_lock_unlock(com_util_app_lock_t *lock)
{
    ...
    memset(&ov, 0, sizeof(ov));
    if (!UnlockFileEx(lock->handle, 0, APP_LOCK_RANGE_LOW, APP_LOCK_RANGE_HIGH, &ov))
    {
        return COM_UTIL_SYNC_SYSTEM_ERROR;
    }
    lock->locked = 0;
