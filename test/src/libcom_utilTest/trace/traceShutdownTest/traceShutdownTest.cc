#include <testfw.h>
#include <mock_com_util.h>
#include <com_util/runtime/shutdown.h>
#include <com_util/trace/tracer.h>
#include <com_util/trace/tracer_internal.h>

using testing::NiceMock;
using testing::Return;
using testing::_;

namespace
{

class traceShutdownTest : public Test
{
protected:
    NiceMock<Mock_com_util> mock_;

#if defined(PLATFORM_LINUX)
    com_util_syslog_sink_t *os_handle_ =
        reinterpret_cast<com_util_syslog_sink_t *>(static_cast<uintptr_t>(0x1100));
#elif defined(PLATFORM_WINDOWS)
    com_util_etw_provider_t *os_handle_ =
        reinterpret_cast<com_util_etw_provider_t *>(static_cast<uintptr_t>(0x1100));
#endif

    void SetUp() override
    {
        _com_util_shutdown_reset_for_test();

#if defined(PLATFORM_LINUX)
        ON_CALL(mock_, com_util_syslog_sink_create(_, _))
            .WillByDefault(Return(os_handle_));
        ON_CALL(mock_, com_util_syslog_sink_dispose(_))
            .WillByDefault(Return());
        ON_CALL(mock_, com_util_syslog_sink_rename(_, _))
            .WillByDefault(Return(0));
#elif defined(PLATFORM_WINDOWS)
        ON_CALL(mock_, com_util_etw_provider_create(_))
            .WillByDefault(Return(os_handle_));
        ON_CALL(mock_, com_util_etw_provider_dispose(_))
            .WillByDefault(Return());
#endif
    }
};

} // namespace

TEST_F(traceShutdownTest, test_shutdown_disposes_registry_and_rejects_new_create)
{
    // Arrange
    com_util_tracer_t *handle = com_util_tracer_create(); // [手順] - tracer を 1 件生成する。
    ASSERT_NE((com_util_tracer_t *)NULL, handle);
    EXPECT_EQ((size_t)1, trace_registry_count()); // [確認_正常系] - registry に 1 件登録されること。

    com_util_shutdown_event_t event = {
        COM_UTIL_SHUTDOWN_REASON_NORMAL_EXIT,
        COM_UTIL_SHUTDOWN_CODE_KIND_NONE,
        0}; // [状態] - 通常終了イベントで tracer shutdown を実行する。

    // Act
    int invoke_result = _com_util_shutdown_invoke_for_test(&event); // [手順] - 共通 shutdown を実行する。
    com_util_tracer_t *created_after_shutdown = com_util_tracer_create(); // [手順] - shutdown 後に新しい tracer を生成する。

    // Assert
    EXPECT_EQ(0, invoke_result); // [確認_正常系] - shutdown 実行が成功すること。
    EXPECT_EQ((size_t)0, trace_registry_count()); // [確認_正常系] - shutdown 後に registry が空になること。
    EXPECT_EQ((com_util_tracer_t *)NULL, created_after_shutdown); // [確認_正常系] - shutdown 開始後は新規 tracer 作成が拒否されること。
}
