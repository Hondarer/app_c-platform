...
TEST_F(hashtableMoreTest, delete_rec_and_purge)
{
    ...
    (void)com_util_hashtable_add(ht, "keep", value.data(),
                                 COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 残すキーを追加する。
    (void)com_util_hashtable_add(ht, "drop", value.data(),
                                 COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 削除するキーを追加する。
...
TEST_F(hashtableMoreTest, delete_with_lifetime_two_expires_immediately)
{
    ...
    int actual_ret_add = com_util_hashtable_add(
        ht, "b", value.data(), COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 別キーを直ちに追加する。
    com_util_hashtable_dispose(ht);
...
TEST_F(hashtableMoreTest, delete_with_lifetime_two_sets_next_empty_when_table_full)
{
    ...
    (void)com_util_hashtable_add(
        ht, "b", value.data(),
        COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 満杯にする(next_empty が 0 になる)。
    ...
    int actual_ret_add = com_util_hashtable_add(
        ht, "c", value.data(), COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 空いた直後にすぐ追加する。
    com_util_hashtable_dispose(ht);
...
TEST_F(hashtableMoreTest, delete_with_lifetime_two_keeps_earlier_next_empty_hint)
{
    ...
    (void)com_util_hashtable_insert_direct(ht, 3, "c", 1, value.data(), &k_insert_timestamp,
    ...
    int actual_ret_add = com_util_hashtable_add(
        ht, "d", value.data(), COM_UTIL_HASHTABLE_ADD_DELETED_OVERWRITE); // [手順] - 別キーを追加する。
...
TEST_F(hashtableMoreTest, add_reuses_deleted_record_with_highest_status_when_no_empty_slot)
{
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht); // [手順] - テーブルを構築する。
    (void)com_util_hashtable_insert_direct(ht, 1, "old", 2, value.data(), &ts_old,
                                           1); // [手順] - status 2(削除中)のレコードを先に配置する。
    (void)com_util_hashtable_insert_direct(ht, 2, "new", 3, value.data(), &ts_new,
...
TEST_F(hashtableMoreTest, add_reuses_oldest_generation_when_status_ties_with_record_scope)
{
    ...
    com_util_timespec ts_a = {200, 0}; // [状態] - a の方が新しい変更時刻とする。
    ...
    (void)com_util_hashtable_insert_direct(ht, 1, "a", 3, value.data(), &ts_a,
                                           2); // [手順] - status 3、新しい時刻、古い世代を配置する。
    (void)com_util_hashtable_insert_direct(ht, 2, "b", 3, value.data(), &ts_b,
    ...
    int actual_ret_get_status_b =
        com_util_hashtable_get_status(ht, 2, &status_b); // [手順] - b(レコード2)の状態を読む。
    com_util_hashtable_dispose(ht);
...
TEST_F(hashtableMoreTest, add_reuses_lowest_record_number_when_status_ties_with_table_scope)
{
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht); // [手順] - テーブルを構築する。
    (void)com_util_hashtable_insert_direct(ht, 1, "a", 3, value.data(), NULL,
                                           0); // [手順] - status 3 のレコード1を配置する。
    (void)com_util_hashtable_insert_direct(ht, 2, "b", 3, value.data(), NULL,
...
TEST_F(hashtableMoreTest, add_reuses_deleted_record_that_is_not_at_chain_head)
{
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht); // [手順] - テーブルを構築する。
    (void)com_util_hashtable_insert_direct(ht, 1, base_key, 2, value.data(), &ts_deleted,
