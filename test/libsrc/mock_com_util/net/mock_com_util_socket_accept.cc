...
MOCK_WEAK_IMPL(int, com_util_socket_accept, com_util_socket sock, com_util_ipv4_endpoint *peer_out, com_util_socket *sock_out, com_util_error *detail_out)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_socket_accept(sock, peer_out, sock_out, detail_out);
    }
    else
    {
        mock_ret = delegate_real_com_util_socket_accept(sock, peer_out, sock_out, detail_out);
    }
