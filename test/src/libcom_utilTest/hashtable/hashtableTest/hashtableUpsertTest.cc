...
TEST_F(hashtableUpsertTest, inserts_then_updates_the_same_key)
{
    ...
    int actual_ret_first = com_util_hashtable_upsert(ht, "a", value.data(),
                                                     &inserted_first); // [手順] - 未登録のキーを upsert する。
    fill_value(&value, "v2");
    int actual_ret_second = com_util_hashtable_upsert(ht, "a", value.data(),
    ...
    int actual_ret_count = com_util_hashtable_count(ht, &in_use);
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_first);  // [確認_正常系] - 未登録のキーの upsert が成功すること。
    EXPECT_EQ(1, inserted_first);              // [確認_正常系] - 未登録のキーは新規追加として報告されること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_second); // [確認_正常系] - 登録済みのキーの upsert が成功すること。
    ...
    EXPECT_EQ(COM_UTIL_OK, actual_ret_count);
...
TEST_F(hashtableUpsertTest, accepts_null_inserted_out)
{
    ...
    int actual_ret_insert =
        com_util_hashtable_upsert(ht, "a", value.data(), NULL); // [手順] - 格納先を省いて upsert する。
    int actual_ret_update =
        com_util_hashtable_upsert(ht, "a", value.data(), NULL); // [手順] - 格納先を省いて再度 upsert する。
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_insert); // [確認_正常系] - 格納先が NULL でも新規追加できること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_update); // [確認_正常系] - 格納先が NULL でも既存更新できること。
}
...
TEST_F(hashtableUpsertTest, revives_deleted_key_with_the_given_value)
{
    ...
    (void)com_util_hashtable_delete(ht, "a"); // [手順] - キーを削除済みにする。
    fill_value(&value, "v2");
    int actual_ret_upsert =
        com_util_hashtable_upsert(ht, "a", value.data(), &inserted); // [手順] - 削除済みのキーを upsert する。
    int actual_ret_status = com_util_hashtable_get_status(ht, 1, &status);
    int actual_ret_read = com_util_hashtable_find_value_copy(ht, "a", read_back.data(), read_back.size(), &required);
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_upsert); // [確認_正常系] - 削除済みのキーの upsert が成功すること。
...
TEST_F(hashtableUpsertTest, reports_limit_exceeded_when_table_is_full)
{
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht);
    (void)com_util_hashtable_upsert(ht, "a", value.data(), NULL);
    (void)com_util_hashtable_upsert(ht, "b", value.data(), NULL);
    int actual_ret_full =
        com_util_hashtable_upsert(ht, "c", value.data(), &inserted); // [手順] - 満杯の状態で新しいキーを upsert する。
    int actual_ret_existing =
        com_util_hashtable_upsert(ht, "a", value.data(), &inserted); // [手順] - 満杯でも登録済みのキーを upsert する。
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_LIMIT_EXCEEDED,
              actual_ret_full);                  // [確認_異常系] - 満杯での新規追加が LIMIT_EXCEEDED であること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_existing); // [確認_正常系] - 満杯でも登録済みのキーは更新できること。
...
TEST_F(hashtableUpsertTest, guards_reject_invalid_arguments)
{
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht);
    int actual_ret_null_ht = com_util_hashtable_upsert(NULL, "a", value.data(), &inserted);
    int actual_ret_null_key = com_util_hashtable_upsert(ht, NULL, value.data(), &inserted);
    int actual_ret_null_value = com_util_hashtable_upsert(ht, "a", NULL, &inserted);
    int actual_ret_long_key = com_util_hashtable_upsert(ht, "0123456789", value.data(),
                                                        &inserted); // [手順] - key_size に収まらないキーを渡す。
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, actual_ret_null_ht);    // [確認_異常系] - ht が NULL なら失敗すること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, actual_ret_null_key);   // [確認_異常系] - キーが NULL なら失敗すること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, actual_ret_null_value); // [確認_異常系] - 値が NULL なら失敗すること。
...
TEST_F(hashtableUpsertTest, updates_key_that_is_not_at_chain_head)
{
    ...
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht);
    (void)com_util_hashtable_upsert(ht, "a", value.data(), NULL);
    (void)com_util_hashtable_upsert(ht, peer, value.data(),
                                    NULL); // [手順] - 同一バケットへ別キーを足してチェイン先頭をずらす。
    fill_value(&value, "v3");
    int actual_ret_upsert =
        com_util_hashtable_upsert(ht, "a", value.data(), &inserted); // [手順] - チェイン先頭でないキーを upsert する。
    int actual_ret_read = com_util_hashtable_find_value_copy(ht, "a", read_back.data(), read_back.size(), &required);
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_upsert); // [確認_正常系] - チェイン先頭でないキーを更新できること。
    EXPECT_EQ(0, inserted);                    // [確認_正常系] - 既存更新として報告されること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_read);
...
TEST_F(hashtableUpsertTest, reports_storage_full_when_variable_value_does_not_fit)
{
    ...
    *(&config) = {};
    ...
    // Act
    (void)com_util_hashtable_create(&config, NULL, 0, NULL, 0, &ht);
    int actual_ret_insert = com_util_hashtable_upsert(ht, "a", "short", &inserted); // [手順] - 収まる値で追加する。
    int actual_ret_update = com_util_hashtable_upsert(ht, "a", "far too long for the storage",
                                                      &inserted); // [手順] - 収まらない値で同じキーを更新する。
    com_util_hashtable_dispose(ht);

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_insert); // [確認_正常系] - 収まる値の追加が成功すること。
