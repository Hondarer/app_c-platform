...
TEST_F(argparserTest, default_options_applied_only_on_first_call)
{
    ...
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage); // [確認_正常系] - 2 回目時点の usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, register_error_collection_accumulates_all_failures)
{
    ...
    // Act
    int rtc_argparser_register_flag = com_util_argparser_register_flag(
        parser, "-a", "--aa", NULL, &a); // [手順] - 成功する登録を行う (コレクションに積まれない)。
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_register_flag); // [確認_正常系] - 成功する登録に対する com_util_argparser_register_flag の戻り値が COM_UTIL_OK であること。
    int rtc_argparser_register_flag_2 = com_util_argparser_register_flag(
        parser, "-a", "--bb", NULL, &b); // [手順] - 短い名前の重複 (1 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        rtc_argparser_register_flag_2); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、短い名前の重複 (1 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    int rtc_argparser_register_flag_3 = com_util_argparser_register_flag(
        parser, "-c", "--cc", NULL, NULL); // [手順] - 格納先 NULL (2 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        rtc_argparser_register_flag_3); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、格納先 NULL (2 件目のエラー) を発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    int rtc_argparser_register_flag_4 = com_util_argparser_register_flag(
        parser, "-d", "--aa", NULL, &b); // [手順] - 別の重複 (3 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        rtc_argparser_register_flag_4); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、別の重複 (3 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
...
TEST_F(argparserTest, register_error_getters_default_when_absent_or_out_of_range)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        rtc_argparser_register_flag); // [確認_異常系] - com_util_argparser_register_flag の戻り値として、エラーを 1 件発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
...
TEST_F(argparserTest, register_error_message_formatting)
{
    ...
    // Act
    int rtc_argparser_get_register_error_message = com_util_argparser_get_register_error_message(
        parser, 0, message, sizeof(message)); // [手順] - 1 件目のエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_register_error_message); // [確認_正常系] - 1 件目のエラー メッセージを取得した com_util_argparser_get_register_error_message の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, flag_counts_occurrences_and_resets_on_reparse)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - フラグが 3 回出現する引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - フラグなしの引数で再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_int_accepts_all_syntaxes)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c 5" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - "-c 5" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(5, count); // [確認_正常系] - "-c 5" で 5 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("--count"), cstr("6"));
        int rtc_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--count 6" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - "--count 6" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(6, count); // [確認_正常系] - "--count 6" で 6 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("--count=7"));
        int rtc_argparser_parse_3 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--count=7" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_3); // [確認_正常系] - "--count=7" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(7, count); // [確認_正常系] - "--count=7" で 7 が格納されること。
    }
    {
        ARGV(cstr("prog"), cstr("-c=8"));
        int rtc_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c=8" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_4); // [確認_正常系] - "-c=8" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(8, count); // [確認_正常系] - "-c=8" で 8 が格納されること。
    }
    {
        /* 値位置のトークンは照合しないため負数を渡せる */
        ARGV(cstr("prog"), cstr("-c"), cstr("-5"));
        int rtc_argparser_parse_5 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-c -5" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_5); // [確認_正常系] - "-c -5" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_6); // [確認_正常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_int_boundary_and_conversion_errors)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MAX を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - INT_MAX を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        int rtc_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - INT_MIN を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - INT_MIN を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            ...
            rtc_argparser_parse_3); // [確認_異常系] - INT_MAX + 1 の "2147483648" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            ...
            rtc_argparser_parse_4); // [確認_異常系] - 数値でない "12a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            rtc_argparser_parse_5); // [確認_異常系] - 空値の "--count=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
...
TEST_F(argparserTest, positional_int_accepts_negative_value_without_hiding_unknown_options)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - 負数 "-42" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - 負数 "-42" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_OUT_OF_RANGE,
            rtc_argparser_parse_2); // [確認_異常系] - INT_MIN - 1 の "-2147483649" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 範囲外が OUT_OF_RANGE になること。
    }
    {
        ...
        EXPECT_EQ(
            ...
            rtc_argparser_parse_3); // [確認_異常系] - 数値でない "-x" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
...
TEST_F(argparserTest, option_string_points_into_argv)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-n abc" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - "-n abc" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(argv[2], name);  // [確認_正常系] - 格納先が argv[2] そのものを指すこと。
        EXPECT_STREQ("abc", name); // [確認_正常系] - 値が "abc" であること。
    }
    {
        ARGV(cstr("prog"), cstr("--name=xyz"));
        int rtc_argparser_parse_2 = com_util_argparser_parse(parser, argc, argv); // [手順] - "--name=xyz" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - "--name=xyz" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_3); // [確認_正常系] - 空値の "--name=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_STREQ("", name); // [確認_正常系] - 文字列では空値 "" が受理されること。
    }
    {
        ARGV(cstr("prog"), cstr("-n=xyz"));
        int rtc_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - "-n=xyz" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_4); // [確認_正常系] - "-n=xyz" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_STREQ("xyz", name); // [確認_正常系] - 短いオプションの "=" 区切りが受理されること。
    }
    {
        ARGV(cstr("prog"), cstr("-n="));
        int rtc_argparser_parse_5 = com_util_argparser_parse(parser, argc, argv); // [手順] - 空値の "-n=" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_5); // [確認_正常系] - 空値の "-n=" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_string_accepts_value_with_spaces)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - 空白を含む位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - "--param=parameter string" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_3); // [確認_正常系] - "-p=parameter string" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_4); // [確認_正常系] - "--param" と後続トークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_5); // [確認_正常系] - "-p" と後続トークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_string_stores_argv_verbatim)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - 両端にクオートが残存した値を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - 値の途中にクオートを含むトークンを解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, positional_assignment_and_overflow)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "in.txt 3" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - "in.txt 3" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            rtc_argparser_parse_2); // [確認_異常系] - int 位置引数に "abc" を渡して解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            ...
            rtc_argparser_parse_3); // [確認_異常系] - 登録数を超える 3 つの位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
    ...
    {
        ARGV(cstr("prog"));
        int rtc_argparser_parse_4 = com_util_argparser_parse(parser, argc, argv); // [手順] - 位置引数なしで解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            rtc_argparser_parse_4); // [確認_異常系] - 位置引数なしで解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
...
TEST_F(argparserTest, positional_string_array_assignment_and_reparse)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(
            parser, argc, argv); // [手順] - 単数位置引数、可変長位置引数、フラグが混在する入力を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - 単数位置引数、可変長位置引数、フラグが混在する入力を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
            rtc_argparser_parse_2); // [確認_異常系] - 容量 2 を超える 3 件の可変長位置引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
        EXPECT_EQ(COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_3); // [確認_正常系] - 可変長位置引数を省略して再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ((size_t)0,
...
TEST_F(argparserTest, positional_int_array_conversion_and_required)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - 負数と正数を可変長 int 位置引数として解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_INVALID_INTEGER,
            rtc_argparser_parse_2); // [確認_異常系] - 整数へ変換できない "12a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_INVALID_INTEGER であること。
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("values",
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_OUT_OF_RANGE,
            rtc_argparser_parse_3); // [確認_異常系] - int の下限を下回る値を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_OUT_OF_RANGE であること。
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            rtc_argparser_parse_4); // [確認_異常系] - 必須の可変長位置引数を省略して解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_get_error(parser)); // [確認_異常系] - 必須欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("values",
                     com_util_argparser_get_error_target(
    ...
    char usage[256];
    int rtc_argparser_get_usage = com_util_argparser_get_usage(
        parser, usage, sizeof(usage), NULL); // [手順] - 必須の可変長位置引数を含む usage を組み立てる。
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、必須の可変長位置引数を含む usage を組み立てた結果が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
...
TEST_F(argparserTest, unknown_option_detection)
{
    ...
    {
        ARGV(cstr("prog"), cstr("-x"));
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - 未登録の "-x" を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse); // [確認_異常系] - 未登録の "-x" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse_2); // [確認_異常系] - 短オプション連結の "-vv" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse_3); // [確認_異常系] - 未登録の "--bogus" を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, array_option_multiple_occurrences)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - "-i dir1 --include=dir2" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            ...
            rtc_argparser_parse_2); // [確認_異常系] - 容量 2 を超える 3 回の出現を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_TOO_MANY_OCCURRENCES であること。
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_OCCURRENCES,
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_3); // [確認_正常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, array_option_int_and_required)
{
    ...
    // Assert
    {
        ...
        int rtc_argparser_parse = com_util_argparser_parse(parser, argc, argv); // [手順] - "-p 80 -p 443" を解析する。
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - "-p 80 -p 443" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_MISSING_REQUIRED,
            rtc_argparser_parse_2); // [確認_異常系] - オプション非出現の引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
...
TEST_F(argparserTest, reparse_clears_error_state)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse); // [確認_異常系] - com_util_argparser_parse の戻り値として、未登録の "-x" で解析エラーを発生させた結果が COM_UTIL_ERR_UNKNOWN_OPTION であること。
        EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse_2); // [確認_正常系] - 正しい引数 "-v" で再解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        EXPECT_EQ(COM_UTIL_OK,
...
TEST_F(argparserTest, multiple_handles_are_independent)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - parser1 で "-a" を解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
        int rtc_argparser_parse_2 =
            com_util_argparser_parse(parser2, argc, argv); // [手順] - "-a" 未登録の parser2 でも同じ引数を解析する。
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse_2); // [確認_異常系] - "-a" 未登録の parser2 でも同じ引数を解析した com_util_argparser_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    }
...
TEST_F(argparserTest, error_message_formatting)
{
    ...
    // Act
    int rtc_argparser_get_error_message = com_util_argparser_get_error_message(
        parser, message, sizeof(message)); // [手順] - 未解析の状態でエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_error_message); // [確認_正常系] - 未解析の状態でエラー メッセージを取得した com_util_argparser_get_error_message の戻り値が COM_UTIL_OK であること。
...

    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            rtc_argparser_parse); // [確認_異常系] - com_util_argparser_parse の戻り値として、未登録の "--bogus" で解析エラーを発生させた結果が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    }
...
TEST_F(argparserTest, usage_formatting)
{
    ...
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、get_usage で usage を組み立てた結果が COM_UTIL_OK であること。
    ...
    size_t query_size = 0;
    int rtc_argparser_get_usage_2 = com_util_argparser_get_usage(
        parser, NULL, 0, &query_size); // [手順] - buffer NULL でサイズ問い合わせのみ行う。
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage_2); // [確認_正常系] - com_util_argparser_get_usage の戻り値として、buffer NULL でサイズ問い合わせのみ行った結果が COM_UTIL_OK であること。
...
TEST_F(argparserTest, usage_program_name_resolution)
{
    ...
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage); // [確認_正常系] - 解析前に usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
...

    {
        ...
        ASSERT_EQ(
            COM_UTIL_OK,
            rtc_argparser_parse); // [確認_正常系] - argv[0] を "/usr/local/bin/mytool" として解析した com_util_argparser_parse の戻り値が COM_UTIL_OK であること。
    }
    int rtc_argparser_get_usage_2 =
        com_util_argparser_get_usage(parser, usage, sizeof(usage), NULL); // [手順] - 解析後に usage を取得する。
    ASSERT_EQ(
        COM_UTIL_OK,
        rtc_argparser_get_usage_2); // [確認_正常系] - 解析後に usage を取得した com_util_argparser_get_usage の戻り値が COM_UTIL_OK であること。
