...
TEST_F(argparserTest, create_and_dispose)
{
    ...
    // Assert
    EXPECT_TRUE(handle_created); // [確認_正常系] - com_util_argparser_handle_create の戻り値が NULL でないこと。
...
TEST_F(argparserTest, default_options_applied_only_on_first_call)
{
    ...
    char before[256];
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_get_usage(first, before, sizeof(before),
                                                         NULL)); // [状態] - 初回時点の usage を記録する。
                                                                 // [状態確認] - com_util_argparser_handle_get_usage の戻り値が COM_UTIL_OK であること。
    ...
    int actual_ret_argparser_get_usage =
        com_util_argparser_handle_get_usage(second, after, sizeof(after), NULL); // [手順] - 2 回目時点の usage を取得する。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - 2 回目時点の usage を取得した com_util_argparser_handle_get_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, register_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_flag(
                  NULL, "-v", "--verbose", NULL,
                  &storage)); // [確認_異常系] - parser NULL の登録が INVALID_ARGUMENT になること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_register_flag(parser, "-v", "--verbose", NULL,
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_flag(parser, "v", NULL, NULL,
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_register_flag(parser, NULL, "--a=b", NULL,
                                          &storage)); // [確認_異常系] - 長い名前 "--a=b" の形式不正が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_option_int(
                  parser, "-c", NULL, NULL, NULL, 0x100u,
                  &storage)); // [確認_異常系] - 未定義の登録フラグ 0x100 が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_option_string_array(
                  parser, "-i", NULL, NULL, NULL, 0, &string_storage, 0,
                  NULL)); // [確認_異常系] - 配列オプションの capacity 0 / count NULL が検出されること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, com_util_argparser_handle_register_positional_int(
                                                 parser, NULL, NULL, 0,
                                                 &storage)); // [確認_異常系] - 位置引数の名前 NULL が検出されること。
    size_t positional_count = 0;
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_positional_int_array(
...
TEST_F(argparserTest, register_rejects_duplicate_definition)
{
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_handle_register_option_int(parser, "-v", NULL, NULL, NULL, 0,
                                                &storage)); // [確認_異常系] - 短い名前 "-v" の重複が検出されること。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        com_util_argparser_handle_register_flag(parser, NULL, "--verbose", NULL,
...
TEST_F(argparserTest, register_rejects_required_positional_after_optional)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(
                               parser, "first", NULL, 0,
                               &first)); // [状態] - 任意の位置引数 "first" を登録済みとする。
                                         // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_register_positional_string(
...
TEST_F(argparserTest, register_accepts_required_positionals_in_sequence)
{
    ...
    // Act
    int first_result =
        com_util_argparser_handle_register_positional_string(parser, "first", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                       &first); // [手順] - 必須位置引数 "first" を登録する。
    int second_result =
        com_util_argparser_handle_register_positional_string(parser, "second", NULL, COM_UTIL_ARGPARSER_REQUIRED,
...
TEST_F(argparserTest, register_requires_variadic_positional_to_be_last)
{
    ...
    int verbose = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string_array(
                               parser, "values", NULL, 0, values, 2,
                               &value_count)); // [状態] - 可変長位置引数 "values" を登録済みとする。
                                               // [状態確認] - com_util_argparser_handle_register_positional_string_array の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_register_positional_string_array(
                  parser, "more", NULL, 0, values, 2,
                  &value_count)); // [確認_異常系] - 可変長位置引数を複数登録できないこと。
    EXPECT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(
...
TEST_F(argparserTest, register_error_collection_accumulates_all_failures)
{
    ...
    // Act
    int actual_ret_argparser_register_flag = com_util_argparser_handle_register_flag(
        parser, "-a", "--aa", NULL, &a); // [手順] - 成功する登録を行う (コレクションに積まれない)。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_register_flag); // [確認_正常系] - 成功する登録に対する com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    int actual_ret_argparser_register_flag_2 = com_util_argparser_handle_register_flag(
        parser, "-a", "--bb", NULL, &b); // [手順] - 短い名前の重複 (1 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        actual_ret_argparser_register_flag_2); // [確認_異常系] - com_util_argparser_handle_register_flag の戻り値として、短い名前の重複 (1 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    int actual_ret_argparser_register_flag_3 = com_util_argparser_handle_register_flag(
        parser, "-c", "--cc", NULL, NULL); // [手順] - 格納先 NULL (2 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_argparser_register_flag_3); // [確認_異常系] - com_util_argparser_handle_register_flag の戻り値として、格納先 NULL (2 件目のエラー) を発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    int actual_ret_argparser_register_flag_4 = com_util_argparser_handle_register_flag(
        parser, "-d", "--aa", NULL, &b); // [手順] - 別の重複 (3 件目のエラー) を発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        actual_ret_argparser_register_flag_4); // [確認_異常系] - com_util_argparser_handle_register_flag の戻り値として、別の重複 (3 件目のエラー) を発生させた結果が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。

    // Assert
    ASSERT_EQ((size_t)3,
              com_util_argparser_handle_get_register_error_count(parser)); // [確認_正常系] - エラー件数が 3 であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_get_register_error(parser,
                                                     0)); // [確認_正常系] - 1 件目が DUPLICATE_DEFINITION であること。
    EXPECT_STREQ("--bb", com_util_argparser_handle_get_register_error_target(
                             parser, 0)); // [確認_正常系] - 1 件目の対象が "--bb" であること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT, com_util_argparser_handle_get_register_error(
                                                 parser, 1)); // [確認_正常系] - 2 件目が INVALID_ARGUMENT であること。
    EXPECT_STREQ("--cc", com_util_argparser_handle_get_register_error_target(
                             parser, 1)); // [確認_正常系] - 2 件目の対象が "--cc" であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_get_register_error(parser,
                                                     2)); // [確認_正常系] - 3 件目が DUPLICATE_DEFINITION であること。
    EXPECT_STREQ("--aa", com_util_argparser_handle_get_register_error_target(
...
TEST_F(argparserTest, register_error_getters_default_when_absent_or_out_of_range)
{
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_handle_get_register_error(
            parser,
            0)); // [確認_正常系] - com_util_argparser_handle_get_register_error の戻り値として、エラーなしのとき OK が返ること。
    EXPECT_EQ(nullptr, com_util_argparser_handle_get_register_error_target(
                           parser, 0)); // [確認_正常系] - エラーなしのとき対象が NULL であること。

    EXPECT_EQ(
        (size_t)0,
        com_util_argparser_handle_get_register_error_count(
            NULL)); // [確認_異常系] - com_util_argparser_handle_get_register_error_count の戻り値として、NULL ハンドルで件数 0 が返ること。
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_handle_get_register_error(
            NULL,
            0)); // [確認_異常系] - com_util_argparser_handle_get_register_error の戻り値として、NULL ハンドルで OK が返ること。
    EXPECT_EQ(
        nullptr,
        com_util_argparser_handle_get_register_error_target(
            NULL,
            0)); // [確認_異常系] - com_util_argparser_handle_get_register_error_target の戻り値として、NULL ハンドルで NULL が返ること。

    int actual_ret_argparser_register_flag =
        com_util_argparser_handle_register_flag(parser, "-a", NULL, NULL, NULL); // [手順] - エラーを 1 件発生させる。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_argparser_register_flag); // [確認_異常系] - com_util_argparser_handle_register_flag の戻り値として、エラーを 1 件発生させた結果が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    ASSERT_EQ((size_t)1, com_util_argparser_handle_get_register_error_count(parser));
    EXPECT_EQ(
        COM_UTIL_OK,
        com_util_argparser_handle_get_register_error(
            parser,
            1)); // [確認_異常系] - com_util_argparser_handle_get_register_error の戻り値として、範囲外インデックスで OK が返ること。
    EXPECT_EQ(
        nullptr,
        com_util_argparser_handle_get_register_error_target(
            parser,
            1)); // [確認_異常系] - com_util_argparser_handle_get_register_error_target の戻り値として、範囲外インデックスで NULL が返ること。
...
TEST_F(argparserTest, register_error_message_formatting)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-a", "--aa", NULL, &a)); // [状態] - フラグ "-a" / "--aa" を登録する。
                                                                                               // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_register_flag(parser, "-a", "--bb", NULL,
                                                &a)); // [状態] - 重複登録エラーを 1 件積んだ parser とする。
                                                      // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    ASSERT_EQ((size_t)1, com_util_argparser_handle_get_register_error_count(parser)); // [状態確認] - com_util_argparser_handle_get_register_error_count の戻り値が 1 であること。
    char message[128];

    // Pre-Assert

    // Act
    int actual_ret_argparser_get_register_error_message = com_util_argparser_handle_get_register_error_message(
        parser, 0, message, sizeof(message)); // [手順] - 1 件目のエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_register_error_message); // [確認_正常系] - 1 件目のエラー メッセージを取得した com_util_argparser_handle_get_register_error_message の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        ...
        com_util_argparser_handle_get_register_error_message(
            ...
            sizeof(
                small_buffer))); // [確認_異常系] - com_util_argparser_handle_get_register_error_message の戻り値として、バッファー不足時は切り詰めて BUFFER_TOO_SMALL が返ること。
...
TEST_F(argparserTest, print_register_error_messages_rejects_invalid_arguments)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_print_register_error_messages(
...
TEST_F(argparserTest, print_register_error_messages_is_noop_without_error)
{
    ...
    // Act
    int result = com_util_argparser_handle_print_register_error_messages(
        parser, stderr); // [手順] - print_register_error_messages を呼び出す。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        result); // [確認_正常系] - com_util_argparser_handle_print_register_error_messages の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, print_register_error_messages_writes_all_to_stream)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_DUPLICATE_DEFINITION,
        com_util_argparser_handle_register_flag(parser, "-a", "--bb", NULL, &a)); // [状態] - 重複登録エラーを積んでおく。
                                                                            // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_register_flag(parser, "-c", "--cc", NULL, NULL)); // [状態] - 不正引数エラーを積んでおく。
                                                                              // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
...
TEST_F(argparserTest, register_grows_beyond_initial_capacity)
{
    ...
    for (int i = 0; i < spec_count; i++)
    {
        ...
        ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, NULL, long_name, NULL, &storages[i]));
    }
...
TEST_F(argparserTest, flag_with_value_is_unexpected_value)
{
    ...
    EXPECT_EQ(
        ...
        result); // [確認_異常系] - com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNEXPECTED_VALUE であること。
    EXPECT_EQ(COM_UTIL_ERR_UNEXPECTED_VALUE,
              com_util_argparser_handle_get_error(parser)); // [確認_異常系] - エラー種別が UNEXPECTED_VALUE であること。
    EXPECT_STREQ("--verbose",
                 com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "--verbose" であること。
    EXPECT_EQ(1, com_util_argparser_handle_get_error_index(parser));  // [確認_異常系] - エラー位置が argv[1] であること。
...
TEST_F(argparserTest, flag_with_short_value_is_unexpected_value)
{
    ...
    EXPECT_EQ(COM_UTIL_ERR_UNEXPECTED_VALUE,
              com_util_argparser_handle_get_error(parser)); // [確認_異常系] - エラー種別が UNEXPECTED_VALUE であること。
    EXPECT_STREQ("--verbose", com_util_argparser_handle_get_error_target(
...
TEST_F(argparserTest, option_int_accepts_all_syntaxes)
{
    ...
    int count = -100;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_option_int(parser, "-c", "--count", "N", NULL, 0,
                                                      &count)); // [状態] - int オプション "-c" / "--count" を登録する。
                                                                // [状態確認] - com_util_argparser_handle_register_option_int の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, option_int_boundary_and_conversion_errors)
{
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 範囲外が OUT_OF_RANGE になること。
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("--count", com_util_argparser_handle_get_error_target(
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - int の空値が INVALID_INTEGER になること。
    }
...
TEST_F(argparserTest, positional_int_accepts_negative_value_without_hiding_unknown_options)
{
    ...
    int value = 0;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_int(parser, "value", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                          &value)); // [状態] - 必須の int 位置引数 "value" を登録する。
                                                                    // [状態確認] - com_util_argparser_handle_register_positional_int の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            com_util_argparser_handle_get_error(parser)); // [確認_異常系] - "-x" が UNKNOWN_OPTION として検出されること。
    }
...
TEST_F(argparserTest, option_string_accepts_value_with_spaces)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_string(
                               ...
                               &param)); // [状態] - 文字列オプション "-p" / "--param" を登録する。
                                         // [状態確認] - com_util_argparser_handle_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_string(parser, "input", NULL, 0,
...
TEST_F(argparserTest, positional_assignment_and_overflow)
{
    ...
    int level = -1;
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_string(parser, "input", NULL, COM_UTIL_ARGPARSER_REQUIRED,
                                                             &input)); // [状態] - 必須の位置引数 "input" を登録する。
                                                                       // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_int(parser, "level", NULL, 0,
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("level", com_util_argparser_handle_get_error_target(
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 超過が TOO_MANY_ARGUMENTS になること。
        EXPECT_STREQ("extra", com_util_argparser_handle_get_error_target(
                                  parser)); // [確認_異常系] - エラー対象が超過トークン "extra" であること。
        EXPECT_EQ(3, com_util_argparser_handle_get_error_index(parser)); // [確認_異常系] - エラー位置が argv[3] であること。
    }
    {
        ARGV(cstr("prog"));
        ...
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 必須欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("input",
                     com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "input" であること。
        EXPECT_EQ(-1, com_util_argparser_handle_get_error_index(
...
TEST_F(argparserTest, positional_string_array_assignment_and_reparse)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string_array(
                               ...
                               &file_count)); // [状態] - 任意の可変長位置引数 "files" を容量 2 で登録する。
                                              // [状態確認] - com_util_argparser_handle_register_positional_string_array の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(
                               parser, "-v", "--verbose", NULL,
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 容量超過が TOO_MANY_ARGUMENTS になること。
        EXPECT_STREQ(
            "c.txt",
            com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が超過した "c.txt" であること。
        EXPECT_EQ(4,
                  com_util_argparser_handle_get_error_index(
...
TEST_F(argparserTest, positional_int_array_conversion_and_required)
{
    ...
    int values[3] = {};
    size_t value_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_int_array(
                               parser, "values", NULL, COM_UTIL_ARGPARSER_REQUIRED, values, 3,
                               &value_count)); // [状態] - 必須の可変長 int 位置引数 "values" を登録する。
                                               // [状態確認] - com_util_argparser_handle_register_positional_int_array の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_INVALID_INTEGER,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 変換エラーが INVALID_INTEGER になること。
        EXPECT_STREQ("values",
                     com_util_argparser_handle_get_error_target(
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_OUT_OF_RANGE,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 範囲外エラーが OUT_OF_RANGE になること。
    }
    {
        ARGV(cstr("prog"));
        int actual_ret_argparser_parse_4 =
    ...
    char usage[256];
    int actual_ret_argparser_get_usage = com_util_argparser_handle_get_usage(
        parser, usage, sizeof(usage), NULL); // [手順] - 必須の可変長位置引数を含む usage を組み立てる。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - com_util_argparser_handle_get_usage の戻り値として、必須の可変長位置引数を含む usage を組み立てた結果が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
...
TEST_F(argparserTest, unknown_option_detection)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - UNKNOWN_OPTION になること。
        EXPECT_STREQ("-x",
                     com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "-x" であること。
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_UNKNOWN_OPTION,
            com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 連結は未サポートで UNKNOWN_OPTION になること。
    }
    {
        ...
        EXPECT_STREQ("--bogus", com_util_argparser_handle_get_error_target(
...
TEST_F(argparserTest, missing_value_at_end)
{
    ...
    EXPECT_EQ(COM_UTIL_ERR_MISSING_VALUE,
              result); // [確認_異常系] - com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_MISSING_VALUE であること。
    EXPECT_EQ(COM_UTIL_ERR_MISSING_VALUE,
              com_util_argparser_handle_get_error(parser)); // [確認_異常系] - エラー種別が MISSING_VALUE であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, duplicate_option_occurrence)
{
    ...
    EXPECT_EQ(
        ...
        result); // [確認_異常系] - com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_DUPLICATE_OPTION であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_OPTION,
              com_util_argparser_handle_get_error(parser)); // [確認_異常系] - エラー種別が DUPLICATE_OPTION であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, array_option_multiple_occurrences)
{
    ...
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_option_string_array(
                  ...
                  &include_count)); // [状態] - 容量 2 の文字列配列オプション "-i" / "--include" を登録する。
                                    // [状態確認] - com_util_argparser_handle_register_option_string_array の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(
            COM_UTIL_ERR_TOO_MANY_OCCURRENCES,
            com_util_argparser_handle_get_error(parser)); // [確認_異常系] - 容量超過が TOO_MANY_OCCURRENCES になること。
        EXPECT_STREQ("--include", com_util_argparser_handle_get_error_target(
...
TEST_F(argparserTest, array_option_int_and_required)
{
    ...
    int ports[4] = {};
    size_t port_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_int_array(
                               parser, "-p", "--port", "PORT", NULL, COM_UTIL_ARGPARSER_REQUIRED, ports, 4,
                               &port_count)); // [状態] - 必須の int 配列オプション "-p" / "--port" を登録する。
                                              // [状態確認] - com_util_argparser_handle_register_option_int_array の戻り値が COM_UTIL_OK であること。
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
                  com_util_argparser_handle_get_error(
                      parser)); // [確認_異常系] - REQUIRED の配列オプション欠落が MISSING_REQUIRED になること。
        EXPECT_STREQ("--port", com_util_argparser_handle_get_error_target(
...
TEST_F(argparserTest, missing_required_option)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_MISSING_REQUIRED,
        result); // [確認_異常系] - com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_MISSING_REQUIRED であること。
    EXPECT_EQ(COM_UTIL_ERR_MISSING_REQUIRED,
              com_util_argparser_handle_get_error(parser)); // [確認_異常系] - エラー種別が MISSING_REQUIRED であること。
    EXPECT_STREQ("--count",
                 com_util_argparser_handle_get_error_target(parser)); // [確認_異常系] - エラー対象が "--count" であること。
...
TEST_F(argparserTest, reparse_clears_error_state)
{
    ...
    // Assert
    {
        ...
        EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
                  com_util_argparser_handle_get_error(parser)); // [確認_異常系] - UNKNOWN_OPTION が記録されること。
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_OK,
                  com_util_argparser_handle_get_error(parser)); // [確認_正常系] - エラー種別が NONE に戻ること。
        EXPECT_EQ(nullptr,
                  com_util_argparser_handle_get_error_target(parser));    // [確認_正常系] - エラー対象が NULL に戻ること。
        EXPECT_EQ(-1, com_util_argparser_handle_get_error_index(parser)); // [確認_正常系] - エラー位置が -1 に戻ること。
    }
...
TEST_F(argparserTest, multiple_handles_are_independent)
{
    ...
    int flag1 = 0;
    int flag2 = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser1, "-a", NULL, NULL,
                                                             &flag1)); // [状態] - parser1 に "-a" を登録する。
                                                                       // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser2, "-b", NULL, NULL,
    ...
    EXPECT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_get_error(parser1)); // [確認_正常系] - parser1 のエラー状態が NONE であること。
    EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              com_util_argparser_handle_get_error(
...
TEST_F(argparserTest, error_message_formatting)
{
    ...
    // Act
    int actual_ret_argparser_get_error_message = com_util_argparser_handle_get_error_message(
        parser, message, sizeof(message)); // [手順] - 未解析の状態でエラー メッセージを取得する。

    // Assert
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_error_message); // [確認_正常系] - 未解析の状態でエラー メッセージを取得した com_util_argparser_handle_get_error_message の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(COM_UTIL_OK, com_util_argparser_handle_get_error_message(parser, message, sizeof(message)));
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_BUFFER_TOO_SMALL,
        com_util_argparser_handle_get_error_message(
            parser, small_buffer,
            sizeof(
                small_buffer))); // [確認_異常系] - com_util_argparser_handle_get_error_message の戻り値として、バッファー不足時に BUFFER_TOO_SMALL が返ること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_get_error_message(parser, NULL,
                                                    1)); // [確認_異常系] - buffer NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_get_error_message(parser, message,
...
TEST_F(argparserTest, usage_formatting)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-v", "--verbose", "verbose output", &verbose)); // [状態] - フラグ "-v" / "--verbose" を登録する。
                                                                                                                      // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_int(parser, "-c", "--count", "N", "count value",
                                                                   COM_UTIL_ARGPARSER_REQUIRED, &count)); // [状態] - 必須の int オプション "-c" / "--count" を登録する。
                                                                                                          // [状態確認] - com_util_argparser_handle_register_option_int の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_option_string(parser, NULL, "--name", "NAME", "display name", 0, &name)); // [状態] - 文字列オプション "--name" を登録する。
                                                                                                                     // [状態確認] - com_util_argparser_handle_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(parser, "input", "input file",
                                                                          COM_UTIL_ARGPARSER_REQUIRED, &input)); // [状態] - 必須の位置引数 "input" を登録する。
                                                                                                                 // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(parser, "output", "output file", 0, &output)); // [状態] - 位置引数 "output" を登録する。
                                                                                                                         // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string_array(
    ...
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - com_util_argparser_handle_get_usage の戻り値として、get_usage で usage を組み立てた結果が COM_UTIL_OK であること。
    ...
    size_t query_size = 0;
    int actual_ret_argparser_get_usage_2 = com_util_argparser_handle_get_usage(
        parser, NULL, 0, &query_size); // [手順] - buffer NULL でサイズ問い合わせのみ行う。
    EXPECT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage_2); // [確認_正常系] - com_util_argparser_handle_get_usage の戻り値として、buffer NULL でサイズ問い合わせのみ行った結果が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_BUFFER_TOO_SMALL,
        com_util_argparser_handle_get_usage(
            parser, small_buffer, sizeof(small_buffer),
            &small_required)); // [確認_異常系] - com_util_argparser_handle_get_usage の戻り値として、バッファー不足時に BUFFER_TOO_SMALL が返ること。
    ...
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_get_usage(NULL, usage, sizeof(usage),
...
TEST_F(argparserTest, print_usage_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_print_usage(NULL, stdout)); // [確認_異常系] - parser NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        com_util_argparser_handle_print_usage(parser, NULL)); // [確認_異常系] - stream NULL が INVALID_ARGUMENT になること。
...
TEST_F(argparserTest, print_usage_writes_to_stream)
{
    ...
    // Act
    int result = com_util_argparser_handle_print_usage(parser, stdout); // [手順] - print_usage を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK,
              result); // [確認_正常系] - com_util_argparser_handle_print_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, print_error_messages_rejects_invalid_arguments)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_print_error_messages(
                  NULL, stderr)); // [確認_異常系] - parser NULL が INVALID_ARGUMENT になること。
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              com_util_argparser_handle_print_error_messages(
                  parser, NULL)); // [確認_異常系] - stream NULL が INVALID_ARGUMENT になること。
...
TEST_F(argparserTest, print_error_messages_is_noop_without_error)
{
    ...
    // Act
    int result = com_util_argparser_handle_print_error_messages(parser, stderr); // [手順] - print_error_messages を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK,
              result); // [確認_正常系] - com_util_argparser_handle_print_error_messages の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, usage_program_name_resolution)
{
    ...
    // Act
    int actual_ret_argparser_get_usage =
        com_util_argparser_handle_get_usage(parser, usage, sizeof(usage), NULL); // [手順] - 解析前に usage を取得する。

    // Assert
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage); // [確認_正常系] - 解析前に usage を取得した com_util_argparser_handle_get_usage の戻り値が COM_UTIL_OK であること。
    EXPECT_THAT(
        std::string(usage),
        HasSubstr(
    ...
    int actual_ret_argparser_get_usage_2 =
        com_util_argparser_handle_get_usage(parser, usage, sizeof(usage), NULL); // [手順] - 解析後に usage を取得する。
    ASSERT_EQ(
        COM_UTIL_OK,
        actual_ret_argparser_get_usage_2); // [確認_正常系] - 解析後に usage を取得した com_util_argparser_handle_get_usage の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, parse_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(nullptr, com_util_argparser_handle_get_error_target(
...
TEST_F(argparserTest, positional_register_rejects_null_parser_and_invalid_flags)
{
    ...
    // Act
    int null_parser_result = com_util_argparser_handle_register_positional_int(
        NULL, "value", NULL, 0u, &storage); // [手順] - parser NULL で位置引数を登録する。
    int invalid_flags_result = com_util_argparser_handle_register_positional_int(
...
TEST_F(argparserTest, register_wrappers_reject_invalid_storage_and_registration)
{
    ...
    // Act
    int option_int_null = com_util_argparser_handle_register_option_int(
        parser, "-i", NULL, NULL, NULL, 0u, NULL); // [手順] - int オプションの storage NULL を検証する。
    int option_string_null = com_util_argparser_handle_register_option_string(
        parser, "-s", NULL, NULL, NULL, 0u, NULL); // [手順] - string オプションの storage NULL を検証する。
    int option_int_array_storage_null = com_util_argparser_handle_register_option_int_array(
        parser, "-a", NULL, NULL, NULL, 0u, NULL, 1u, &int_count); // [手順] - int 配列の storage NULL を検証する。
    int option_int_array_capacity_zero = com_util_argparser_handle_register_option_int_array(
        parser, "-b", NULL, NULL, NULL, 0u, int_array, 0u, &int_count); // [手順] - int 配列の capacity 0 を検証する。
    int option_int_array_count_null = com_util_argparser_handle_register_option_int_array(
        parser, "-c", NULL, NULL, NULL, 0u, int_array, 1u, NULL); // [手順] - int 配列の count NULL を検証する。
    int option_string_array_storage_null = com_util_argparser_handle_register_option_string_array(
        parser, "-d", NULL, NULL, NULL, 0u, NULL, 1u,
        &string_count); // [手順] - string 配列の storage NULL を検証する。
    int option_string_array_capacity_zero = com_util_argparser_handle_register_option_string_array(
        parser, "-e", NULL, NULL, NULL, 0u, string_array, 0u,
        &string_count); // [手順] - string 配列の capacity 0 を検証する。
    int option_string_array_count_null = com_util_argparser_handle_register_option_string_array(
        parser, "-f", NULL, NULL, NULL, 0u, string_array, 1u, NULL); // [手順] - string 配列の count NULL を検証する。
    int positional_int_null = com_util_argparser_handle_register_positional_int(
        parser, "value", NULL, 0u, NULL); // [手順] - int 位置引数の storage NULL を検証する。
    int positional_string_null = com_util_argparser_handle_register_positional_string(
        parser, "text", NULL, 0u, NULL); // [手順] - string 位置引数の storage NULL を検証する。
    int positional_int_array_storage_null = com_util_argparser_handle_register_positional_int_array(
        parser, "values", NULL, 0u, NULL, 1u, &int_count); // [手順] - int 配列位置引数の storage NULL を検証する。
    int positional_int_array_capacity_zero = com_util_argparser_handle_register_positional_int_array(
        parser, "values2", NULL, 0u, int_array, 0u, &int_count); // [手順] - int 配列位置引数の capacity 0 を検証する。
    int positional_int_array_count_null = com_util_argparser_handle_register_positional_int_array(
        parser, "values3", NULL, 0u, int_array, 1u, NULL); // [手順] - int 配列位置引数の count NULL を検証する。
    int positional_string_array_storage_null = com_util_argparser_handle_register_positional_string_array(
        parser, "texts", NULL, 0u, NULL, 1u, &string_count); // [手順] - string 配列位置引数の storage NULL を検証する。
    int positional_string_array_capacity_zero = com_util_argparser_handle_register_positional_string_array(
        parser, "texts2", NULL, 0u, string_array, 0u,
        &string_count); // [手順] - string 配列位置引数の capacity 0 を検証する。
    int positional_string_array_count_null = com_util_argparser_handle_register_positional_string_array(
        parser, "texts3", NULL, 0u, string_array, 1u, NULL); // [手順] - string 配列位置引数の count NULL を検証する。

    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_int_array(parser, "-x", "--x", NULL, NULL, 0u, int_array,
                                                                         1u, &int_count));
    int option_int_array_duplicate = com_util_argparser_handle_register_option_int_array(
        parser, "-x", "--x2", NULL, NULL, 0u, int_array, 1u, &int_count); // [手順] - int 配列の重複登録を検証する。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_string_array(parser, "-y", "--y", NULL, NULL, 0u,
                                                                            string_array, 1u, &string_count));
    int option_string_array_duplicate =
        com_util_argparser_handle_register_option_string_array(parser, "-y", "--y2", NULL, NULL, 0u, string_array, 1u,
                                                         &string_count); // [手順] - string 配列の重複登録を検証する。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string_array(parser, "tail", NULL, 0u, string_array,
                                                                                1u, &string_count));
    int positional_int_array_duplicate = com_util_argparser_handle_register_positional_int_array(
        parser, "tail2", NULL, 0u, int_array, 1u, &int_count); // [手順] - 可変長位置引数後の int 配列登録を検証する。
    int positional_string_array_duplicate = com_util_argparser_handle_register_positional_string_array(
...
TEST_F(argparserTest, parse_rejects_null_tokens_and_handles_negative_positionals)
{
    ...
    int values[2] = {};
    size_t value_count = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_int(parser, "-c", "--count", NULL, NULL, 0u, &count)); // [状態] - int オプション "-c" / "--count" を登録する。
                                                                                                                      // [状態確認] - com_util_argparser_handle_register_option_int の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_int_array(parser, "values", NULL, 0u, values, 2u, &value_count)); // [状態] - 可変長 int 位置引数 "values" を登録する。
                                                                                                                        // [状態確認] - com_util_argparser_handle_register_positional_int_array の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, parse_classifies_bare_prefixes_and_unregistered_negative_value)
{
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        double_dash_result); // [確認_異常系] - "--" を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_TOO_MANY_ARGUMENTS,
        single_dash_result); // [確認_異常系] - "-" を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_TOO_MANY_ARGUMENTS であること。
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        negative_result); // [確認_異常系] - 登録のない負数を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, parse_searches_options_with_only_one_name)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-a", NULL, NULL, &short_flag)); // [状態] - フラグ "-a" を登録する。
                                                                                                      // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(parser, "input", NULL, 0u, &positional)); // [状態] - 位置引数 "input" を登録する。
                                                                                                                    // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, NULL, "--beta", NULL, &long_flag)); // [状態] - フラグ "--beta" を登録する。
                                                                                                         // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        unknown_long_result); // [確認_異常系] - --zeta を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
    EXPECT_EQ(
        COM_UTIL_ERR_UNKNOWN_OPTION,
        unknown_short_result); // [確認_異常系] - -b を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, parse_handles_positional_search_and_array_conversion_failure)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(parser, "text", NULL, 0u, &text)); // [状態] - 位置引数 "text" を登録する。
                                                                                                             // [状態確認] - com_util_argparser_handle_register_positional_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_int_array(parser, "-n", "--number", NULL, NULL, 0u,
                                                                         numbers, 1u, &number_count)); // [状態] - int 配列オプション "-n" / "--number" を登録する。
                                                                                                       // [状態確認] - com_util_argparser_handle_register_option_int_array の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK,
              com_util_argparser_handle_register_positional_int_array(parser, "values", NULL, 0u, values, 1u, &value_count)); // [状態] - 可変長 int 位置引数 "values" を登録する。
                                                                                                                        // [状態確認] - com_util_argparser_handle_register_positional_int_array の戻り値が COM_UTIL_OK であること。
...
TEST_F(argparserTest, usage_handles_short_name_default_value_and_long_label)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-v", NULL, NULL, &flag)); // [状態] - フラグ "-v" を登録する。
                                                                                                // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_option_string(parser, "-n", NULL, NULL, NULL, 0u, &value)); // [状態] - 文字列オプション "-n" を登録する。
                                                                                                                    // [状態確認] - com_util_argparser_handle_register_option_string の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_positional_string(parser, "123456789012345678901234567890",
...
TEST_F(argparserTest, error_message_formats_all_error_results)
{
    ...
    for (int result : results)
    {
        ...
        return_codes.push_back(com_util_argparser_handle_get_error_message(
...
TEST_F(argparserTest, error_message_returns_invalid_argument_when_snprintf_fails)
{
    ...
    // Act
    int result = com_util_argparser_handle_get_error_message(
...
TEST_F(argparserTest, register_error_message_formats_all_results)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-a", "--alpha", NULL, &storage)); // [状態] - フラグ "-a" / "--alpha" を登録する。
                                                                                                        // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_register_flag(parser, "-a", "--beta", NULL, &storage)); // [状態] - 重複登録エラーを積んだ状態とする。
                                                                                          // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_ERR_DUPLICATE_DEFINITION であること。
    ASSERT_EQ((size_t)1, com_util_argparser_handle_get_register_error_count(parser)); // [状態確認] - com_util_argparser_handle_get_register_error_count の戻り値が 1 であること。
    char message[128] = {};
    const int results[] = {
    ...
    for (int result : results)
    {
        ...
        return_codes.push_back(com_util_argparser_handle_get_register_error_message(
            parser, 0u, message, sizeof(message))); // [手順] - 各登録結果のエラーメッセージを取得する。
    }
    test_argparser_clear_register_error_target(parser, 0u);
    int null_target_result = com_util_argparser_handle_get_register_error_message(
...
TEST_F(argparserTest, register_error_message_returns_invalid_argument_when_snprintf_fails)
{
    ...
    // Act
    int result = com_util_argparser_handle_get_register_error_message(
...
TEST_F(argparserTest, print_usage_returns_buffer_too_small_when_usage_changes)
{
    ...
    // Act
    int result = com_util_argparser_handle_print_usage(
...
TEST_F(argparserTest, print_error_messages_writes_buffer_too_small_message)
{
    ...
    // Act
    int result = com_util_argparser_handle_print_error_messages(
...
TEST_F(argparserTest, print_register_error_messages_writes_buffer_too_small_message)
{
    ...
    int storage = 0;
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-a", long_name.c_str(), NULL, &storage)); // [状態] - 長い long_name のフラグ "-a" を登録する。
                                                                                                                // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_register_flag(parser, "-b", long_name.c_str(), NULL,
    ...
    // Act
    int result =
        com_util_argparser_handle_print_register_error_messages(parser, stderr); // [手順] - 長い対象の登録エラーを出力する。
...
TEST_F(argparserTest, print_register_error_messages_skips_message_when_snprintf_fails)
{
    ...
    ASSERT_EQ(COM_UTIL_OK, com_util_argparser_handle_register_flag(parser, "-a", "--alpha", NULL, &storage)); // [状態] - フラグ "-a" / "--alpha" を登録する。
                                                                                                        // [状態確認] - com_util_argparser_handle_register_flag の戻り値が COM_UTIL_OK であること。
    ASSERT_EQ(COM_UTIL_ERR_DUPLICATE_DEFINITION,
              com_util_argparser_handle_register_flag(parser, "-b", "--alpha", NULL,
...
TEST_F(argparserTest, parse_skips_short_name_when_length_differs)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ERR_UNKNOWN_OPTION,
              result); // [確認_異常系] - 長さが一致しない短い名前を解析した com_util_argparser_handle_parse の戻り値が COM_UTIL_ERR_UNKNOWN_OPTION であること。
...
TEST_F(argparserTest, default_init_resets_previous_registrations)
{
    ...
    (void)com_util_argparser_get_usage(first_usage, sizeof(first_usage), NULL);
