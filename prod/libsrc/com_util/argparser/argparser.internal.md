...
{
    /* COM_UTIL_ARGPARSER_INVALID_ARGUMENT / DUPLICATE_DEFINITION / OUT_OF_MEMORY のいずれか */
}
```
...
`_com_util_argparser_parse()` が `COM_UTIL_ARGPARSER_PARSE_ERROR` を返した場合、詳細は次の API で取得します。
...
バッファーが不足する場合は `COM_UTIL_ARGPARSER_BUFFER_TOO_SMALL` が返り、バッファーには切り詰めた内容が格納されます。
