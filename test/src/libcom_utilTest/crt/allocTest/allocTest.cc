...
TEST_F(allocTest, malloc_allocates_requested_size)
{
    ...
    // Act
    buf = (uint8_t *)com_util_malloc(16U); // [手順] - 16 バイトを指定して com_util_malloc を呼び出す。

    // Assert
    ASSERT_NE((uint8_t *)NULL, buf); // [確認_正常系] - com_util_malloc の戻り値が NULL でないこと。
...
TEST_F(allocTest, malloc_rejects_zero_size)
{
    ...
    // Act
    ptr = com_util_malloc(0U); // [手順] - 0 を指定して com_util_malloc を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - com_util_malloc の戻り値が NULL であること。
}
...
TEST_F(allocTest, malloc_returns_null_on_failure)
{
    ...
    // Act
    ptr = com_util_malloc(SIZE_MAX / 2U); // [手順] - 確保できない大きさ (SIZE_MAX / 2) を指定して呼び出す。
...
TEST_F(allocTest, malloc_zerofill_clears_whole_area)
{
    ...
    // Act
    // [手順] - 64 バイトを指定して com_util_malloc_zerofill を呼び出す。
    buf = (uint8_t *)com_util_malloc_zerofill(64U);

    // Assert
    ASSERT_NE((uint8_t *)NULL, buf); // [確認_正常系] - com_util_malloc_zerofill の戻り値が NULL でないこと。
    for (i = 0U; i < 64U; i++)
    {
        if (buf[i] != 0U)
        {
...
TEST_F(allocTest, malloc_zerofill_rejects_zero_size)
{
    ...
    // Act
    ptr = com_util_malloc_zerofill(0U); // [手順] - 0 を指定して com_util_malloc_zerofill を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - com_util_malloc_zerofill の戻り値が NULL であること。
}
...
TEST_F(allocTest, calloc_allocates_and_clears)
{
    ...
    // Act
    // [手順] - 要素数 8、要素サイズ 4 で com_util_calloc を呼び出す。
    values = (int32_t *)com_util_calloc(8U, sizeof(*values));

    // Assert
    ASSERT_NE((int32_t *)NULL, values); // [確認_正常系] - com_util_calloc の戻り値が NULL でないこと。
    for (i = 0U; i < 8U; i++)
    {
        if (values[i] != 0)
        {
...
TEST_F(allocTest, calloc_rejects_zero_count)
{
    ...
    // Act
    ptr = com_util_calloc(0U, 4U); // [手順] - 要素数 0、要素サイズ 4 で com_util_calloc を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 要素数 0 を渡した com_util_calloc の戻り値が NULL であること。
}
...
TEST_F(allocTest, calloc_rejects_zero_size)
{
    ...
    // Act
    ptr = com_util_calloc(4U, 0U); // [手順] - 要素数 4、要素サイズ 0 で com_util_calloc を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 要素サイズ 0 を渡した com_util_calloc の戻り値が NULL であること。
}
...
TEST_F(allocTest, calloc_rejects_size_overflow)
{
    ...
    // Act
    ptr = com_util_calloc(huge, 8U); // [手順] - 乗算が回り込む要素数と要素サイズ 8 で com_util_calloc を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 乗算が回り込む指定の com_util_calloc の戻り値が NULL であること。
}
...
TEST_F(allocTest, realloc_preserves_existing_content)
{
    ...
    // Act
    new_values = (int32_t *)com_util_realloc(values, 16U, sizeof(*new_values)); // [手順] - 要素数 16 へ伸長する。

    // Assert
    ASSERT_NE((int32_t *)NULL, new_values); // [確認_正常系] - com_util_realloc の戻り値が NULL でないこと。
    EXPECT_EQ(11, new_values[0]);           // [確認_正常系] - 伸長後の先頭要素が 11 のまま保持されていること。
    EXPECT_EQ(44, new_values[3]);           // [確認_正常系] - 伸長後の 4 番目の要素が 44 のまま保持されていること。

    // Cleanup
    com_util_free(new_values);
}
...
TEST_F(allocTest, realloc_allocates_when_ptr_is_null)
{
    ...
    // Act
    // [手順] - ptr に NULL を渡して com_util_realloc を呼び出す。
    values = (int32_t *)com_util_realloc(NULL, 4U, sizeof(*values));

    // Assert
    // [確認_正常系] - ptr に NULL を渡した com_util_realloc の戻り値が NULL でないこと。
    ASSERT_NE((int32_t *)NULL, values);
...
TEST_F(allocTest, realloc_rejects_zero_count_without_free)
{
    ...
    // Act
    ptr = com_util_realloc(values, 0U, sizeof(*values)); // [手順] - 要素数 0 を指定して com_util_realloc を呼び出す。

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 要素数 0 を渡した com_util_realloc の戻り値が NULL であること。
...
TEST_F(allocTest, realloc_rejects_size_overflow_without_free)
{
    ...
    // Act
    // [手順] - 乗算が回り込む要素数と要素サイズ 8 で com_util_realloc を呼び出す。
    ptr = com_util_realloc(values, huge, 8U);

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 乗算が回り込む指定の com_util_realloc の戻り値が NULL であること。
...
TEST_F(allocTest, realloc_zerofill_clears_extended_range_only)
{
    ...
    // Act
    // [手順] - 旧要素数 4、新要素数 16 を指定して com_util_realloc_zerofill を呼び出す。
    new_values = (int32_t *)com_util_realloc_zerofill(values, 4U, 16U, sizeof(*new_values));

    // Assert
    ASSERT_NE((int32_t *)NULL, new_values); // [確認_正常系] - com_util_realloc_zerofill の戻り値が NULL でないこと。
    for (i = 0U; i < 4U; i++)
    {
        if (new_values[i] != 7)
        {
...
TEST_F(allocTest, realloc_zerofill_skips_when_not_extended)
{
    ...
    // Act
    // [手順] - 旧要素数 8、新要素数 2 を指定して com_util_realloc_zerofill を呼び出す (縮小)。
    new_values = (int32_t *)com_util_realloc_zerofill(values, 8U, 2U, sizeof(*new_values));

    // Assert
    // [確認_正常系] - 縮小指定の com_util_realloc_zerofill の戻り値が NULL でないこと。
...
TEST_F(allocTest, free_accepts_null)
{
    ...
    // Act
    com_util_free(NULL); // [手順] - NULL を指定して com_util_free を呼び出す。

    // Assert
    SUCCEED(); // [確認_正常系] - NULL を渡した com_util_free が異常終了せずに戻ること。
}
...
TEST_F(allocTest, realloc_returns_null_on_failure_without_free)
{
    ...
    // Act
    // [手順] - 確保できない要素数 (SIZE_MAX / 4) と要素サイズ 4 で com_util_realloc を呼び出す。
    ptr = com_util_realloc(values, SIZE_MAX / 4U, 4U);

    // Assert
    EXPECT_EQ((void *)NULL, ptr); // [確認_異常系] - 確保に失敗した com_util_realloc の戻り値が NULL であること。
...
TEST_F(allocTest, realloc_zerofill_rejects_zero_count_without_free)
{
    ...
    // Act
    // [手順] - 旧要素数 4、新要素数 0 を指定して com_util_realloc_zerofill を呼び出す。
    ptr = com_util_realloc_zerofill(values, 4U, 0U, sizeof(*values));

    // Assert
    // [確認_異常系] - 要素数 0 を渡した com_util_realloc_zerofill の戻り値が NULL であること。
...
TEST_F(allocTest, realloc_zerofill_returns_null_on_failure_without_free)
{
    ...
    // Act
    // [手順] - 旧要素数 4、確保できない新要素数 (SIZE_MAX / 4) と要素サイズ 4 で com_util_realloc_zerofill を呼び出す。
    ptr = com_util_realloc_zerofill(values, 4U, SIZE_MAX / 4U, 4U);

    // Assert
    // [確認_異常系] - 確保に失敗した com_util_realloc_zerofill の戻り値が NULL であること。
    EXPECT_EQ((void *)NULL, ptr);
