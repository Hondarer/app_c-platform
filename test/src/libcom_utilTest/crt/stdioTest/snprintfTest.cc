...
TEST_F(snprintfTest, formats_into_buffer)
{
    ...
    // Act
    int ret = com_util_snprintf(buf, sizeof(buf), "%s=%d", "count",
                                7); // [手順] - "%s=%d" に "count" と 7 を与えて com_util_snprintf を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, ret);  // [確認_正常系] - com_util_snprintf の戻り値が COM_UTIL_OK であること。
...
TEST_F(snprintfTest, exact_fit_succeeds)
{
    ...
    // Act
    int ret = com_util_snprintf(buf, sizeof(buf), "abc"); // [手順] - 3 文字の "abc" を書式として渡す。
...
TEST_F(snprintfTest, truncation_clears_buffer)
{
    ...
    // Act
    int ret = com_util_snprintf(buf, sizeof(buf), "abcd"); // [手順] - 終端を含めて 5 バイトを要する "abcd" を渡す。

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_BUFFER_TOO_SMALL,
              ret);        // [確認_異常系] - com_util_snprintf の戻り値が COM_UTIL_ERR_BUFFER_TOO_SMALL であること。
...
TEST_F(snprintfTest, null_dest_returns_invalid_argument)
{
    ...
    // Act
    int ret = com_util_snprintf(NULL, 16u, "abc"); // [手順] - 書き込み先に NULL を渡して com_util_snprintf を呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_INVALID_ARGUMENT,
              ret); // [確認_異常系] - com_util_snprintf の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
}
...
TEST_F(snprintfTest, null_format_returns_invalid_argument)
{
    ...
    // Act
    int ret = com_util_snprintf(buf, sizeof(buf), NULL); // [手順] - 書式に NULL を渡して com_util_snprintf を呼び出す。
...
TEST_F(snprintfTest, vsnprintf_formats_into_buffer)
{
    ...
    // Act
    int ret = call_com_util_vsnprintf(buf, sizeof(buf), "%s-%s", "a",
                                      "b"); // [手順] - va_list 経由で "%s-%s" に "a" と "b" を与えて呼び出す。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, ret); // [確認_正常系] - com_util_vsnprintf の戻り値が COM_UTIL_OK であること。
...
TEST_F(snprintfTest, vsnprintf_truncation_clears_buffer)
{
    ...
    // Act
    int ret = call_com_util_vsnprintf(buf, sizeof(buf), "%s",
                                      "abcd"); // [手順] - va_list 経由で収まらない "abcd" を書き込ませる。

    // Assert
    EXPECT_EQ(COM_UTIL_ERR_BUFFER_TOO_SMALL,
              ret);        // [確認_異常系] - com_util_vsnprintf の戻り値が COM_UTIL_ERR_BUFFER_TOO_SMALL であること。
    EXPECT_STREQ("", buf); // [確認_異常系] - 切り詰めた結果を残さずバッファーが空文字列になること。
}
