...
MOCK_WEAK_IMPL(int, cplat_file_write, cplat_file *file, const void *buf, size_t len, cplat_error *detail_out)
{
    int mock_ret = CPLAT_ERR_UNKNOWN;

    if (_mock_cplat != nullptr)
    {
        mock_ret = _mock_cplat->cplat_file_write(file, buf, len, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_file_write(file, buf, len, detail_out);
    }
