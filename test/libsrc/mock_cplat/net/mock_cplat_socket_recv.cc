...
MOCK_WEAK_IMPL(int, cplat_socket_recv, cplat_socket sock, void *buf, size_t len, size_t *received_out, cplat_error *detail_out)
{
    ...
    {
        mock_ret = _mock_cplat->cplat_socket_recv(sock, buf, len, received_out, detail_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_socket_recv(sock, buf, len, received_out, detail_out);
    }
