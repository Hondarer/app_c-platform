...
本書は、メモリ ロック API の詳細仕様を定める正本です。  
runtime カテゴリ全体の要件と機能の位置づけは [実行時支援 (runtime) 機能仕様](functional-spec/runtime.md)、cplat が公開する API 全体の一覧は [cplat API チート シート](api-cheatsheet.md) を参照してください。
