...
extern "C"
{
     ...
     *  @return         @p string_id がカタログに存在しない場合は @ref CPLAT_ERR_NOT_FOUND を返します。
     ...
     *  @par            使用例
        @code{.c}
        static const cplat_string_catalog catalog = {entries, id_index, entry_count, id_index_count};
        char text[CPLAT_STRING_CATALOG_TEXT_MAX];
        cplat_string_catalog_set_language(CPLAT_STRING_CATALOG_LANGUAGE_JAPANESE);
        int ret = cplat_string_catalog_format(&catalog, text, sizeof(text), SAMPLE_MESSAGES_ID_FILE_OPEN_FAILED,
                                         "config.json", 2);
        if (ret == CPLAT_OK)
        {
     ...
     *  読み取り専用のカタログだけを参照します。
     */
    CPLAT_EXPORT int CPLAT_API cplat_string_catalog_verify(const cplat_string_catalog *catalog, int *string_id_out,
