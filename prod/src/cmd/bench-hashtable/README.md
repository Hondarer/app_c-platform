...
可変長ストレージの確保は空き領域の個数に比例します。  
capacity を増やしても 1 件あたりの所要時間がほぼ一定であることを、実測で確かめるために用意しています。

## ドキュメント

- [ベンチマークの測定方法](benchmark-method.md) - 測定軸、計測の仕組み、実行手順、CSV の列
- [hashtable 可変長ストレージ アロケーターの計算量改善](../../../../docs/proposals/hashtable-storage-allocator.md) - 設計の背景と計算量の分析
...
結論の根拠として引用する数値は [hashtable 可変長ストレージ アロケーターの計算量改善](../../../../docs/proposals/hashtable-storage-allocator.md) へ転記します。  
