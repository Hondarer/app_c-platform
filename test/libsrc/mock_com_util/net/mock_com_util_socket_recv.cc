...
MOCK_WEAK_IMPL(int, com_util_socket_recv, com_util_socket sock, void *buf, size_t len, size_t *received_out, com_util_error *detail_out)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_socket_recv(sock, buf, len, received_out, detail_out);
    }
    else
    {
        mock_ret = delegate_real_com_util_socket_recv(sock, buf, len, received_out, detail_out);
    }
