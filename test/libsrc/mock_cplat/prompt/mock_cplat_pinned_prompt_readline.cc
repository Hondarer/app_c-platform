...
MOCK_WEAK_IMPL(int, cplat_pinned_prompt_readline_at, cplat_pinned_prompt *screen, char *buf, size_t buf_size,
               const char *prompt_str, const char *file, int line)
{
    int mock_ret = 0;
    ...
    {
        mock_ret = _mock_cplat->cplat_pinned_prompt_readline_at(screen, buf, buf_size, prompt_str, file, line);
    }
    else
    {
        mock_ret = delegate_real_cplat_pinned_prompt_readline_at(screen, buf, buf_size, prompt_str, file, line);
    }
