...
TEST_F(argparserTest, create_and_dispose)
{
    ...
    _com_util_argparser_dispose(NULL);
}
...
{
    // Arrange & Act
    com_util_argparser *first = _com_util_argparser_default(NULL);
    com_util_argparser *second = _com_util_argparser_default(NULL);
...
TEST_F(argparserTest, default_options_applied_only_on_first_call)
{
    ...
    char before[256];
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(first, before, sizeof(before), NULL));

    com_util_argparser_options options = {};
    options.program_name = "should-be-ignored";

    // Act
    com_util_argparser *second = _com_util_argparser_default(&options);

    char after[256];
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(second, after, sizeof(after), NULL));
...
TEST_F(argparserTest, dispose_ignores_default_handle)
{
    ...
    // Act
    _com_util_argparser_dispose(first);
    com_util_argparser *second = _com_util_argparser_default(NULL);
...
TEST_F(argparserTest, register_rejects_invalid_arguments)
{
    ...
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(NULL, "-v", "--verbose", NULL, &storage));
    // storage NULL
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, "-v", "--verbose", NULL, NULL));
    // 両方の名前が NULL
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, NULL, NULL, NULL, &storage));
    // 短い名前の形式不正
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, "-vv", NULL, NULL, &storage));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, "v", NULL, NULL, &storage));
    // 長い名前の形式不正
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, NULL, "-verbose", NULL, &storage));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_flag(parser, NULL, "--a=b", NULL, &storage));
    // 未定義の登録フラグ
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_option_int(parser, "-c", NULL, NULL, NULL, 0x100u, &storage));
    // 配列オプションの capacity 0 / count NULL
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_register_option_string_array(
                                                       parser, "-i", NULL, NULL, NULL, 0, &string_storage, 0, NULL));
    // 位置引数の名前 NULL
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_register_positional_int(parser, NULL, NULL, 0, &storage));
...
TEST_F(argparserTest, register_rejects_duplicate_definition)
{
    ...
    ASSERT_NE(nullptr, parser);
    int storage = 0;
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_flag(parser, "-v", "--verbose", NULL, &storage));

    // Act & Assert
    // 短い名前の重複
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION,
              _com_util_argparser_register_option_int(parser, "-v", NULL, NULL, NULL, 0, &storage));
    // 長い名前の重複
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION,
              _com_util_argparser_register_flag(parser, NULL, "--verbose", NULL, &storage));
...
TEST_F(argparserTest, register_rejects_required_positional_after_optional)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_positional_string(parser, "first", NULL, 0, &first));

    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_register_positional_string(
...
TEST_F(argparserTest, register_error_collection_accumulates_all_failures)
{
    ...
    // 短い名前の重複 (1 件目)
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION,
              _com_util_argparser_register_flag(parser, "-a", "--bb", NULL, &b));
    // 格納先 NULL (2 件目)
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_register_flag(parser, "-c", "--cc", NULL, NULL));
    // 別の重複 (3 件目)
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION,
              _com_util_argparser_register_flag(parser, "-d", "--aa", NULL, &b));

    // Assert
    ASSERT_EQ((size_t)3, _com_util_argparser_get_register_error_count(parser));
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION, _com_util_argparser_get_register_error(parser, 0));
    EXPECT_STREQ("--bb", _com_util_argparser_get_register_error_target(parser, 0));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_register_error(parser, 1));
    EXPECT_STREQ("--cc", _com_util_argparser_get_register_error_target(parser, 1));
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION, _com_util_argparser_get_register_error(parser, 2));
    EXPECT_STREQ("--aa", _com_util_argparser_get_register_error_target(parser, 2));
...
TEST_F(argparserTest, register_error_getters_default_when_absent_or_out_of_range)
{
    ...
    // Act & Assert
    // エラーなし
    EXPECT_EQ((size_t)0, _com_util_argparser_get_register_error_count(parser));
    EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_register_error(parser, 0));
    EXPECT_EQ(nullptr, _com_util_argparser_get_register_error_target(parser, 0));

    // NULL ハンドル
    EXPECT_EQ((size_t)0, _com_util_argparser_get_register_error_count(NULL));
    EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_register_error(NULL, 0));
    EXPECT_EQ(nullptr, _com_util_argparser_get_register_error_target(NULL, 0));

    // 範囲外インデックス
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_register_flag(parser, "-a", NULL, NULL, NULL));
    ASSERT_EQ((size_t)1, _com_util_argparser_get_register_error_count(parser));
    EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_register_error(parser, 1));
    EXPECT_EQ(nullptr, _com_util_argparser_get_register_error_target(parser, 1));
...
TEST_F(argparserTest, register_error_message_formatting)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_flag(parser, "-a", "--aa", NULL, &a));
    EXPECT_EQ(COM_UTIL_ARGPARSER_DUPLICATE_DEFINITION,
              _com_util_argparser_register_flag(parser, "-a", "--bb", NULL, &a));
    ASSERT_EQ((size_t)1, _com_util_argparser_get_register_error_count(parser));
    char message[128];

    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_OK,
              _com_util_argparser_get_register_error_message(parser, 0, message, sizeof(message)));
    EXPECT_STREQ("failed to register '--bb': duplicate definition", message);

    // バッファー不足時は切り詰めて BUFFER_TOO_SMALL
    char small_buffer[8];
    EXPECT_EQ(COM_UTIL_ARGPARSER_BUFFER_TOO_SMALL,
              _com_util_argparser_get_register_error_message(parser, 0, small_buffer, sizeof(small_buffer)));

    // 不正引数
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_get_register_error_message(NULL, 0, message, sizeof(message)));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_get_register_error_message(parser, 0, NULL, sizeof(message)));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_get_register_error_message(parser, 0, message, 0));
    // 範囲外インデックス
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_get_register_error_message(parser, 1, message, sizeof(message)));
...
TEST_F(argparserTest, print_register_error_messages_rejects_invalid_arguments)
{
    ...
    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_register_error_messages(NULL, stderr));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_register_error_messages(parser, NULL));
...
TEST_F(argparserTest, print_register_error_messages_is_noop_without_error)
{
    ...
    // Act
    int result = _com_util_argparser_print_register_error_messages(parser, stderr);
...
TEST_F(argparserTest, option_int_accepts_all_syntaxes)
{
    ...
    int count = -100;
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK,
              _com_util_argparser_register_option_int(parser, "-c", "--count", "N", NULL, 0, &count));
...
TEST_F(argparserTest, option_int_boundary_and_conversion_errors)
{
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_INVALID_INT, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("--count", _com_util_argparser_get_error_target(parser));
    }
    {
...
TEST_F(argparserTest, option_string_points_into_argv)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK,
              _com_util_argparser_register_option_string(parser, "-n", "--name", "NAME", NULL, 0, &name));
...
TEST_F(argparserTest, positional_assignment_and_overflow)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_positional_int(parser, "level", NULL, 0, &level));
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_INVALID_INT, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("level", _com_util_argparser_get_error_target(parser));
    }
    {
        // 登録数を超える位置引数
        ARGV(cstr("prog"), cstr("a"), cstr("1"), cstr("extra"));
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_TOO_MANY_POSITIONALS, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("extra", _com_util_argparser_get_error_target(parser));
        EXPECT_EQ(3, _com_util_argparser_get_error_index(parser));
    }
    {
        // 必須位置引数の欠落
        ARGV(cstr("prog"));
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_MISSING_REQUIRED, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("input", _com_util_argparser_get_error_target(parser));
        EXPECT_EQ(-1, _com_util_argparser_get_error_index(parser));
    }
...
TEST_F(argparserTest, unknown_option_detection)
{
    ...
    // Act & Assert
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_UNKNOWN_OPTION, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("-x", _com_util_argparser_get_error_target(parser));
    }
    {
        // 短オプションの連結は未サポート
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_STREQ("--bogus", _com_util_argparser_get_error_target(parser));
    }
...
TEST_F(argparserTest, missing_value_at_end)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, result);
    EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_MISSING_VALUE, _com_util_argparser_get_error(parser));
    EXPECT_STREQ("--count", _com_util_argparser_get_error_target(parser));
...
TEST_F(argparserTest, duplicate_option_occurrence)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, result);
    EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_DUPLICATE_OPTION, _com_util_argparser_get_error(parser));
    EXPECT_STREQ("--count", _com_util_argparser_get_error_target(parser));
...
TEST_F(argparserTest, array_option_multiple_occurrences)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_option_string_array(
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_TOO_MANY_OCCURRENCES, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("--include", _com_util_argparser_get_error_target(parser));
    }
    {
...
TEST_F(argparserTest, array_option_int_and_required)
{
    ...
    size_t port_count = 0;
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK,
              _com_util_argparser_register_option_int_array(parser, "-p", "--port", "PORT", NULL,
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_MISSING_REQUIRED, _com_util_argparser_get_error(parser));
        EXPECT_STREQ("--port", _com_util_argparser_get_error_target(parser));
    }
...
TEST_F(argparserTest, missing_required_option)
{
    ...
    ASSERT_NE(nullptr, parser);
    int count = 0;
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_option_int(parser, "-c", "--count", "N", NULL,
...
TEST_F(argparserTest, reparse_clears_error_state)
{
    ...
    }
    {
        ...
        EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_parse(parser, argc, argv));
        EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_NONE, _com_util_argparser_get_error(parser));
        EXPECT_EQ(nullptr, _com_util_argparser_get_error_target(parser));
        EXPECT_EQ(-1, _com_util_argparser_get_error_index(parser));
    }
...
TEST_F(argparserTest, multiple_handles_are_independent)
{
    ...
    int flag1 = 0;
    int flag2 = 0;
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_flag(parser1, "-a", NULL, NULL, &flag1));
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_register_flag(parser2, "-b", NULL, NULL, &flag2));

    // Act & Assert
    {
        ARGV(cstr("prog"), cstr("-a"));
        EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_parse(parser1, argc, argv));
        // parser2 には -a は未登録
        EXPECT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser2, argc, argv));
    }
    EXPECT_EQ(1, flag1);
    EXPECT_EQ(0, flag2);
    // エラー状態も独立している
    EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_NONE, _com_util_argparser_get_error(parser1));
    EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_UNKNOWN_OPTION, _com_util_argparser_get_error(parser2));
...
TEST_F(argparserTest, error_message_formatting)
{
    ...
    char small_buffer[8];
    EXPECT_EQ(COM_UTIL_ARGPARSER_BUFFER_TOO_SMALL,
              _com_util_argparser_get_error_message(parser, small_buffer, sizeof(small_buffer)));
    EXPECT_STREQ("unknown", small_buffer);

    // 不正引数
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT,
              _com_util_argparser_get_error_message(NULL, message, sizeof(message)));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_error_message(parser, NULL, 1));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_error_message(parser, message, 0));
...
TEST_F(argparserTest, usage_formatting)
{
    ...
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(parser, usage, sizeof(usage), &required_size));
    ...
    size_t query_size = 0;
    EXPECT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(parser, NULL, 0, &query_size));
    ...
    EXPECT_EQ(COM_UTIL_ARGPARSER_BUFFER_TOO_SMALL,
              _com_util_argparser_get_usage(parser, small_buffer, sizeof(small_buffer), &small_required));
    EXPECT_EQ(required_size, small_required);
    EXPECT_EQ((size_t)15, strlen(small_buffer));

    // 不正引数
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_usage(NULL, usage, sizeof(usage), NULL));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_usage(parser, NULL, 0, NULL));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_get_usage(parser, usage, 0, NULL));
...
TEST_F(argparserTest, print_usage_rejects_invalid_arguments)
{
    ...
    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_usage(NULL, stdout));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_usage(parser, NULL));
...
TEST_F(argparserTest, print_usage_writes_to_stream)
{
    ...
    // Act
    int result = _com_util_argparser_print_usage(parser, stdout);
...
TEST_F(argparserTest, print_error_messages_rejects_invalid_arguments)
{
    ...
    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_error_messages(NULL, stderr));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_print_error_messages(parser, NULL));
...
TEST_F(argparserTest, print_error_messages_is_noop_without_error)
{
    ...
    // Act
    int result = _com_util_argparser_print_error_messages(parser, stderr);
...
TEST_F(argparserTest, print_error_messages_writes_to_stream)
{
    ...
    ARGV(cstr("prog"), cstr("--bogus"));
    ASSERT_EQ(COM_UTIL_ARGPARSER_PARSE_ERROR, _com_util_argparser_parse(parser, argc, argv));
...
TEST_F(argparserTest, usage_program_name_resolution)
{
    ...
    // Act & Assert
    // 解析前はプレースホルダーを使用する
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(parser, usage, sizeof(usage), NULL));
    EXPECT_THAT(std::string(usage), HasSubstr("Usage: {program} [OPTIONS]\n"));

    // 解析後は argv[0] のベース名を使用する
    {
        ARGV(cstr("/usr/local/bin/mytool"), cstr("-v"));
        ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_parse(parser, argc, argv));
    }
    ASSERT_EQ(COM_UTIL_ARGPARSER_OK, _com_util_argparser_get_usage(parser, usage, sizeof(usage), NULL));
...
TEST_F(argparserTest, parse_rejects_invalid_arguments)
{
    ...
    // Act & Assert
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_parse(NULL, argc, argv));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_parse(parser, 0, argv));
    EXPECT_EQ(COM_UTIL_ARGPARSER_INVALID_ARGUMENT, _com_util_argparser_parse(parser, argc, NULL));

    // getter の NULL 安全
    EXPECT_EQ(COM_UTIL_ARGPARSER_ERROR_NONE, _com_util_argparser_get_error(NULL));
    EXPECT_EQ(nullptr, _com_util_argparser_get_error_target(NULL));
    EXPECT_EQ(-1, _com_util_argparser_get_error_index(NULL));
