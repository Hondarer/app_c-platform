 ...
 *  @brief          Compression API を使用してデータを圧縮および展開する Windows 向け機能を実装します。
 *  @author         Tetsuo Honda
 *  @date           2026/03/05
 *  @version        1.0.0
 *
...
int cplat_compress(uint8_t *dst, size_t *dst_len, const uint8_t *src, const size_t src_len)
{
    ...
    memmove(dst + CPLAT_COMPRESS_HEADER_SIZE, dst + CPLAT_COMPRESS_HEADER_SIZE + 2U, (size_t)(cmp_len - 2U));

    *dst_len = CPLAT_COMPRESS_HEADER_SIZE + (size_t)(cmp_len - 2U);
    return CPLAT_OK;
}
...
int cplat_decompress(uint8_t *dst, size_t *dst_len, const uint8_t *src, const size_t src_len)
{
    ...
    raw_deflate_len = src_len - CPLAT_COMPRESS_HEADER_SIZE;
    ...
    tmp = (uint8_t *)cplat_malloc(tmp_len);
    if (tmp == NULL)
    {
    ...
    /* raw DEFLATE データ */
    memcpy(tmp + 30, src + CPLAT_COMPRESS_HEADER_SIZE, raw_deflate_len);
