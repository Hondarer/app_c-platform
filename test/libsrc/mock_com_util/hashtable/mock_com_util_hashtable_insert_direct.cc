                                                   ...
                                                   const void *value, const com_util_timespec *timestamp)
{
    ...
    return real_fn(ht, record, key, status, value, timestamp);
}

MOCK_WEAK_IMPL(int, com_util_hashtable_insert_direct, com_util_hashtable *ht, uint64_t record, const void *key,
               int status, const void *value, const com_util_timespec *timestamp)
{
    int mock_ret = COM_UTIL_ERR_UNKNOWN;

    if (_mock_com_util != nullptr)
    {
        mock_ret = _mock_com_util->com_util_hashtable_insert_direct(ht, record, key, status, value, timestamp);
    }
    else
    {
