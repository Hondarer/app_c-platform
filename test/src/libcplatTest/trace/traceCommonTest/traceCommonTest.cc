...
TEST(traceCommonTest, resolves_valid_explicit_timestamp)
{
    ...
    // Act
    int actual_ret = trace_resolve_timestamp(
        &timestamp, &resolved,
        &fallback_used); // [手順] - 有効な明示タイムスタンプを指定して trace_resolve_timestamp を呼び出す。

    // Assert
    EXPECT_EQ(0, actual_ret);                            // [確認_正常系] - trace_resolve_timestamp の戻り値が 0 であること。
...
TEST(traceCommonTest, resolves_current_time_for_null_timestamp)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_cplat, cplat_get_realtime(&resolved))
        .WillOnce(
...
TEST(traceCommonTest, rejects_invalid_current_time)
{
    ...
    // Assert
    EXPECT_EQ(-1, actual_ret);          // [確認_異常系] - trace_resolve_timestamp の戻り値が -1 であること。
...
TEST(traceCommonTest, formats_valid_timestamp)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_cplat, cplat_format_realtime_iso8601_local(buf, sizeof(buf), &timestamp))
        .WillOnce(
    ...
    // Act
    int actual_ret = trace_format_local_timestamp(
        buf, sizeof(buf),
        &timestamp); // [手順] - 有効なタイムスタンプを指定して trace_format_local_timestamp を呼び出す。

    // Assert
    EXPECT_EQ(CPLAT_OK, actual_ret);    // [確認_正常系] - trace_format_local_timestamp の戻り値が CPLAT_OK であること。
...
TEST(traceCommonTest, rejects_invalid_timestamp_for_formatting)
{
    ...
    // Assert
    EXPECT_EQ(-1, actual_ret); // [確認_異常系] - trace_format_local_timestamp の戻り値が -1 であること。
}
...
TEST(traceCommonTest, propagates_error_for_null_buffer)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_cplat, cplat_format_realtime_iso8601_local(nullptr, 32, &timestamp))
        .WillOnce(Return(
    ...
    EXPECT_EQ(
        ...
        actual_ret); // [確認_異常系] - trace_format_local_timestamp の戻り値が CPLAT_ERR_INVALID_ARGUMENT であること。
}
...
TEST(traceCommonTest, propagates_error_for_zero_buffer_size)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_cplat, cplat_format_realtime_iso8601_local(buf, 0, &timestamp))
        .WillOnce(Return(
    ...
    EXPECT_EQ(CPLAT_ERR_UNKNOWN,
              actual_ret); // [確認_異常系] - trace_format_local_timestamp の戻り値が CPLAT_ERR_UNKNOWN であること。
}
