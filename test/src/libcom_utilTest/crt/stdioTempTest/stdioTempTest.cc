...
int getenv_empty_tmpdir(const char *, char *buffer, size_t, int *, com_util_error *)
{
    buffer[0] = '\0';
    return COM_UTIL_OK;
}
...
class stdioTempTest : public testing::Test
{
  protected:
    ...
    void SetUp() override
    {
        ON_CALL(mock_com_util_, com_util_getenv(StrEq("TMPDIR"), _, _, _, _)).WillByDefault(getenv_empty_tmpdir);
...
TEST_F(stdioTempTest, opens_writable_file_and_reports_path)
{
    ...
    // Assert
    EXPECT_EQ(kStream, fp);               // [確認_正常系] - com_util_fopen_temp の戻り値が番兵ストリームであること。
...
TEST_F(stdioTempTest, uses_tmpdir_when_getenv_returns_a_path)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_com_util_, com_util_getenv(StrEq("TMPDIR"), _, _, _, _))
        .WillOnce(
            [](const char *, char *buffer, size_t buffer_size, int *, com_util_error *)
            {
                ...
                // [Pre-Assert手順] - "/var/tmp" を格納し、COM_UTIL_OK を返却する。
    EXPECT_CALL(mock_unistd_, mkostemp(_, _, _, path, O_CLOEXEC))
        .WillOnce(
            [](const char *, int, const char *, char *tmpl, int)
            {
    ...
    // Act
    FILE *fp = com_util_fopen_temp("ptr", "wb", path, sizeof(path),
                                   nullptr); // [手順] - TMPDIR が "/var/tmp" の状態で com_util_fopen_temp を呼び出す。
...
TEST_F(stdioTempTest, returns_unique_paths_for_repeated_calls)
{
    ...
    // Act
    FILE *fp1 = com_util_fopen_temp("ptr", "wb", path1, sizeof(path1),
                                    nullptr); // [手順] - 1 回目の com_util_fopen_temp を呼び出す。
    FILE *fp2 = com_util_fopen_temp("ptr", "wb", path2, sizeof(path2),
                                    nullptr); // [手順] - 2 回目の com_util_fopen_temp を呼び出す。
    FILE *fp3 = com_util_fopen_temp("ptr", "wb", path3, sizeof(path3),
                                    nullptr); // [手順] - 3 回目の com_util_fopen_temp を呼び出す。
    FILE *fp4 = com_util_fopen_temp("ptr", "wb", path4, sizeof(path4),
                                    nullptr); // [手順] - 4 回目の com_util_fopen_temp を呼び出す。

    // Assert
    EXPECT_EQ(kStream, fp1); // [確認_正常系] - 1 回目の com_util_fopen_temp の戻り値が番兵ストリームであること。
    EXPECT_EQ(kStream, fp2); // [確認_正常系] - 2 回目の com_util_fopen_temp の戻り値が番兵ストリームであること。
    EXPECT_EQ(kStream, fp3); // [確認_正常系] - 3 回目の com_util_fopen_temp の戻り値が番兵ストリームであること。
    EXPECT_EQ(kStream, fp4); // [確認_正常系] - 4 回目の com_util_fopen_temp の戻り値が番兵ストリームであること。
...
TEST_F(stdioTempTest, null_prefix_is_accepted)
{
    ...
    // Assert
    EXPECT_EQ(kStream,
              fp); // [確認_正常系] - prefix が NULL の com_util_fopen_temp の戻り値が番兵ストリームであること。
...
TEST_F(stdioTempTest, prefix_longer_than_three_chars_is_truncated)
{
    ...
    // Assert
    EXPECT_EQ(kStream,
              fp); // [確認_正常系] - 4 文字以上の prefix でも com_util_fopen_temp の戻り値が番兵ストリームであること。
...
TEST_F(stdioTempTest, tmpdir_buffer_too_small_returns_enametoolong)
{
    ...
    EXPECT_CALL(mock_com_util_, com_util_getenv(StrEq("TMPDIR"), _, _, _, _))
        .WillOnce(Return(
            COM_UTIL_ERR_BUFFER_TOO_SMALL)); // [Pre-Assert確認_異常系] - getenv(TMPDIR) が 1 回呼び出されること。
...
TEST_F(stdioTempTest, path_formatting_failure_returns_enametoolong)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_com_util_, com_util_snprintf(_, _, _))
        .WillOnce(Return(COM_UTIL_ERR_UNKNOWN)); // [Pre-Assert確認_異常系] - snprintf が 1 回呼び出されること。
