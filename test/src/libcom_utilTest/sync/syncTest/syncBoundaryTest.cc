...
TEST(syncBoundaryTest, map_wait_rc_translates_supported_results)
{
    ...
    // Assert
    EXPECT_EQ(COM_UTIL_OK, ok_result); // [確認_正常系] - 成功コードが COM_UTIL_OK に変換されること。
    EXPECT_EQ(COM_UTIL_ERR_TIMEOUT,
              timeout_result);                   // [確認_正常系] - ETIMEDOUT が COM_UTIL_ERR_TIMEOUT に変換されること。
    EXPECT_EQ(COM_UTIL_ERR_BUSY, busy_result);   // [確認_正常系] - EBUSY が COM_UTIL_ERR_BUSY に変換されること。
    EXPECT_EQ(COM_UTIL_ERR_BUSY, again_result);  // [確認_正常系] - EAGAIN が COM_UTIL_ERR_BUSY に変換されること。
    EXPECT_EQ(COM_UTIL_ERR_BUSY, access_result); // [確認_正常系] - EACCES が COM_UTIL_ERR_BUSY に変換されること。
...
TEST(syncBoundaryTest, condvar_rejects_invalid_arguments)
{
    ...
    // Act
    int create_result = com_util_condvar_create(NULL); // [手順] - NULL の格納先で condvar を生成する。
...
TEST(syncBoundaryTest, condvar_timeout_signal_and_broadcast_are_supported)
{
    ...
    // Act
    int create_cv_result = com_util_condvar_create(&cv);                          // [手順] - condvar を生成する。
    int create_lock_result = com_util_local_lock_create(&lock);                   // [手順] - 待機用 lock を生成する。
    int lock_result = com_util_local_lock_lock(lock, COM_UTIL_SYNC_WAIT_FOREVER); // [手順] - lock を取得する。
    int wait_result = com_util_condvar_wait(cv, lock, COM_UTIL_SYNC_NO_WAIT); // [手順] - 即時タイムアウトで待機する。
    int unlock_result = com_util_local_lock_unlock(lock);                     // [手順] - lock を解放する。
    int signal_result = com_util_condvar_signal(cv);                          // [手順] - condvar を通知する。
    int broadcast_result = com_util_condvar_broadcast(cv);                    // [手順] - condvar を一斉通知する。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, create_cv_result);     // [確認_正常系] - condvar 生成が成功すること。
    EXPECT_EQ(COM_UTIL_OK, create_lock_result);   // [確認_正常系] - lock 生成が成功すること。
    EXPECT_EQ(COM_UTIL_OK, lock_result);          // [確認_正常系] - lock 取得が成功すること。
    EXPECT_EQ(COM_UTIL_ERR_TIMEOUT, wait_result); // [確認_正常系] - 即時待機が TIMEOUT になること。
    EXPECT_EQ(COM_UTIL_OK, unlock_result);        // [確認_正常系] - lock 解放が成功すること。
    EXPECT_EQ(COM_UTIL_OK, signal_result);        // [確認_正常系] - condvar 通知が成功すること。
...
TEST(syncBoundaryTest, condvar_wait_forever_returns_after_signal)
{
    ...
    // Act
    int lock_result =
        com_util_local_lock_lock(lock, COM_UTIL_SYNC_WAIT_FOREVER); // [手順] - 待機側が解放した lock を取得する。
    int signal_result = com_util_condvar_signal(cv);                // [手順] - 待機側を通知する。
    int unlock_result = com_util_local_lock_unlock(lock);           // [手順] - 通知側の lock を解放する。
    waiter.join();

    // Assert
    EXPECT_EQ(COM_UTIL_OK, lock_result);   // [確認_正常系] - 通知側の lock 取得が成功すること。
    EXPECT_EQ(COM_UTIL_OK, signal_result); // [確認_正常系] - condvar 通知が成功すること。
    EXPECT_EQ(COM_UTIL_OK, unlock_result); // [確認_正常系] - 通知側の lock 解放が成功すること。
...
TEST(syncBoundaryTest, thread_create_and_join_runs_start_function)
{
    ...
    // Act
    int create_result = com_util_thread_create(&thread, set_thread_flag, &flag); // [手順] - スレッドを生成する。
    int join_result = com_util_thread_join(thread, COM_UTIL_SYNC_WAIT_FOREVER);  // [手順] - スレッド終了を待機する。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, create_result); // [確認_正常系] - スレッド生成が成功すること。
    EXPECT_EQ(COM_UTIL_OK, join_result);   // [確認_正常系] - スレッド join が成功すること。
...
TEST(syncBoundaryTest, call_once_runs_function_only_once)
{
    ...
    // Act
    com_util_call_once(&flag, count_once_call); // [手順] - call_once を 1 回目に呼び出す。
    com_util_call_once(&flag, count_once_call); // [手順] - call_once を 2 回目に呼び出す。
    com_util_call_once(NULL, count_once_call);  // [手順] - NULL flag で call_once を呼び出す。
    com_util_call_once(&flag, NULL);            // [手順] - NULL 関数で call_once を呼び出す。
