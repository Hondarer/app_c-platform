...
MOCK_WEAK_IMPL(int, com_util_socket_set_nonblocking, com_util_socket sock, int enable, com_util_error *detail_out)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_socket_set_nonblocking(sock, enable, detail_out);
    }
    else
    {
        mock_ret = delegate_real_com_util_socket_set_nonblocking(sock, enable, detail_out);
    }
