...
この文書は、`app/com_util/` 配下の作業に適用します。  
...
- [docs/coding-guideline.md](docs/coding-guideline.md)は、com_util 固有の規範です。
...
公開関数を追加、削除、または名称変更する場合は、`test/src/libcom_utilTest/exportTest/exportTest.cc` の `COM_UTIL_EXPORT_TABLE_COMMON` と `COM_UTIL_EXPORT_TABLE_PLATFORM` も同じ変更で確認してください。  
公開関数のシグネチャを変更する場合も、型検査を含むエクスポート テストとの一致を確認してください。

## app 固有の規則

- 一般的な C/C++ 規範は、[共通コーディング規範](../general/docs/coding-guideline.md) に従ってください。
- com_util 固有の結果コード、標準時刻型、制約は、[com_util コーディング規範](docs/coding-guideline.md) に集約してください。
...
- `bench-hashtable` の測定軸を変更する場合は、`prod/src/cmd/bench-hashtable/benchmark-method.md` と `prod/libsrc/com_util/hashtable/hashtable-storage-allocator.md` を同じ変更で確認してください。
- `bench-hashtable` の測定結果は管理対象外です。共有する数値と測定環境は、`prod/libsrc/com_util/hashtable/hashtable-storage-allocator.md` に記載してください。
