               ...
               unsigned int status_mask, uint64_t *record_out, int *has_record_out)
{
    ...
    {
        mock_ret = _mock_cplat->cplat_hashtable_next_record(ht, from, status_mask, record_out, has_record_out);
    }
    else
    {
        mock_ret = delegate_real_cplat_hashtable_next_record(ht, from, status_mask, record_out, has_record_out);
    }
