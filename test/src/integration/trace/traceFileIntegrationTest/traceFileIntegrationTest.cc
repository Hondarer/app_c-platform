...
TEST_F(traceFileIntegrationTest, test_enable_file_trace_writes_messages)
{
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_tracer_write); // [確認_正常系] - ERROR 行を書き込んだ _com_util_tracer_write の戻り値が COM_UTIL_OK であること。
...
TEST_F(traceFileIntegrationTest, test_debug_level_outputs_verbose_and_debug_markers)
{
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_tracer_write); // [確認_正常系] - VERBOSE 行を書き込んだ _com_util_tracer_write の戻り値が COM_UTIL_OK であること。
...
TEST_F(traceFileIntegrationTest, test_level_none_disables_file_trace)
{
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_tracer_write); // [確認_正常系] - 無効化前に呼び出した _com_util_tracer_write の戻り値が COM_UTIL_OK であること。
...
TEST_F(traceFileIntegrationTest, test_two_tracers_share_default_path_in_single_process)
{
    ...
    EXPECT_EQ(
        COM_UTIL_OK,
        rtc_tracer_write); // [確認_正常系] - 1 つ目から呼び出した _com_util_tracer_write の戻り値が COM_UTIL_OK であること。
