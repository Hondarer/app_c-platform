...
{
    static auto real_fn = reinterpret_cast<decltype(&com_util_access_fmt)>(
...
MOCK_WEAK_IMPL(int, com_util_access_fmt, int mode, com_util_error *detail_out, const char *format, ...)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_access_fmt(mode, detail_out, buf.data());
    }
    else
    {
        mock_ret = delegate_real_com_util_access_fmt(mode, detail_out, buf.data());
    }
