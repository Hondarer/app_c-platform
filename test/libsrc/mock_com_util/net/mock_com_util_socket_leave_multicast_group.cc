...
MOCK_WEAK_IMPL(int, com_util_socket_leave_multicast_group, com_util_socket sock, uint32_t group_address, uint32_t interface_address, com_util_error *detail_out)
{
    ...
    {
        mock_ret = _mock_com_util->com_util_socket_leave_multicast_group(sock, group_address, interface_address, detail_out);
    }
    else
    {
        mock_ret = delegate_real_com_util_socket_leave_multicast_group(sock, group_address, interface_address, detail_out);
    }
