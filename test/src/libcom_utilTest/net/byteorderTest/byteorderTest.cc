...
TEST(byteorderTest, hton16_places_most_significant_byte_first)
{
    ...
    // Act
    uint16_t converted = com_util_hton16((uint16_t)0x1234U); // [手順] - 0x1234 をネットワーク バイト オーダーへ変換する。

    // Assert
    to_bytes(converted, bytes);
    EXPECT_EQ(0x12U, bytes[0]); // [確認_正常系] - com_util_hton16 の結果の先頭バイトが上位バイトの 0x12 であること。
    EXPECT_EQ(0x34U, bytes[1]); // [確認_正常系] - com_util_hton16 の結果の 2 バイト目が下位バイトの 0x34 であること。
}
...
TEST(byteorderTest, ntoh16_reads_most_significant_byte_first)
{
    ...
    // Act
    uint16_t converted = com_util_ntoh16(network); // [手順] - バイト列をホスト バイト オーダーへ変換する。

    // Assert
    EXPECT_EQ((uint16_t)0x1234U,
              converted); // [確認_正常系] - com_util_ntoh16 の戻り値が 0x1234 であること。
}
...
TEST(byteorderTest, hton32_places_most_significant_byte_first)
{
    ...
    // Act
    uint32_t converted = com_util_hton32(0x12345678U); // [手順] - 0x12345678 をネットワーク バイト オーダーへ変換する。

    // Assert
    to_bytes(converted, bytes);
    EXPECT_EQ(0x12U, bytes[0]); // [確認_正常系] - com_util_hton32 の結果の 1 バイト目が 0x12 であること。
    EXPECT_EQ(0x34U, bytes[1]); // [確認_正常系] - com_util_hton32 の結果の 2 バイト目が 0x34 であること。
    EXPECT_EQ(0x56U, bytes[2]); // [確認_正常系] - com_util_hton32 の結果の 3 バイト目が 0x56 であること。
    EXPECT_EQ(0x78U, bytes[3]); // [確認_正常系] - com_util_hton32 の結果の 4 バイト目が 0x78 であること。
}
...
TEST(byteorderTest, ntoh32_reads_most_significant_byte_first)
{
    ...
    // Act
    uint32_t converted = com_util_ntoh32(network); // [手順] - バイト列をホスト バイト オーダーへ変換する。

    // Assert
    EXPECT_EQ(0x12345678U,
              converted); // [確認_正常系] - com_util_ntoh32 の戻り値が 0x12345678 であること。
}
...
TEST(byteorderTest, hton16_and_ntoh16_round_trip)
{
    ...
    {
        EXPECT_EQ(value, com_util_ntoh16(com_util_hton16(value)));
        // [確認_正常系] - 境界値を含む各値について com_util_hton16 と com_util_ntoh16 の往復結果が元の値と一致すること。
    }
}
...
TEST(byteorderTest, hton32_and_ntoh32_round_trip)
{
    ...
    {
        EXPECT_EQ(value, com_util_ntoh32(com_util_hton32(value)));
        // [確認_正常系] - 境界値を含む各値について com_util_hton32 と com_util_ntoh32 の往復結果が元の値と一致すること。
    }
}
...
TEST(byteorderTest, hton16_swaps_boundary_values)
{
    ...
    // Act
    uint16_t converted_low = com_util_hton16((uint16_t)0x00FFU);  // [手順] - 下位バイトのみの値を変換する。
    uint16_t converted_high = com_util_hton16((uint16_t)0xFF00U); // [手順] - 上位バイトのみの値を変換する。
