...
{
    // Arrange
    cplat_mmap *map = NULL; // [状態] - ローカル ロック生成前のマップ用パスを用意する。

    // Pre-Assert
    EXPECT_CALL(mock_cplat_, cplat_local_lock_create(_))
        .WillOnce(
            Return(CPLAT_ERR_UNKNOWN)); // [Pre-Assert確認_異常系] - ローカル ロック生成が 1 回呼び出されること。
                                           // [Pre-Assert手順] - cplat_local_lock_create にエラーを返却させる。
...
#endif /* PLATFORM_ */
    EXPECT_CALL(mock_cplat_, cplat_file_close(_, _))
        .Times(1); // [Pre-Assert確認_異常系] - ロック生成失敗時に cplat_file_close が 1 回呼び出されること。
    ...
    EXPECT_NE(
        CPLAT_OK,
        actual_ret); // [確認_異常系] - ローカル ロック生成失敗時の cplat_mmap_attach の戻り値が CPLAT_OK 以外であること。
    EXPECT_EQ((cplat_mmap *)NULL,
              map); // [確認_異常系] - ローカル ロック生成失敗時に cplat_mmap_attach のマップが NULL であること。
}
...
TEST_F(mmapFailureInjectionTest, get_rwlock_reports_error_when_local_lock_lock_fails)
{
    ...
    EXPECT_CALL(mock_cplat_, cplat_local_lock_lock(_, CPLAT_SYNC_WAIT_FOREVER))
        .WillOnce(Return(
            CPLAT_ERR_UNKNOWN)); // [Pre-Assert確認_異常系] - get_rwlock のローカル ロック取得が 1 回呼び出されること。
                                    // [Pre-Assert手順] - cplat_local_lock_lock にエラーを返却させる。

    // Act
    int actual_ret = cplat_mmap_get_rwlock(map, &lock,
                                       &detail); // [手順] - ローカル ロック取得失敗を注入して get_rwlock を呼び出す。

    // Assert
    EXPECT_NE(
        CPLAT_OK,
        actual_ret); // [確認_異常系] - ローカル ロック取得失敗時の cplat_mmap_get_rwlock の戻り値が CPLAT_OK 以外であること。
...
TEST_F(mmapFailureInjectionTest, get_rwlock_reports_error_when_interprocess_lock_open_fails)
{
    ...
    EXPECT_CALL(mock_cplat_, cplat_interprocess_rwlock_open(_, _))
        .WillOnce(
            Return(CPLAT_ERR_UNKNOWN)); // [Pre-Assert確認_異常系] - 遅延 RW ロック生成が 1 回呼び出されること。
    // [Pre-Assert手順] - cplat_interprocess_rwlock_open にエラーを返却させる。

    // Act
    int actual_ret =
        cplat_mmap_get_rwlock(map, &lock,
                                 &detail); // [手順] - プロセス間 RW ロック生成失敗を注入して get_rwlock を呼び出す。

    // Assert
    EXPECT_NE(
        CPLAT_OK,
        actual_ret); // [確認_異常系] - プロセス間 RW ロック生成失敗時の cplat_mmap_get_rwlock の戻り値が CPLAT_OK 以外であること。
    EXPECT_EQ((cplat_interprocess_rwlock *)NULL,
...
TEST_F(mmapFailureInjectionTest, attach_unmaps_when_identity_duplication_fails)
{
    ...
    EXPECT_CALL(mock_cplat_, cplat_malloc(_))
        .WillOnce(Return(nullptr))
        .WillRepeatedly(
            DoDefault()); // [Pre-Assert確認_異常系] - cplat_malloc が識別子の複製のために 1 回目に呼び出されること。
