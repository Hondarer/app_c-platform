...
class stringCatalogRenderTest : public Test
{
  protected:
    ...
    {
        values[index].kind = CPLAT_STRING_CATALOG_ARGUMENT_KIND_INT32;
    ...
    {
        values[index].kind = CPLAT_STRING_CATALOG_ARGUMENT_KIND_STRING;
...
TEST_F(stringCatalogRenderTest, plain_text)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "開始しました。", values,
...
TEST_F(stringCatalogRenderTest, single_placeholder)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "値は {0} です。", values,
...
TEST_F(stringCatalogRenderTest, reordered_placeholder)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{1} / {0}", values,
...
TEST_F(stringCatalogRenderTest, repeated_placeholder)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{0}-{0}", values,
...
TEST_F(stringCatalogRenderTest, escaped_braces)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "value={{ {0} }}", values,
...
TEST_F(stringCatalogRenderTest, argument_kind_text)
{
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 印字できる文字を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 印字できない文字を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);  // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号付き 8 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号なし 8 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号付き 16 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号なし 16 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号なし 32 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    EXPECT_STREQ("12", dest);        // [確認_正常系] - 10 進数で表現されること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号付き 64 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);   // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 符号なし 64 bit 整数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);  // [確認_正常系] - 戻り値が CPLAT_OK であること。
    EXPECT_STREQ("4294967296", dest); // [確認_正常系] - 10 進数で表現されること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 8 bit の 16 進数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 16 bit の 16 進数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 32 bit の 16 進数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);  // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 64 bit の 16 進数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);          // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - バイト数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{0}", values,
                                            1); // [手順] - 符号付きのバイト数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);            // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - ポインターを展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);      // [確認_正常系] - 戻り値が CPLAT_OK であること。
    EXPECT_EQ(18U, strlen(dest));         // [確認_正常系] - 0x と 16 桁の 16 進数で表現されること。
    EXPECT_EQ(0, strncmp(dest, "0x", 2)); // [確認_正常系] - 先頭が 0x であること。

    values[0].kind = CPLAT_STRING_CATALOG_ARGUMENT_KIND_DOUBLE;
    values[0].value.double_value = 12.5;
    actual_ret =
        string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - 倍精度浮動小数点数を展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    ...
    actual_ret = string_catalog_render_text(dest, sizeof(dest), "{0}", values, 1); // [手順] - エラー コードを展開する。
    EXPECT_EQ(CPLAT_OK, actual_ret);      // [確認_正常系] - 戻り値が CPLAT_OK であること。
...
TEST_F(stringCatalogRenderTest, truncated_output)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(small_dest, sizeof(small_dest), "{0}", values,
                                            1); // [手順] - 容量に収まらない値を展開する。
...
TEST_F(stringCatalogRenderTest, truncated_after_full)
{
    ...
    // Act
    actual_ret = string_catalog_render_text(small_dest, sizeof(small_dest), "{0}def", values,
...
TEST_F(stringCatalogRenderTest, two_digit_placeholder)
{
    ...
    for (index = 0; index < CPLAT_STRING_CATALOG_ARGUMENT_MAX; index++)
    {
    ...
    // Act
    actual_ret_first =
        string_catalog_render_text(wide_dest, sizeof(wide_dest), "{0}/{9}/{10}", values,
                                   CPLAT_STRING_CATALOG_ARGUMENT_MAX); // [手順] - 1 桁と 2 桁を混ぜて展開する。

    // Assert
    EXPECT_EQ(CPLAT_OK, actual_ret_first); // [確認_正常系] - 戻り値が CPLAT_OK であること。
    EXPECT_STREQ("0/9/10", wide_dest);     // [確認_正常系] - 2 桁のインデックスが対応する値へ展開されること。

    // Act
    actual_ret_last =
        string_catalog_render_text(wide_dest, sizeof(wide_dest), "{31}", values,
                                   CPLAT_STRING_CATALOG_ARGUMENT_MAX); // [手順] - 最大インデックスを展開する。

    // Assert
    EXPECT_EQ(CPLAT_OK, actual_ret_last); // [確認_正常系] - 戻り値が CPLAT_OK であること。
...
TEST_F(stringCatalogRenderTest, invalid_two_digit_placeholder)
{
    ...
    actual_ret_leading_zero = string_catalog_render_text(
        ...
        CPLAT_STRING_CATALOG_ARGUMENT_MAX); // [手順] - 先行ゼロを含む 2 桁のインデックスを展開する。
