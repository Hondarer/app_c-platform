...
Windows と Linux、x86 と x64 のような環境差異を、呼び出し側が処理系依存マクロに直接依存せずに扱えるようにします。
...
アーキテクチャーに依存した処理を切り替えるときに使います。

## 設計の考え方

`platform.h` は、OS 判定とアーキテクチャー判定を分けて扱います。  
