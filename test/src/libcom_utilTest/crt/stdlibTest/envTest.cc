...
TEST_F(envTest, setenv_value_is_readable)
{
    ...
    int actual_ret_getenv = com_util_getenv("COM_UTIL_ENV_TEST", buf, sizeof(buf), &exists,
                                     NULL); // [手順] - COM_UTIL_ENV_TEST の値を取得する。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_setenv); // [確認_正常系] - com_util_setenv の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_getenv); // [確認_正常系] - com_util_getenv の戻り値が COM_UTIL_OK であること。
...
TEST_F(envTest, setenv_without_overwrite_keeps_existing_value)
{
    ...
    // Act
    int actual_ret_setenv = com_util_setenv("COM_UTIL_ENV_TEST", "second", 0,
    ...
    EXPECT_EQ(COM_UTIL_OK,
              actual_ret_setenv); // [確認_正常系] - overwrite が 0 の com_util_setenv の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_getenv); // [確認_正常系] - com_util_getenv の戻り値が COM_UTIL_OK であること。
...
TEST_F(envTest, setenv_without_overwrite_sets_value_when_absent)
{
    ...
    // Act
    int actual_ret_setenv = com_util_setenv("COM_UTIL_ENV_TEST", "created", 0,
...
TEST_F(envTest, unsetenv_removes_variable)
{
    ...
    // Act
    int actual_ret_unsetenv = com_util_unsetenv("COM_UTIL_ENV_TEST", NULL); // [手順] - COM_UTIL_ENV_TEST を削除する。
    int actual_ret_getenv = com_util_getenv("COM_UTIL_ENV_TEST", NULL, 0, &exists,
                                     NULL); // [手順] - 削除後に COM_UTIL_ENV_TEST の有無を取得する。

    // Assert
    EXPECT_EQ(COM_UTIL_OK, actual_ret_unsetenv); // [確認_正常系] - com_util_unsetenv の戻り値が COM_UTIL_OK であること。
    EXPECT_EQ(COM_UTIL_OK, actual_ret_getenv);   // [確認_正常系] - com_util_getenv の戻り値が COM_UTIL_OK であること。
...
TEST_F(envTest, setenv_rejects_invalid_name_and_value)
{
    ...
    // Act
    int actual_ret_null_name = com_util_setenv(NULL, "value1", 1, &detail);   // [手順] - 変数名に NULL を指定して呼び出す。
    int actual_ret_empty_name = com_util_setenv("", "value1", 1, NULL);       // [手順] - 変数名に空文字列を指定して呼び出す。
    int actual_ret_equal_in_name = com_util_setenv("A=B", "value1", 1, NULL); // [手順] - 変数名に '=' を含めて呼び出す。
    int actual_ret_null_value = com_util_setenv("COM_UTIL_ENV_TEST", NULL, 1, NULL); // [手順] - 値に NULL を指定して呼び出す。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_null_name); // [確認_異常系] - 変数名が NULL のとき com_util_setenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    ...
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_empty_name); // [確認_異常系] - 変数名が空文字列のとき com_util_setenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_equal_in_name); // [確認_異常系] - 変数名に '=' を含むとき com_util_setenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_null_value); // [確認_異常系] - 値が NULL のとき com_util_setenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
}
...
TEST_F(envTest, unsetenv_rejects_invalid_name)
{
    ...
    // Act
    int actual_ret_null_name = com_util_unsetenv(NULL, &detail);   // [手順] - 変数名に NULL を指定して呼び出す。
    int actual_ret_empty_name = com_util_unsetenv("", NULL);       // [手順] - 変数名に空文字列を指定して呼び出す。
    int actual_ret_equal_in_name = com_util_unsetenv("A=B", NULL); // [手順] - 変数名に '=' を含めて呼び出す。

    // Assert
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_null_name); // [確認_異常系] - 変数名が NULL のとき com_util_unsetenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(
        EINVAL,
        com_util_error_get_errno(&detail)); // [確認_異常系] - com_util_error_get_errno の戻り値が EINVAL であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_empty_name); // [確認_異常系] - 変数名が空文字列のとき com_util_unsetenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(
        COM_UTIL_ERR_INVALID_ARGUMENT,
        actual_ret_equal_in_name); // [確認_異常系] - 変数名に '=' を含むとき com_util_unsetenv の戻り値が COM_UTIL_ERR_INVALID_ARGUMENT であること。
}
