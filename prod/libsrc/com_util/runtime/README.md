...
- `com_util_module_get_path`: モジュールの絶対パスを取得します。
- `com_util_module_get_basename`: モジュールの basename を取得します。
...
- `com_util_elevated_process_is_elevated`: 現在のプロセスが管理者/root 権限で動作しているかを確認します。
- `com_util_elevated_process_run_if_needed`: 管理者/root 権限が必要な処理のため、必要に応じて昇格実行します。
- `com_util_elevated_process_run_with_result`: 同様に昇格実行し、昇格プロセスが報告した結果メッセージを取得します。
- `com_util_elevated_process_extract_result_target` / `com_util_elevated_process_report_result`: 昇格プロセス側で結果メッセージを報告します。
...
- ロックは `com_util_local_lock` を使用します。
